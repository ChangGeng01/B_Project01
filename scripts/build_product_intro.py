...
def build_document():
    ...
    first_release_modules = [
        ("核心业务：", "主数据、客户档案与客户 360、合同、销售订单与客户信用额度校验、采购与供应商、库存台账与存货计价、成本归集、项目任务与交付节点、售后工单与设备台账。"),
    ...
    security_items = [
        ...
        ("控制外泄：", "敏感字段按经批准的清单脱敏，并可执行水印、导出审批、打印阻断和剪贴板拦截；浏览器门户端受浏览器能力限制，打印阻断与已下载文件失效为尽力而为，不作承诺。"),
    ...
    scope_items = [
        ...
        ("首版范围：", "首版是以合同为中心的销售、采购、财务与工单闭环：合同、订单与客户信用额度校验、采购与供应商、库存台账与存货计价、成本归集、售后工单与设备台账、应收应付与简易总账、发票申请与开具登记、报表与经营看板，以及权限与定制能力；对外协同只有供应商门户，成品连接器只有电子签章。"),
