...
def build_document():
    ...
    add_bullet(doc, "当前 P340 已安装的 1TB HDD 永久不具备生产资格，认证不能把它变成生产权威盘、RAID1 成员或备份目标。只有换装原始容量不少于 2,000,000,000,000 bytes 的可认证企业 CMR HDD，且 NTFS 卷同时满足 1,589,137,899,520 bytes 名义下限及包含当次文件系统元数据的动态容量公式，再通过 UPS、两层备份、洁净恢复、20 人实机混合负载和同一候选版本连续 72 小时稳定运行等全部证据门，才可成为 SINGLE_DISK_DEGRADED_PRODUCTION 候选；证据闭合前状态仍是 PRODUCTION_NOT_READY。后续优先升级两块各自满足同一容量档的匹配企业 HDD 经验证 RAID1 和 64GB 内存。", bullet_num_id)
    ...
    scope_items = [
        ...
        ("性能与容量：", "ThinkStation P340 i5-10500、32GB、256GB SSD 与当前单 1TB HDD 只用于开发验证，1TB HDD 永久不能通过认证成为生产盘。约 20 名活跃用户的生产认证目标必须先换装原始容量不少于 2,000,000,000,000 bytes 的合格企业 CMR HDD，并同时满足 NTFS 动态容量公式与全部证据门；在此之前为 PRODUCTION_NOT_READY，不是登录硬上限或已达成承诺，恢复时间按数据量和实测证书签发。"),
    ]
