...
def build_document():
    ...
    add_table(
        ...
        [
            ...
            ["经营报表月底手工拼", "业务数据持续汇总到统一报表；常用报表的性能指标以合同约定的基线环境与验收清单为准", "管理层更早发现问题"],
        ],
        [2500, 3590, 3270],
        body_font_size=9.9,
    )
    ...
    add_bullet(doc, "离线只用于现场作业和记录；合同生效、付款、开票、财务过账、结账和敏感数据导出不得离线完成，必须联网并经人工审批与中心确认。", bullet_num_id)
    ...
    add_bullet(doc, "电脑端 Office 或 WPS 宿主支持批量导入、导出、在线查询、受控提交、公式、条件格式和数据透视表；移动端按平台能力清单提供查看或原生表单替代。", bullet_num_id)
    ...
    add_bullet(doc, "电脑端支持模板套用、在线编辑、批注、修订、版本比较、多人协作、电子签名、敏感信息遮盖和长期归档；移动端以查看和批注为主，写入按能力清单转电脑端完成。", bullet_num_id)
    ...
    add_bullet(doc, "AI 不能绕过权限；合同生效、付款、开票、财务过账、结账和敏感数据导出等高风险操作必须人工确认或审批。", bullet_num_id)
    ...
    add_bullet(doc, "可连接银行、税务、电子签名、支付、物流、HR、PLM、MES、物联网、GIS、BI 和文档系统；实际范围取决于对方接口、准入和逐客户联调条件。", bullet_num_id)
    ...
    add_body(
        doc,
        "四个平台提供按规格定义的等价业务结果，并根据屏幕和使用场景自动调整。电脑适合批量操作、复杂报表、Office/WPS 和配置；手机和平板适合审批、查询、扫码、拍照、签到和现场作业。财务过账与结账、开票与税务写入、报表和仪表盘创作、Office 文档编辑协作、系统配置发布和动态扩展在移动端按清单提供查看、原生表单或不适用的替代路径。",
    )
    add_bullet(doc, "不同设备使用同一套数据、权限和业务规则。", bullet_num_id)
    add_bullet(doc, "仓库、工厂和现场可在受控条件下断网工作，恢复网络后再安全同步。", bullet_num_id)
    add_bullet(doc, "离线范围限于库存、生产、质检和现场工单；合同生效、付款、开票、财务过账、结账和敏感数据导出不得离线完成，必须联网并经人工审批与中心确认。", bullet_num_id)
    ...
    security_items = [
        ...
        ("防止外泄：", "敏感内容可脱敏、水印、导出审批和追踪去向；桌面端与合规移动端的打印、复制和下载失效控制按策略强制执行，浏览器门户端受浏览器能力限制，相关控制为尽力而为且不作承诺。"),
    ...
    add_table(
        ...
        [
            ...
            ["管理层", "报表滞后、口径不一、无法看到全局", "统一指标、准实时经营视图、风险提醒和利润分析"],
        ],
        [1500, 3770, 4090],
    )
    ...
    scope_items = [
        ...
        ("离线边界：", "库存、生产、质检和现场工单可在受控条件下离线处理，边缘节点默认支持 3 天独立运行并可延长；合同生效、付款、开票、财务过账、结账和敏感数据导出不得离线完成，必须联网并经人工审批与中心确认。"),
        ("人工确认：", "AI 与自动化可在授权范围内自动生成单据、任务和建议；合同生效、付款、开票、财务过账、结账和敏感数据导出属高风险动作，只生成待审批任务，必须经人工审批与中心确认后执行。"),
        ("外部连接：", "银行、税务与数电发票、支付、电子签章等接入需完成相应准入，并按具体银行和属地逐项开户、报备与联调；未完成时按交付说明和合同采用回单、对账文件导入等替代方式。"),
    ]
