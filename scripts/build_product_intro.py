...
def build_document():
    ...
    flow_steps = [
        ...
        ("财务处理：", "业务单据自动形成应收、应付、发票、付款和会计处理建议。"),
    ...
    add_body(doc, "销售下单后，系统自动检查价格权限、合同、库存、交期和客户信用。")
    ...
    add_bullet(doc, "按区域、技能、备件、服务级别和工程师日程自动分派，超时自动升级。", bullet_num_id)
    add_bullet(doc, "现场人员可用手机签到、扫码、拍照、换件和客户签字，断网时也能工作。", bullet_num_id)
    ...
    add_table(
        ...
        [
            ...
            ["售后", "投诉丢失、找不到历史、工单超期", "自动建单、智能分派、现场移动作业和服务追踪"],
