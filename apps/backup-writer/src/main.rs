//! backup-writer — 无监听、spool 目录、IPC 客户端、每日周期心跳占位、优雅停机。
//!
...
use config::{BackupWriterConfig, DEFAULTS, SHUTDOWN_DRAIN_MS};

const PROCESS: ProcessKind = ProcessKind::BackupWriter;

/// 心跳周期。按阶段 1 计划第 3.2 节的每日周期心跳占位。
pub const HEARTBEAT_PERIOD: Duration = Duration::from_secs(86400);
...
async fn serve(
    ...
    serving.spawn_critical("backup-writer 心跳循环", async move {
