...
const BIN: &str = env!("CARGO_BIN_EXE_ops-agent");

fn run_default_check() -> Output {
    let isolated =
        std::env::temp_dir().join(format!("ep-ops-default-kms-check-{}", std::process::id()));
    ...
    command.output().expect("能够启动 ops-agent --check")
}
