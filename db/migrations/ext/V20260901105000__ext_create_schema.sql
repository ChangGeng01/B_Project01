...
-- db/migrations/ext/V20260901105000__ext_create_schema.sql
-- 建 ext schema，设属主为 ep_mod_ext，授 USAGE 与默认权限（阶段 2 计划第 3.2 节）。
...
alter schema ext owner to ep_mod_ext;

-- 三条固定授权：USAGE 给应用读写与只读分析两个角色；
-- 属主角色日后在本 schema 建的表，默认授予 ep_app_rw 三种 DML（无 DELETE）、
-- ep_analyst_ro 只读。
grant usage on schema ext to ep_app_rw, ep_analyst_ro;
alter default privileges for role ep_mod_ext in schema ext
