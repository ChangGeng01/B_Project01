...
-- db/migrations/platform_msg/V20260901103000__platform_msg_create_schema.sql
-- 建 platform_msg schema，设属主为 ep_mod_platform_msg，授 USAGE 与默认权限（阶段 2 计划第 3.2 节）。
...
reset role;
create schema if not exists platform_msg;
alter schema platform_msg owner to ep_mod_platform_msg;

-- 三条固定授权：USAGE 给应用读写与只读分析两个角色；
-- 属主角色日后在本 schema 建的表，默认授予 ep_app_rw 三种 DML（无 DELETE）、
-- ep_analyst_ro 只读。
grant usage on schema platform_msg to ep_app_rw, ep_analyst_ro;
alter default privileges for role ep_mod_platform_msg in schema platform_msg
