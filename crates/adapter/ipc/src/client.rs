...
impl std::fmt::Display for ClientError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ClientError::Connect { path, detail } => {
            ...
            ClientError::IdMismatch { want, got } => {
                write!(f, "响应 id 不匹配，期望 {want} 实际 {got}")
            }
        }
    }
}
...
impl IpcClient {
    ...
    async fn exchange(&self, id: &str, body: Vec<u8>) -> Result<Value, ClientError> {
        ...
        let raw = read_frame(&mut stream, self.max_frame_bytes)
            .await
            .map_err(ClientError::Frame)?;
        ...
        if !resp.ok {
            return Err(ClientError::Remote(resp.error.unwrap_or(
                crate::message::IpcErrorBody {
...
mod tests {
        ...
        Fut: std::future::Future<Output = ()>,
    {
        ...
        let server = IpcServer::new(
            &path,
            1024,
            MethodTable::new().with("system.echo", Arc::new(Echo)),
        );
    ...
    async fn request_and_response_round_trip_over_the_socket() {
        with_server("ok", |path| async move {
            ...
            let got = client
                .call("system.echo", serde_json::json!({"n": 1}))
                .await
                .unwrap();
            assert_eq!(got["n"], 1);
        })
