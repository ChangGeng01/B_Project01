...
use sql::{DatabaseReachable, MigrationVersionMatched, RlsEnabledAndForced, RuntimeRolePrivilegesBounded, SqlContext};
...
pub fn baseline_registry(
    ...
    let items = vec![
        ...
        SelfCheckItem::new(
            ...
            "系统时钟与授时源偏差小于阈值",
            Severity::Blocking,
            Arc::new(ClockSkewWithinLimit { max_ms: clock_skew_max_ms }),
        ),
        SelfCheckItem::new(
...
mod tests {
    ...
    fn registration_order_matches_the_baseline_roster() {
        let reg = baseline_registry(ProcessKind::CoreServer, "内置默认".into(), 1_000, None);
        let want: Vec<&str> =
            BASELINE_ITEMS.iter().take(REGISTERED_THIS_STAGE).map(|(n, _)| *n).collect();
        assert_eq!(reg.names(), want);
    }

    #[test]
    fn offsite_sink_requirements_is_not_registered_this_stage() {
        let reg = baseline_registry(ProcessKind::CoreServer, String::new(), 1_000, None);
        assert!(!reg.names().contains(&"offsite-sink-requirements"), "本阶段整条推迟，不留占位");
    ...
    async fn processes_without_sql_session_mark_sql_items_not_applicable() {
        for p in [
            ...
            for name in [
                ...
                assert_eq!(item.outcome, Outcome::NotApplicable, "{} 的 {name}", p.name());
            }
        }
    }
