...
//! **按裁定 F-18 结论四它已收为单方法 `module_state`**——`license_status` 与
//! `is_feature_enabled` 两个方法撤下，因为它们必须读 `license_grants` 与
//! `feature_flags` 两张**本部署不建的表**（F-18 结论三）。
//! 运行期的模块启停入口按计划第 3.5.9 节归阶段 13b 与阶段 14；
//! 计划第 3.4.11 节的两处过滤点明写「都在 job-worker」，不在本 crate。
//!
//! # 哪些留下、哪些撤——同一条判别式的两面（F-18 结论五）
//!
//! **只吃入参、不需要本部署不存在的载体的，留下**：[`classify_subscription`]、
//! [`in_restricted_run`]、[`restricted::restricted_run_verdict`]、
//! [`usage::named_user_usage_verdict`] 四个纯函数与其全部用例，
//! 为规格第 1 章「同时可向其他企业销售」那一面保留契约与判据。
//!
//! **必须读一张本部署不建的表的，撤**：上述两个 trait 方法。
//!
//! 判别式要一致地用：F-18 的取证曾建议把 [`usage`] 整模块撤，
//! 但 `named_user_usage_verdict` 只吃两个整数，与 `classify_subscription`
//! 只吃 [`LicenseFacts`] 同形，**不能对一个纯函数破例**。
//!
//! # 未覆盖（明写，不以「校验过了」的外观掩盖）
//!
//! 零、**辛-8 的三件已由裁定 F-18 处置**：临期告警无触发源与凭证签名不被校验
//! 两件整件随商业面不交付；可信时间那一件劈两半——许可判定那半随商业面撤，
//! 「本机时刻是否可信」那半拆出**辛-15** 单独登记，因为它与商业许可无关：
//! 审计事件按 `occurred_at` 分段而 `verify_segment` 零时间断言，
//! 而唯一的时钟异常检测在本平台「永久停在未覆盖」，
//! 时钟回拨告警还正好路由到那一项。**内部使用同样暴露。**
//!
