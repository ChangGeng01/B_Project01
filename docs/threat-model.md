...
正式生产基线是客户自控物理机或客户自控 IaaS 中的一台 Windows Server 2022。服务器控制中心、权威服务、PostgreSQL、文件存储、自动化和启用的隔离宿主均在该 Windows Server 上运行；进程数量不是产品契约。客户之间使用独立部署；同一客户的多个法人共享一个逻辑数据库，并以 RLS、动态属性权限和独立密钥域隔离。服务器控制中心属于权威节点，远程浏览器只是显示入口，不是第五个业务客户端。
...
| IaaS provider 与客户 tenant root | 宿主、磁盘快照、虚拟网络、VM 电源与控制面 | 应用无法技术阻止其复制/回滚/停机；风险必须由客户控制、vTPM 证据、职责分离与合同披露约束 |
...
| IaaS 控制面 → 单机 VM | 宿主、虚拟盘、快照、网络、vTPM | 客户自控境内 tenant/region、vTPM attestation、无托管组件、故障域与凭据域分离、同机快照不得冒充离站备份、完整恢复演练 |
...
16. IaaS 只替换硬件承载体；无 provider/region/驻留/vTPM/控制权/备份故障域证据，或出现托管组件时失败关闭。

### 关键假设与残余边界

- 客户负责服务器物理安全、Windows 基线、网络分区、WAF/防火墙、时间同步、证书与客户管理员治理；产品必须提供可验证的最小配置和失败门禁，但不能阻止拥有客户操作系统、数据库、KMS 与备份存储最高权限的主体联合滥用。
- 首版单机部署没有高可用。物理服务器、IaaS VM 或宿主失效都会停机，控制目标是依靠服务器外且故障/凭据域分离的副本恢复，不是持续可用。
...
- IaaS provider/tenant root 处于平台控制之外，可能复制磁盘/内存、回滚快照、改变网络或关机；vTPM 只增强证据，不能消除该残余风险。签名发行者或获批 MCP server 自身被攻破也仍是供应链/外部信任风险。
...
- PostgreSQL 16 的 Windows 安装不能依赖安装员临场选择：`Postgres16WindowsPackageLockV1`、`Postgres16WindowsInstallContractV1` 与同代 `Postgres16WindowsInstallReadbackV1` 必须分别由已签 `WindowsAuthorityArtifactSetV1` 和 `ReleaseWindowsServiceInstallEvidenceV1` 传递认证。V1 固定禁止 package/lock 降级，`.control` 文件、SBOM 与扩展 exact-set 一致。`ep-postgres16` 固定为按需启动的虚拟服务账户、无自动恢复重启；DATA_HDD 解锁、存储清单、vault、配置与 TLS 全部验证前，PostgreSQL 进程数必须为零。九个路径角色的投影/读回按 enum 顺序双射：Engine 只在 SSD Set A，PGDATA（含 live `pg_wal` 与数据库临时关系）、归档 staging、独立 process/restore scratch、日志、TLS 与有效配置全部在 DATA_HDD。关键 GUC/HBA/ident 的 effective canonical vectors 必须逐字节等于投影，监听 exact-set 只有 loopback；禁止 `initdb --waldir`、用户 tablespace、reparse 后代、外部 CIDR、`trust`、ambient include 或 `postgresql.auto.conf` 偷换有效值。
...
- IaaS 存储只有取得介质/服务合同和部署证据后才能声明满足 HDD 策略；普通“云盘”标签不构成证据。
...
- `BackupTopologyV1` 由 deployment/epoch/generation/storage-manifest 和 active-config current-head 绑定；install/PITR/activation 的 enclosing binding 再把它与候选 exact-join。active config 必须逐字指向 supplied topology；revision 1/null predecessor 后只能 prior+1 且 exact 引用前一完整 envelope，旧签名、fork、回滚或目录“最新”均拒绝。角色按 enum exact 六行且 principal/credential 唯一；writer credential exact-join mTLS client SPKI，target-agent credential exact-join server SPKI。它同时固定一台服务器外连续目标和按序 `ROTATION_A|ROTATION_B` 两块 distinct HDD。连续目标与生产主机不得共享 host、管理域、凭据域或故障域；live readback 必须携带并逐字重复连续目标及 A/B 的故障/管理/凭据/保管/位置域，由 topology-pinned support evidence 认证，Boolean 不作原始证据。
- 每次服务安装、PITR、生产启用及 retry 都必须新采 strict plain `StorageSafeguardReadbackV1`，使用新 32-byte challenge、session、对象和同 boot/attempt binding；expiry 必须 checked-equal `observed + topology.max_age`，max age 在 1..=300 秒，消费时 trusted now 与 current topology head 均仍有效。唯一 support-evidence root 对 target receipt 使用 target-agent 单签、对介质转换/安全弹出/物理断开/保管/健康使用 topology 中两个互异人类保管者双签；所有 refs 必须 typed-load正确 tag 并 exact-hash containing-field projection。writer、target-agent、partial-maintenance、retention、signer、recovery 六类的完整 canonical 权限负探针逐项被存储侧拒绝且前后摘要不变；target-agent 的任意直接操作全部拒绝，正常 append 只走一次性 capability。
- 容量不只计算公式：连续盘 `total>=retained+validation+growth+reserve`、`free>=validation+growth+reserve`、quota+reserve 不越 total、used 不越 quota、reserve 实际可用；离线盘 `total>=recoverable+validation+growth` 且 free 足够 validation+growth。所有加法 checked，partial count/bytes/oldest-age optionality一致且 expired 为零。最小有效保留期 checked-equal `max(法定/合同保留,90 天,2×检测延迟 P99+洁净恢复验证窗口,2×离线轮换周期)`，至少两个已验证代际、离线年龄不超过 604800 秒；A/B 永远 `bundle_contains_recovery_material=false`。生产启用时 A/B 只能处于 `VERIFIED_DISCONNECTED|SEALED_VERIFIED`，零挂载、授权撤销、安全弹出、物理断开、健康且恰好两个互异人类 custody binding/signature；介质 state transition sequence/predecessor/hash/合法边全部闭合。
...
- UPS 的攻击面包括把 Windows 聚合电池状态冒充设备身份/自检/输出状态、恶意适配器取得任意设备或网络权限、响应丢失后重复安排断电，以及重启后事后拼接 ACK。`UpsAdapterManifestV1`、`UpsStatusReadbackV1`、`UpsOutletCycleCommandV1` 与 `UpsOutletCycleCommandAckV1` 必须使用唯一 UPS schema；状态与控制端口分权，命令本身不得携带 endpoint、credential、路径、argv 或厂商任意 payload。
...
- status 每 5 秒采集、有效期严格为 `observed+15s`，sequence 只在同一已验证 process-start binding 内递增；进程重启必须重建安全绑定。同一 `(ups_adapter_identity,command_id)` 和相同 command digest 必须 query/adopt 并返回逐字节相同 ACK。adapter 在任何 provider 调用前耐久化不可重采的 private monotonic start marker；ACK 必须同 boot/source 且在 checked `start <= observed <= min(start+30s,command deadline)` 内被观察，UTC 仅供报告。query/adopt 也不得越过这 30 秒；POWER 的 600 秒仅是 User32/composite/preshutdown 崩溃对账窗，不能放宽、重置或复活内层限制。相同 ID 不同 digest 为 `COMMAND_ID_CONFLICT`，30 秒时无法证明是否已发送或无法查询为 `COMMAND_STATE_UNKNOWN` 且禁止重发。boot 已改变而 composite ACK 未耐久化时必须保持失败，不能用事后观察到的断电/重启事实重建 PASS。
...
供应链风险包括依赖替换、构建机泄密、迁移脚本植入、签名密钥滥用、白标制品混淆、旧版本降级和伪造离线补丁。缓解包括本地 Git、锁定依赖、离线依赖仓库、固定 `cargo xtask ci`、SBOM、可复现构建、制品摘要、生产 Authenticode、版本单调性、客户侧离线验签、双人批准和可审计安装/回退。
...
### 17. IaaS carrier 与云控制面

风险包括把云盘快照当离站备份、生产与备份共享 tenant root/凭据域、provider/region 不满足数据驻留、vTPM 缺失却继续启用、宿主快照复制内存/密钥，以及用云托管数据库/KMS 偷换拓扑。控制是 carrier 两值闭集、部署表 CHECK、provider/region/驻留/客户控制/vTPM attestation 证据、禁止托管组件、生产与备份在 site/zone 或 region、账户/凭据域和介质三维满足批准隔离，以及与物理机完全相同的恢复和发布门禁。
