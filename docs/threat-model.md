...
当前物理服务器基线是 ThinkStation P340 Tower、i5-10500、32GB RAM 和 256GB runtime SSD；已安装的单块 1 TB HDD profile 永久 `production_eligible=false`，不能承载权威生产数据、充当 RAID1 成员或备份；被合格新盘替代并经安全擦除与处置批准后，唯一允许的复用是可随时丢弃的非权威暂存。同机单盘降级生产候选必须改装并重新认证一块 CMR DATA_HDD，raw device `>=2,000,000,000,000` bytes，NTFS authority volume 以 `1,589,137,899,520` bytes 为名义 floor 且 checked 满足 `volume_total_bytes >= max(1,589,137,899,520,1,481,763,717,120+107,374,182,400+measured_unclassifiable_filesystem_allocation_bytes)`。生产容量不是“其余组件”概数：九桶 exact-set 依次为 `161061273600,64424509440,42949672960,375809638400,375809638400,375809638400,25769803776,8589934592,51539607552` bytes，checked-sum `1,481,763,717,120` bytes，另有不可借用 `107,374,182,400` bytes。十四容量类/十四 canonical selector 只唯一计费产品管理 `data_root` 对象与登记 VSS extent；只有整卷 collector 证明为 NTFS/BitLocker 自身的 allocation 才进入独立 measured 标量且不能借桶，普通卷外对象、无法访问或解释不了的 extent 进入独立 `unclassified_allocation_bytes` 并触发 deployment-wide hold。60 GiB 共享桶以 PostgreSQL log 20 GiB、两类 writer report spool 各 2 GiB、两类 MCP completion spool 各 1 GiB、其余历史/控制 34 GiB 闭合，24 GiB backup staging 不得借位。未知、重复、跨桶、legal hold 借桶、元数据 catch-all class 或局部限额先于聚合 admission 都失败关闭。总 live `pg_wal` 硬上限为 `700 GiB`；同一 segment 正常恢复/slot 优先计费，archiver-failure 只计额外前沿；archive-failure growth / total live WAL 在 `300/650 GiB` 进入 fail-closed hold，在 `350/700 GiB` 触发 hard shutdown，采样 `<=30 s`，且实测峰值 WAL 速率乘 `(采样周期+关停耗时)` 必须 `<50 GiB`。RAID1 的每块成员盘必须独立满足相同 2 TB 档，原 1 TB 盘不得复用为成员或备份目标；上述非权威暂存例外不改变这一禁令。

未来客户自控 IaaS 只能通过新的 graph/profile version 启用互斥的独立 `IAAS_WINDOWS_SERVER_HDD_STRICT` 认证载体，并且不继承 P340 CPU/主板/物理 UPS 证据；当前 selector 对它固定返回 `PROFILE_NOT_IMPLEMENTED` / `STORAGE_MEDIA_UNVERIFIED`。所有持久客户数据和衍生数据必须写入加密 HDD；SSD 只允许 signed Set A 的系统、程序、静态依赖、可重新下载模型与可重新登记的非秘密 metadata，以及 exact 四类 mutable Set B：有界 POWER capsule、有界 package-recovery continuation capsule、recovery-domain-signed kernel pointer/journal head、可重建的 content-addressed signed native-code slot/cache。每类都有独立大小/保留/off-host mirror/终态删除/SSD-loss 重建契约，闭集外持久字节或客户/业务 authority 字节失败关闭。只有通过上述容量档与全部证据的单 HDD 才能显示“单磁盘降级生产”；服务器外备份、UPS 和完整恢复演练仍是上线前置条件。
...
- PostgreSQL data/WAL、索引、附件、审计、应用日志、导出、temp、spool 和插件/容器 workdir 全部路由到加密 HDD。Windows pagefile/swapfile/休眠和内核、完整、mini、WER LocalDump 按总体设计 §13.2.1 在两卷均禁用，不接受 HDD 重路由例外。
...
- PostgreSQL 历史日志删除是独立高风险边界：已签 exact policy 固定普通 `max_age=30 days`、`max_total=20 GiB`（`21,474,836,480` bytes）、当前打开日志不删、最近至少 7 日必须保留，站点法定/合同保留和对象 legal hold 只能延长。该日志类与 archive/backup report spool、core/worker MCP completion spool 及历史/audit/control 余量共享同一 `64,424,509,440`-byte 聚合桶；每个局部上限与聚合上限都必须满足。只有现有 `EPAuthorityControl` 内签名、双人批准、带 exact 预览清单/前后 digest/审计回执的 typed `POSTGRES_LOG_RETENTION_CLEANUP` 可删除既定 closed log；PostgreSQL service identity、backup writer 与普通 Authority 的删除/改 ACL/解除 hold/改政策负探针必须全部被拒绝。legal hold 导致 30 日/20 GiB 无法同时满足时保留 protected bytes 并 fail-close，不得删 current/7-day/held 集，也不得向别桶借位。DATA_HDD free 低于 `max(existing yellow_free,100 GiB)` 暂停批量，低于 `max(existing red_free,100 GiB)` 立即 deployment-wide hold；100 GiB 是不可借用阻断底线，不能由旧 50/40 GiB 日志策略下限放宽。
- 安装、启动、升级和 F-57 最终发布门都运行写入探针与路径枚举；发现客户字节落 SSD 即阻止生产就绪。
- Windows 持久文件政策固定八行：六类 DISABLED 的策略与重启后实际文件缺席必须一致；VSS diff area 和产品扫描 quarantine 只在 verified DATA_HDD。DATA_HDD 延迟解锁不得反向成为 pagefile 自举依赖；32 GiB 无 pagefile 档的 72 小时 commit/headroom 不合格即禁止生产，不能仅声称无客户载荷。Defender 保持保护 OS/代码，客户根仅使用已签 exact exclusion set 与产品扫描流水线，不能扩大排除范围。
...
14. PostgreSQL 历史日志只能由 `EPAuthorityControl` typed 操作按签名清单清理，current/7-day/legal-hold 集不可删；批量暂停和全局 hold 都必须取现有计算门与 100 GiB 不可借底线中更严格值。
