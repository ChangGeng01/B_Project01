...
登记红存在期间，流水线聚合退出码取最重，每次推送整体报红——开发者据此**看不出**哪一处红
是自己刚造成的。为此新增一件对照工具，**它不是门禁**：不在 `pipeline-stages.tsv` 里，
不进 `ci.yml`，不使任何门禁变绿。全绿的唯一途径仍是把红真正修掉。

| 文件 | 作用 |
| --- | --- |
| `.github/ci/known-red-baseline.tsv` | 每道判定面**已登记的不符计数**，取值来自实测而非转抄裁定 |
| `.github/ci/compare-red-baseline.sh` | 逐面实测并与上表对照，只回答「比基线多出来的红有没有」 |

退出码沿用 `verify-pipeline-commands.sh` 那一套，不另立第二套：
`0` 无新回归、`2` 有新回归、`3` 未覆盖（实测取不到，或 `--gates-only` 跳过了 `cargo-test` 行）、
`64` 用法错误。**跳过即未覆盖**：只测一部分时一律不得判无回归。
