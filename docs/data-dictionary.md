...
`invoice.invoice_reversals` 使用 `direction`、`reversal_kind`、`source_sales_invoice_id`、`source_purchase_invoice_id`、服务端汇总三金额及可空 `invoice_number_registry_id`，两种原票 id 按方向恰一非空；删除单 `source_invoice_id` 与头 `red_tax_rate`。`invoice.invoice_reversal_lines` 的业务列为 `invoice_reversal_id`、两组 source invoice/line id、`quantity_effect_kind`、`pricing_effect_kind`、`quantity`、`tax_rate`、`net_amount`、`tax_amount`、`gross_amount`，关键列全部 NOT NULL，来源组用 NULL-safe XOR、默认 `MATCH SIMPLE` 复合外键与延迟头行一致性触发器约束。
