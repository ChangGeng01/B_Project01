...
| `docs/f57-feature-owner-registry.v1.tsv` | Exact 18-row business fact-owner/crate/schema/repository mapping（F-69：含 `inventory-costing`） |
...
fn feature_owner_registry_is_the_exact_master_set() {
    ...
    assert_eq!(owners.rows().len(), 18); // F-69: master §2.1 gained `inventory-costing`
...
fn foundation_schema_is_the_unique_zero_import_root_and_nominal_owner() {
    ...
    assert_eq!(foundation_owned_feature_owner_id_registry().len(), 18); // F-69
...
#[derive(Clone, Debug, Eq, Hash, PartialEq, serde::Deserialize, serde::Serialize)]
#[serde(try_from = "String", into = "String")]
pub struct FeatureOwnerIdV1(String); // private; exact member of the 18-row owner registry (F-69)
...
The CLI acceptance suite must also exercise the real executable boundary: before this task, the repository's unregistered `f57` token returns unknown-command/exit `2`; after this task, a syntactically valid branch whose owner is not yet delivered returns explicit `NOT_DELIVERED`/exit `70`, while malformed options still return `2`. Both paths must create zero PASS records, signed receipts or gate evidence. In particular, `cargo xtask f57 verify --level l0 --changed-from HEAD` remains `NOT_DELIVERED` until Task 6 supplies the L0 dispatcher; registering the parser is not an L0/L1 or G0-green claim.
...
- Create: `crates/platform/capability-graph/tests/fixtures/semantic-seven-table-normalization-v1-golden.json`
...
fn semantic_contracts_are_typed_self_contained_and_cycle_free() {
    ...
    assert_eq!(authored.registered_table_keys(), [
        ...
        "business_state_semantic_symbol_registry_v1",
    ...
    assert_eq!(authored.normalized_seven_table_rows().len(), 101);
    assert!(authored.source_layouts_and_codecs_equal_master_seven_table_registry());
    assert!(authored.normalized_rows_contain_no_opaque_policy_or_state_utf8());
    assert_eq!(
        canonical_json_bytes(authored.normalized_seven_table_rows()).unwrap(),
        include_bytes!("fixtures/semantic-seven-table-normalization-v1-golden.json"),
    );
    assert_eq!(authored.state_semantic_symbol_row_count(), 12);
    assert_eq!(authored.state_semantic_symbol_kind_counts(), [
        (StateSemanticSymbolKindV1::Action, 3),
        (StateSemanticSymbolKindV1::Guard, 3),
        (StateSemanticSymbolKindV1::Invariant, 3),
        (StateSemanticSymbolKindV1::ReverseFactTrigger, 3),
    ]);
    assert!(authored.purchase_order_reverse_transition_symbol_sets_equal_registered_exact_set());
    assert!(authored.purchase_order_reverse_action_ids_are_nonnull_and_exact());
    assert!(authored.transitions_without_contract_authored_actions_use_null_action_id());
    assert_eq!(
    ...
    assert!(compiled.state_semantic_symbols_resolve_once_by_domain_and_reference_kind());
    ...
    assert_code(graph_with_missing_duplicate_or_orphan_state_semantic_symbol(), "CAPABILITY_STATE_SEMANTIC_SYMBOL_SET_MISMATCH");
    assert_code(graph_with_null_purchase_order_reverse_action(), "CAPABILITY_STATE_SEMANTIC_SYMBOL_SET_MISMATCH");
    assert_code(graph_with_synthesized_action_for_unnamed_transition(), "CAPABILITY_STATE_SEMANTIC_SYMBOL_SET_MISMATCH");
    assert_code(graph_with_wrong_kind_or_cross_domain_state_semantic_symbol(), "CAPABILITY_STATE_SEMANTIC_SYMBOL_KIND_MISMATCH");
    assert_code(graph_with_empty_or_opaque_state_semantic_definition(), "CAPABILITY_STATE_SEMANTIC_SYMBOL_DEFINITION_INVALID");
}

#[test]
fn reviewed_native_format_parser_corpus_is_exact_isolated_and_private_key_free() {
    ...
    assert!(parser_signature_structure_terminates_only_at_its_platform_exact_der_root());
    assert!(corpus_is_parser_only_and_has_no_os_install_or_lifecycle_evidence_qualification());
    assert!(fixture_roots_are_never_installed_in_any_os_or_distribution_trust_store());
    assert_code(native_lifecycle_using_a_corpus_package_or_root(), "F57_CLIENT_NATIVE_INPUT_IS_FIXTURE");
...
`docs/schemas/f57-semantic-normalized-payloads.v1.schema.json` is the sole schema owner of the strict normalized `StateDomainDefinitionV1`, `TimeoutPolicyDefinitionV1` and `WorkflowDefinitionV1` object families consumed through `CANONICAL_JCS_OBJECT`. It imports the capability-graph schema exactly once for graph-owned nominals and has no reverse import. `StateDomainDefinitionV1` closes the exact fields named by business contract §14.6, with tagged `PERSISTED_LIFECYCLE|DERIVED_CLASSIFICATION`, typed transition/precedence/guard/invariant/reverse-trigger rows and no prose field. Each transition has the required nullable field `action_id: Option<SemanticTokenV1>` plus `guard_ids`, `invariant_ids`, and `reverse_fact_triggers` as `Vec<SemanticTokenV1>`. `action_id=null` is legal exactly when the source contract did not author a named action; every present token is role-checked against the graph-owned `StateSemanticSymbolKindV1` row in `STATE_SEMANTIC_SYMBOL_REGISTRY`, and no importer may synthesize an alias for an unnamed action. `TimeoutPolicyDefinitionV1` closes the model-tagged parameter union and exact fields in §8.2.2.1. `WorkflowDefinitionV1` closes §8.4's complete definition, step/edge/condition/compensation/upgrade/rollout types; arbitrary JSON, script/SQL/network expressions and untyped maps are unrepresentable. `semantic.rs` is the sole Rust owner of these DTOs and validators. The schema and Rust are generated/byte-golden together; a schema-name alias, opaque string, copied nominal, unknown field or graph↔payload schema cycle blocks G0-02.

For `PURCHASE_ORDER_V1`, the reviewed normalization golden must contain exactly the three non-forward transition bindings authored in business contract §14.6: `PURCHASE_ORDER_APPROVED_CHANGE_RECONFIRM`, `PURCHASE_ORDER_PARTIAL_RECEIPT_CHANGE_RECONFIRM`, and `PURCHASE_ORDER_CREDIT_REVERSAL_REOPEN`. Each of those three `action_id` fields is non-null and exact; their `guard_ids`, `invariant_ids`, and `reverse_fact_triggers` must byte-equal the arrays written in that source cell. Every referenced token must resolve exactly once to `business_state_semantic_symbol_registry_v1`, whose current 12-row exact-set contains exactly three `ACTION`, three `GUARD`, three `INVARIANT` and three `REVERSE_FACT_TRIGGER` definitions, all bound to `PURCHASE_ORDER_V1`; registered row keys and referenced tokens must be set-equal. A null purchase reverse action, a synthetic action on a transition whose source cell names none, an empty array, wrong-kind/cross-domain row, duplicate/orphan token, prose-derived alias, shared catch-all trigger, reordered array, or transition without its exact reverse trigger fails `semantic_authoring_preflight` before graph digest construction. The historical `docs/f57-api-state-domains.seed.tsv` remains byte-immutable and is not backfilled with these fields.

G0-02 creates and reviews one immutable 20-file native-format parser corpus before finalizing the graph: four public DER roots at the exact `testkit/fixtures/client-lifecycle/trust/{android,ios,macos,windows}-fixture-root.der` paths and the exact 16 package paths listed in Files. Four platform lanes create format-valid synthetic containers with externally held fixture signing material; only public roots and final bytes enter the repository. IDs use only `ep.f57.fixture.*`; no private key, password, provisioning secret or production trust anchor enters the tree/manifest/log/staged set. The sole trust context is a memory-only parser harness containing the exact synthetic platform root, never an OS/native distribution store, even on a resettable device. An IPA with a synthetic CMS/profile is deliberately not qualified for Apple installation. Each lane binds nonzero source-tree/toolchain digests and its closed reproducible recipe; the verifier checks native container/metadata and synthetic signature structure. Corpus role/outcome wires are parser/policy expectations, not native install/upgrade/readback success. G0 does not emit `ClientPlatformLifecycleEvidenceV1` from these inputs. G5/G6 real same-identity native lifecycle inputs are separate and fail closed when unavailable; fixture IDs/roots/versions cannot fill that gap. Ordinary G0/G5 runs never regenerate/resign the corpus; any byte/root/recipe change requires reviewed graph-version amendment and complete affected closure.
...
Expected: after native-format parser corpus preflight, creates `docs/capability-graph/f57-core.v1.json` with required `architecture_inputs=[]`, `capability_package_slot_templates=[]`, exact canonical four trust-root/16 package source rows, and 185 unique requirement capability references. The imported graph is reviewable but cannot be used as a runtime generation.
...
#[test]
fn p340_persistent_hdd_capacity_projection_and_config_limits_are_exact() {
    let projections = project_all(&compiled_real_graph()).unwrap();
    let policy = strict_p340_policy(&projections).unwrap();
    assert_eq!(policy.persistent_hdd_buckets.len(), 9);
    assert_eq!(policy.persistent_hdd_capacity_classes.len(), 14);
    assert_eq!(policy.persistent_hdd_root_rules.len(), 14);
    assert_eq!(policy.persistent_hdd_buckets.iter().map(|r| r.maximum_bytes).collect::<Vec<_>>(), [
        161061273600, 64424509440, 42949672960, 375809638400, 375809638400,
        375809638400, 25769803776, 8589934592, 51539607552,
    ]);
    assert_eq!(checked_sum(policy.persistent_hdd_buckets.iter().map(|r| r.maximum_bytes)).unwrap(), 1481763717120);
    assert_eq!(checked_class_sum(&policy, P340PersistentHddBucketIdV1::HistoryControlLogShared).unwrap(), 64424509440);
    assert_eq!(policy.persistent_hdd_capacity_map_sha256, sha256_jcs_capacity_map(&policy).unwrap());
    assert!(policy.root_rules_are_exact_selector_class_bijection());
    assert!(policy.root_rules_cover_only_product_data_root_and_registered_vss_extents());
    assert!(policy.normal_wal_selector_exact_matches_pg16_names_and_lifecycles());
    assert!(healthy_recycled_future_wal_and_archive_status_goldens_charge_once(&policy));
    assert!(recognized_pg16_history_backup_partial_temp_and_windows_deleted_goldens_charge_once(&policy));
    assert_code(project_p340_with_unknown_wal_name_or_lifecycle(), "P340_CAPACITY_ROOT_RULE_NOT_TOTAL_UNIQUE");
    assert_code(project_p340_with_normal_and_failure_wal_duplicate_owner(), "P340_CAPACITY_ROOT_RULE_NOT_TOTAL_UNIQUE");
    assert_eq!(checked_required_data_hdd_volume_bytes(&policy, 0).unwrap(), 1589137899520);
    assert_eq!(checked_required_data_hdd_volume_bytes(&policy, 1).unwrap(), 1589137899521);
    assert_eq!(policy.backup_spill_max_bytes, 25769803776);
    assert_eq!(policy.writer_report_spool_default_bytes, 268435456);
    assert_eq!(policy.writer_report_spool_min_bytes, 67108864);
    assert_eq!(policy.writer_report_spool_hard_max_bytes, 2147483648);
    assert_eq!(policy.mcp_completion_spool_max_bytes_per_owner, 1073741824);
    assert_eq!(policy.postgres_text_log_max_bytes, 21474836480);
    assert!(generated_production_config_templates_exact_match_capacity_policy(&policy));
    assert_code(project_p340_with_unknown_or_duplicate_capacity_class(), "P340_CAPACITY_CLASS_SET_MISMATCH");
    assert_code(project_p340_with_cross_bucket_or_over_budget_class(), "P340_CAPACITY_BUCKET_SUM_MISMATCH");
    assert_code(project_p340_with_root_rule_gap_or_overlap(), "P340_CAPACITY_ROOT_RULE_NOT_TOTAL_UNIQUE");
    assert_code(project_p340_with_filesystem_metadata_capacity_class(), "P340_CAPACITY_ROOT_RULE_NOT_TOTAL_UNIQUE");
    assert_code(required_volume_with_u64_max_metadata(), "P340_ARITHMETIC_OVERFLOW");
    assert_code(project_p340_with_spool_below_min_or_above_max(), "P340_REPORT_SPOOL_LIMIT_INVALID");
    assert_code(project_p340_with_backup_spill_one_byte_over(), "P340_BACKUP_STAGING_LIMIT_MISMATCH");
}
...
The `policy/p340-certification-policy.v1.json` family additionally projects the master's exact nine `P340PersistentHddBucketIdV1` rows, fourteen `P340PersistentHddCapacityClassIdV1` rows and fourteen selector→class rows. The fourteen rules form a total-unique map only over product-managed canonical `data_root` objects plus registered DATA_HDD VSS extents; NTFS/BitLocker volume metadata is deliberately not a fifteenth class. It recomputes the checked `1481763717120`-byte bucket sum, every per-bucket class sum, `persistent_hdd_capacity_map_sha256`, and the checked required-volume helper `max(1589137899520,1481763717120+107374182400+measured_unclassifiable_filesystem_allocation_bytes)`; metadata zero/one byte therefore yields `1589137899520/1589137899521`, and overflow fails generation. It also generates the readback/metric schema fields `measured_unclassifiable_filesystem_allocation_bytes` and independent `unclassified_allocation_bytes`, plus the whole-volume reconciliation rule that permits only proven NTFS/BitLocker metadata in the measured scalar and requires every ordinary file/stream or unexplained extent to make the anomaly nonzero. It never derives a bucket from directory size, `docs/config-reference.md`, existing generated output or environment. The production configuration-template projector consumes only this typed policy and emits `backup.spill_max_bytes=25769803776`, writer report-spool default/min/hard-max `268435456/67108864/2147483648`, each MCP owner `1073741824`, PostgreSQL text-log max `21474836480`, and the exact DATA_HDD relative selector/class IDs. The normal 256-MiB report-spool default does not shrink either signed 2-GiB class reservation. Config catalog IDs and production template values must resolve to one graph owner and exact-match the policy; a legacy 50-GiB spill, 20-GiB writer spool, sub-64-MiB writer spool, unknown root, ninth/tenth bucket drift, thirteenth/fifteenth class, filesystem-metadata catch-all class, duplicate selector, a class sum over its bucket, arithmetic overflow or a template/policy digest mismatch fails generation. No extra runtime key may loosen these constants.

The normal `SLOT_RETAINED_LIVE_WAL` row additionally exact-projects the profile §9.1 `PG16_NORMAL_WAL_OBJECT_SET`, not just retained-history segments: current/retained, bounded healthy archive-pending and preallocated/recycled-for-future segments; recognized root history/backup/partial, archive-status `.ready/.done`, `xlogtemp.<positive-decimal-pid>`, `RECOVERYXLOG`, `RECOVERYHISTORY` and Windows-only `H24.deleted` lifecycle objects. The profile's uppercase-hex name grammar, cluster/timeline/frontier/allocation proof and bounded archive/backup/process/recovery/rename-unlink lifecycle proof are mandatory parts of the same selector meaning and map digest. All support allocations remain in the existing normal 350-GiB class; only recognized, proven failure-only segments outside the normal set enter the disjoint archiver-failure class. G0's planned positive goldens include healthy recycled/future WAL, `.ready/.done` status (including proved cleanup after local segment removal), history/backup/partial and every temporary name including Windows deletion; negative goldens include `archive_status/unknown.ready`, unknown suffix/subdirectory, a future name without allocation proof, a temporary name without lifetime proof and duplicate normal/failure ownership. G1 and G6 import these exact cases/readback rules. Projection, collector, cause-ledger and golden code/execution remain `NOT_IMPLEMENTED`; these are planned acceptance requirements, not delivered test results. The sets stay exactly nine buckets, fourteen classes and fourteen selectors, with every existing byte limit unchanged.
...
fn topology_declaration_wire_media_owner_and_actual_readback_are_exact() {
    ...
    assert_eq!(declaration.hardware_profile_id.as_str(),
        "THINKSTATION_P340_I5_10500_32GB_256GB_SSD_2TB_CMR_HDD_MIN");
