...
- The server baseline is ThinkStation P340 Tower, i5-10500, 32GB RAM, 256GB runtime SSD and about 20 active users. Its currently installed 1 TB HDD is development/validation-only (`production_eligible=false`); a production candidate requires a recertified CMR authority HDD with raw capacity at least `2,000,000,000,000` bytes and an NTFS authority volume at least `1,589,137,899,520` bytes.
...
- `SINGLE_DISK_DEGRADED_PRODUCTION` describes only the same P340 after the qualifying data-disk expansion and complete recertification; the current 1 TB profile has no production terminal. UPS, off-server append-only backup, offline encrypted rotation and a clean-server restore drill remain go-live gates.
...
Expected: PASS only with Task 7 continuous UPS safe shutdown and exact `P340_SINGLE_HDD_V1` HardwareEvidenceV1 for the expanded production candidate (ThinkStation P340 Tower, Intel Core i5-10500, 32GB, one 256GB OS SSD and exactly one CMR authority HDD whose raw device is `>=2000000000000` bytes and whose NTFS authority volume is `>=1589137899520` bytes). The currently installed 1 TB HDD is a separate development-only profile and cannot produce PASS. The data-volume arithmetic is exact: three independent `350 GiB` buckets for archive staging, slot-retained live WAL and persistent-archiver-failure extra live WAL, plus the other fixed components, total `1380 GiB`; a non-borrowable `100 GiB` makes the volume floor `1480 GiB`. Total live `pg_wal` is hard-capped at `700 GiB`; fail-closed hold begins at archive-failure growth `300 GiB` or total live WAL `650 GiB`, hard shutdown at `350/700 GiB`, sampling is `<=30 s`, and measured peak WAL rate times `(sample + shutdown)` remains `<50 GiB`. Secure Boot/TPM/BitLocker/UEFI/AMT/removable-media gates, Windows Server drivers, SMART/temperature/bad-sector checks, flush/power-loss test and 72-hour stability also pass. The barrier load is exactly 15 Workbench + 3 customer portal + 2 supplier portal plus one separately reserved Control Center, with the simultaneous 11 read + 5 write + 2 high-risk + 2 attachment action mix, bursts, one due automation chain, incremental backup, audit checkpoint, health collection and one heavy report. Also require a signed malware definition age `<=72h` under trusted W32Time, emergency reserve, zero customer-content SSD writes, pinned PostgreSQL 16/base-WAL-PITR evidence and all three recovery roles. The signed manifest/capacity certificate records CPU/RAM/system identity, NTFS/filesystem, volume GUID/serial, physical disk/controller/firmware, logical/physical sector, cluster and write-cache/flush properties plus data size, HDD fill and 30/90-day growth. Any hardware change invalidates it and invokes the Task 7 new-profile/recertification protocol. RAID1 later requires two member disks, each independently satisfying the same `2,000,000,000,000`-byte raw-device floor; the old 1 TB disk may not be reused as a member or backup target. A signed offline definition update is tested；stale/revoked/wrong-engine media remains quarantined.
