...
- The currently installed 1 TB HDD is development/validation-only and has `production_eligible=false`. `SINGLE_DISK_DEGRADED_PRODUCTION` is available only after fitting and recertifying one CMR data HDD whose raw device is at least `2,000,000,000,000` bytes and whose NTFS authority volume satisfies checked `volume_total_bytes >= max(1,589,137,899,520,1,481,763,717,120+107,374,182,400+measured_unclassifiable_filesystem_allocation_bytes)`; certification never describes that one qualifying disk as redundant, hot-swappable, or ransomware-safe by itself.
...
The lifecycle policy remains the sole plain JCS member `docs/generated/f57/client-platform-lifecycle-policy.v1.json` of the existing `client-conformance-manifest.v1.json` family, under unchanged purpose/media. Its four UTF-8 platform rows retain exactly four DER roots and 16 role-bound fixed native-format parser fixtures; graph and compiled vectors remain byte-exact and `project_all` reads no ambient source. The source verifier fixed-loads the 20 reviewed bytes and corpus manifest, rejects path/reparse/digest/metadata/role/outcome drift, and verifies synthetic signature structure in a memory-only parser context. These reserved `ep.f57.fixture.*` IDs, roots and `0.9.0` baseline never authorize OS installation or real lifecycle PASS. It does not install fixture roots, including on test devices. The legacy four fixture roles/outcomes and revoked-fixture projection remain solely parser/policy test inputs.

`docs/evidence/f57-client-platform-evidence.v1.schema.json` is the canonical owner for both signed platform/lifecycle envelopes, the four-row policy wire, all eight strict plain readback families, the embedded `ClientNativeLifecycle*V1`, `ClientNativePackageIdentityV1`, native channel/certificate pin types, `ClientNativeUpdateFailureReadbackV1`/kind, `ClientNativeUpdateOperationResultV1` and native certificate-status proof types and Windows install-context types, and exactly 32 descriptor parsers in canonical `(platform UTF-8,evidence-kind ordinal)` order: the eight kinds `PACKAGE_SIGNATURE,INSTALL_START,UPGRADE,REVOCATION,CAPABILITY,RESOURCE,DLP,ACCESSIBILITY` for `android`, then the same eight for `ios`, `macos`, and `windows`. Each descriptor selects one exact purpose/media/`$defs` parser from outer platform plus tagged evidence kind; caller context and generic JSON/octet-stream never select a parser. The aggregate embeds the eight independently signed lifecycle envelopes directly, not refs to guessed lifecycle result filenames. `docs/evidence/f57-client-stack-decision-archive.v1.schema.json` is the sole combined owner for trust-closure, archive-manifest, entry and archive-proof wires; the former split trust-closure/archive-manifest schema paths are forbidden aliases. Foundation is sole owner of `UuidV1`, digests, repository paths, artifact/test/run IDs, `ClientPlatformV1`, `ClientStackKindV1`, `ClientPackageIdV1`, fixture-role/outcome enums, principals, `RunnerIdV1`, `CandidateRunIdentityV1`, `CandidateIdentityV1` and the signed-envelope field set. Every signed client root imports foundation directly and composes that envelope once; client-common owns only client-local package/artifact wrappers, and conformance imports foundation plus requirement-evidence binding, never client-common.

`ClientInstallStartReadbackV1` is one before/after collector and contains the non-optional fields `pre_install_observed_at_unix_ms`, `pre_install_registered_package_count`, `pre_install_matching_application_count`, `pre_install_matching_process_count`, `pre_install_business_cache_entry_count`, and `pre_install_authoritative_business_database_count`. The pre-install time is after durable STARTED and strictly before the common observed time, every pre-install count is exactly zero, and the post-start evidence proves exactly one first launch, authenticated authority readiness and zero local authoritative business databases. `ClientAccessibilityReadbackV1` has its single-value `ClientAccessibilityReadbackPurposeV1`, exact standard wire `WCAG_2_2_AA_PLUS_NATIVE`, the exact eight policy cases, per-case outcomes, and explicit `screen_reader_passed`, `keyboard_or_switch_navigation_passed`, `text_scaling_passed`, `contrast_passed`, and `failed_case_count`; PASS requires all four booleans true, the exact case set and zero failures. Every one-field mutation of the six pre-install fields, accessibility purpose/standard/cases/booleans/count, fixture source, or any of the exact `4/16/32` cardinalities has a failing golden.

Actual lifecycle qualification uses master §3's strict `native_inputs` embedded in `ClientPackageSignatureReadbackV1`, repeated JCS-byte-exact in `ClientUpgradeReadbackV1` and `ClientRevocationReadbackV1`; no fourth readback kind or independently stored input JSON is added. The trusted carrier collects the same current target plus exactly four distinct same-application sources in role order `UPGRADE_BASELINE,REVOKED_PACKAGE,DOWNGRADE_PACKAGE,FAILED_UPDATE_SOURCE`. Real archived customer releases or controlled first-release baselines use the approved native channel; controlled negative sources are validly native-signed and never become customer target artifacts. All five IDs match the actual current ID and reject the corpus namespace. Native version order, Android certificate/rotation proof, Apple application identity/profile/store/Team-ID/requirement constraints and Windows MSI major-upgrade constraints are exactly master's closed per-platform rules, never SemVer-only or SPKI-only approximations. Missing same-identity signed sources, native trust/channel inputs or receipt bytes yields failure without corpus fallback. Baselines may be prepared before first release; the already frozen current package cannot be rebuilt, repacked or re-signed.

The exact native package/DER/lineage/profile/store-receipt/compiled-requirement/failed-transport bytes are create-new/fsynced before freeze, use the existing binary media, and are reached only through master's field-position-selected bounded native byte parsers. All new JSON types are embedded `$defs` under the sole platform schema; no new raw JSON/octet-stream object, schema root, signed root, signer row or 33rd descriptor is invented. `PACKAGE_SIGNATURE` preflight freezes the complete input value and trusted time, verifies native channel pins and every source's authenticity/continuity, then stores/fsyncs/verifies its existing signed lifecycle envelope before native side effects. The exact same input value is required by upgrade/revocation; readbacks have exact field counts `PACKAGE_SIGNATURE=16`, `UPGRADE=13`, `REVOCATION=13`. Native input/pin/nested-field counts follow master definitions and are frozen in schema/byte goldens. The wrapper's product CMS identity and the native channel's package identity are separately verified and never equated. `validate-selected`, both architecture-decision carriers, selected G5 conformance and G6 release reruns share this input preparation/order without adding caller-controlled input flags.

Upgrade readback binds the real baseline and current target, with native version/signing continuity; legacy `failed_update_fixture_ref` binds only the deterministic final-byte-truncated copy of the valid higher-version `FAILED_UPDATE_SOURCE`. It proves the normal updater's integrity rejection leaves the current target launchable, not a simulated OS crash rollback. Revocation binds the real higher-version source, verifies it would otherwise qualify, then separately exercises its exact singleton digest and signer denial in the embedded nonnil isolation ID's synthetic-data policy. Its revoked-set digest hashes that embedded singleton vector, never graph corpus descriptors. A shared current/source signer must deny target new sessions too while the isolated signer rule is active. Downgrade exercises the real lower-version same-app input; all negatives leave installed current bytes unchanged, mutate no authority facts and dispose/reset isolated policy after observation.

`INSTALL_START`, upgrade, revocation and downgrade use independently reset native instances with the same frozen bytes/attempt; installing the baseline never downgrades the install-start instance's target. Upgrade also requires the exact nine-field `native_failed_update` embedded readback, as defined by master: intact higher-version source, native request started after signed preflight, controlled disposable staging-space exhaustion, closed platform-tagged native API result proving that specific precommit failure, unchanged target digest and subsequent successful native launch. The corrupt transport check alone is insufficient. The platform operation result is embedded under the existing schema, never an invented raw binary receipt. Android session/status, Apple command or held process identity/error domain/code, and Windows held MSI identity/return/system code are captured by the trusted native collector and checked against its compiled, qualified OS/channel mapping plus actual low-space measurements. Source/device/request/time/cause/strict-tag checks are required goldens; unqualified OS mappings and generic errors fail closed. If the OS/channel cannot furnish the actual started/failure proof, the platform is blocked rather than reported PASS.

Native trust pin authoring belongs to the existing `xtask/src/f57/client_gate.rs` carrier and evidence goldens. Master fixes its twelve-field native input, six-field channel pin and four-field certificate-pin rows, all exact enums and URI-independent compiled content pins. Customer-approved channel/application/certificate metadata is reviewed and compiled into independently product-signed runner resources before graph/source/candidate identity freeze; no package or bundled JSON self-authorizes its native signer. Runtime materializes only the context-derived refs and exact-matches the rendered compiled row. Native renewal/rotation or store-signing change requires a newer reviewed pin, source/graph amendment, new signed carrier and new attempt. Archived decision replay needs its original authenticated pin/table version and fails on an unsupported historical version. The eight product CMS wrapper roles retain their existing registry DNs and sign exact final native-channel bytes; the existing signed `PACKAGE_SIGNATURE` leaf binds that wrapper ref and independent native proof, including Apple store re-signing, without requiring the store's private key or forming a circular ref.

Native status is independently closed by master's exact three-field `ClientNativeCertificateStatusProofV1` vector and `X509_CRL|OCSP_RESPONSE` tags, reaching original signed CRL/OCSP DER under existing native binary media. Every required native certificate/status mapping, issuer/serial/signature/GOOD result and trusted preflight `thisUpdate <= frozen_at < nextUpdate` window must verify; missing/unknown/soft-failed status blocks the platform. Expiry minima include required native proofs/profiles. The sole explicit exception is a directly approved self-signed Android APK signer without a CA status service, never a general pin-is-unrevoked assumption. Current runs need fresh proofs; permanent archives replay the authenticated original window as history and do not recast `DecisionSigner|Tsa` as native trust roles. Golden closure follows every native DER status ref and rejects missing/extra/cross-certificate/cross-role proof, wrong current/history time and arbitrary binary/JSON substitutes.

Task 1's schema-DAG, object-placement/occurrence, offline-closure and generator goldens freeze all 32 existing `(artifact kind,platform/kind,media,owned-$defs)` metadata tuples plus the newly reachable native field occurrences. Tests include missing sources/proofs, corpus-as-real-input, caller/ambient pin injection, valid signature with wrong app/channel/prefix, invalid Android certificate rotation, native version boundary/ordering error, unsupported native formats, mismatch between package/extracted proof bytes, revoked input already invalid before policy, shared-signer session bypass, wrong corrupt transport, absent/generic native failed-update proof, reused case instance, post-action freeze, cross-leaf input drift, omitted archive object, unexpected JSON root/media/descriptor and wrapper/native signer substitution. Tests independently mutate every strict field, ref and nullable/platform discriminator; none treats a generic rejection or fixed parser corpus outcome as lifecycle PASS. Four roots, 16 corpus entries, eight lifecycle kinds, 32 descriptor parsers, 89 signer rows, 30 projection families and all 185 Requirements stay unchanged.
...
fn lifecycle_policy_is_pure_and_closes_four_roots_sixteen_fixtures_and_thirty_two_readbacks() {
    ...
    assert_eq!(client_readback_field_count("PACKAGE_SIGNATURE"), 16);
    assert_eq!(client_readback_field_count("UPGRADE"), 13);
    assert_eq!(client_readback_field_count("REVOCATION"), 13);
    assert_native_inputs_are_owned_embedded_defs_with_exact_platform_roles_and_native_ref_closure();
    assert_corpus_cannot_authorize_native_install_update_or_revocation_pass();
    assert_package_signature_preflight_is_durable_before_native_side_effects();
    assert_wrapper_and_native_channel_identity_pins_are_independent();
...
**Interfaces:**
- Consumes exactly the physical P340/i5-10500/32GB/256GB runtime SSD with one recertified CMR authority HDD whose raw capacity is at least `2,000,000,000,000` bytes and whose NTFS authority volume satisfies checked `volume_total_bytes >= max(1,589,137,899,520,1,481,763,717,120+107,374,182,400+measured_unclassifiable_filesystem_allocation_bytes)`, plus the actual UPS, the G0-owned strict plain `RuntimeTopologyDeclarationV1` only through G1's private verified wrapper/ref, the verified signed authority-storage manifest and the non-expiring P340 policy definition. The currently installed 1 TB HDD is an explicitly ineligible development profile and cannot enter this production recipe. Candidate-bound capacity/certification and post-infrastructure `RuntimeTopologyCertificationV1` are later release outputs, never pre-qualification inputs. No provider/tenant/vTPM/cache/snapshot/provider-power input is legal in this graph version.
...
The same policy requires each capacity-relevant table to have at least `10000` rows, at least `10000` attachments totaling `53687091200` bytes with all three buckets nonzero, index/WAL bytes at least `5368709120/2147483648`, initial HDD used ratio `3500..=7000` basis points, SSD life `>=20%`, one complete zero-error memory pass over all `34359738368` bytes, clean SSD reinstall plus authority restore `<=28800 s`, data HDD exactly CMR with rating `>=55 TB/year` and warranty through evidence expiry, scanner age `<=259200 s`, absolute W32Time offset `<=1000 ms`, sync age `<=900 s`, CPU/SSD/HDD temperatures `<=85000/70000/55000` millicelsius, SSD raw-device capacity at least `240000000000` bytes, DATA_HDD raw-device capacity at least `2000000000000` bytes, and DATA_HDD NTFS volume capacity satisfying `volume_total_bytes >= max(1589137899520,1481763717120+107374182400+measured_unclassifiable_filesystem_allocation_bytes)` under checked arithmetic. The DATA_HDD policy freezes the nine-bucket exact-set, in enum order, to `161061273600,64424509440,42949672960,375809638400,375809638400,375809638400,25769803776,8589934592,51539607552` bytes. Its checked sum is exactly `1481763717120` bytes (`1380 GiB`); the separate `107374182400`-byte (`100 GiB`) free-space floor is non-borrowable, yielding the `1589137899520`-byte (`1480 GiB`) nominal floor. Fourteen capacity classes and fourteen canonical selectors form a total one-to-one accounting projection only over product-managed canonical `data_root` objects and registered DATA_HDD VSS extents. A fixed whole-volume collector puts only allocation proven to be NTFS/BitLocker volume metadata into `measured_unclassifiable_filesystem_allocation_bytes`; metadata receives no class ID and cannot consume a bucket or the free floor. Any ordinary outside-root file, inaccessible/unexplained extent or partition gap increments independent `unclassified_allocation_bytes` and fails closed. At zero anomaly, checked equality requires `used_bytes=sum(class_usage.allocated_bytes)+measured_unclassifiable_filesystem_allocation_bytes`. The `64424509440`-byte shared bucket is itself closed at maximum legal occupancy by PostgreSQL text log `21474836480`, archive/backup report spools `2147483648` each, core/worker MCP completion spools `1073741824` each, and shared history/audit/control residue `36507222016`; `backup.spill_max_bytes` is the entire backup-staging class and is exactly `25769803776`. The two writer spool runtime defaults remain `268435456`, their parser range is `67108864..=2147483648`, and the policy still reserves each signed hard maximum. `postgres/data` bytes outside the entire `pg_wal` subtree count only in the `42949672960`-byte database class; the normal `SLOT_RETAINED_LIVE_WAL` selector imports the profile §9.1 closed `PG16_NORMAL_WAL_OBJECT_SET` (current/retained, bounded healthy archive-pending and preallocated/recycled-for-future segments, exact PG16 history/backup/partial/archive_status and temporary/Windows-deleted objects with the same required name-and-lifecycle proof); all support allocations stay within its existing 350-GiB budget, normal recovery/slot causes win, and only proven archiver-failure-only segments outside that set enter the disjoint fault class; process/restore temp, approved dumps and VSS DATA_HDD diff extents count only in the `51539607552`-byte scratch class. Aggregate bucket admission precedes local limits. Unknown, duplicate, cross-bucket or unclassified allocation bytes, an inexact class/root map digest, class sums above a bucket, a metadata catch-all class, arithmetic overflow or any attempt by legal hold to borrow another bucket fails closed. Live `pg_wal` has a `700 GiB` (`751619276800`-byte) hard envelope; new ingest enters fail-closed hold at either `300 GiB` archive-failure growth or `650 GiB` total live WAL, and hard shutdown occurs at `350/700 GiB`. Sampling is at most `30 s`, and measured `peak_wal_rate_bytes_per_second * (sample_period_seconds + shutdown_seconds)` must remain strictly below the `50 GiB` reaction margin. The qualification workload's `minimum_wal_bytes=2147483648` is only a generation floor, never a substitute for any fault bucket.

Planned G6 capacity/readback goldens import `PG16_NORMAL_WAL_OBJECT_SET` without redefining it: healthy recycled/future segments, recognized archive-status `.ready/.done` (including proved cleanup after local segment removal), history/backup/partial and verified temporary/Windows `H24.deleted` lifecycle positives must each charge once within the normal class. Unknown names/suffixes/subdirectories, `.partial.deleted`, future filenames lacking allocation proof, temporary objects with unknown lifetime and duplicate normal/failure ownership must fail. Initial/every-minute/final readbacks exact-join cluster, timeline, segment size, retention/future allocation intervals, lifecycle evidence and the current mapping digest. Collector/cause-ledger code and golden execution remain `NOT_IMPLEMENTED`; this is acceptance design only and does not claim a live PostgreSQL, Windows or P340 pass.

The policy also enforces the exact watermark relation `minimum_free_bytes >= yellow_free_bytes > red_free_bytes >= emergency_reserve_bytes`, `maximum_used_bytes = total_bytes - minimum_free_bytes`, and `minimum_free_bytes >= emergency_reserve_bytes + 30*p95_daily_growth_bytes`. For each role, checked `raw_device_capacity_bytes>=volume_total_bytes>0`; both the DATA_HDD raw-device and NTFS-volume floors are mandatory and the filesystem geometry must exact-match the signed volume row. Initial HDD ratio never uses a rounded intermediate and operates on the DATA_HDD volume total: checked `u128` requires `hdd_total_bytes>0`, `hdd_used_bytes<=hdd_total_bytes`, `hdd_used_bytes*10000 >= 3500*hdd_total_bytes`, and `hdd_used_bytes*10000 <= 7000*hdd_total_bytes`. The watermark's five-percent term is exactly `ceil_div(checked_u128(total_bytes)*500,10000)` and `emergency_reserve_bytes=max(21474836480,five_percent_term)`; quotient/remainder, multiplication, `u64` downcast and all later additions/subtractions are checked. Goldens cover both initial-ratio inclusive boundaries and one byte below/above, raw-vs-volume inversion and both floors at equality/one byte below, simultaneous slot-stop plus persistent archiver failure, restart-held behavior until repair, both hold/hard boundaries, the reaction-margin inequality, exact-divisible and nonzero-remainder ceiling branches, the 20-GiB crossover, and overflow/downcast failure. All 25 metric variants have executable predicates; no required metric is presence-only or informational.
...
Every one of the 25 variants has one deterministic predicate. Principal, action and overlay variants exact-equal the frozen mixes. `ErrorRate` has exact fields `operation_count,error_count,error_basis_points,high_risk_completed_count,attachment_completed_count`; checked `u128` requires `operation_count=read_histogram.operation_count+write_histogram.operation_count+high_risk_completed_count+attachment_completed_count`, each completed count is at least its matching `ActionMix`, `operation_count>0`, `error_count<=operation_count`, and `error_basis_points=min(10000,ceil(error_count*10000/operation_count))`, followed by the zero-error release requirement. CPU P95 is the one-based nearest-rank sample at `ceil(95*N/100)` after numeric sorting. `PageFaults` starts at cumulative zero, never decreases and bounds every adjacent delta. HDD free/growth uses checked arithmetic to prove `free_bytes+used_bytes=input.hdd_total_bytes`, the basis-point floor, and `projected_daily_growth_bytes=max(input.p95_daily_growth_bytes_30d,input.p95_daily_growth_bytes_90d)`; every one of all `4321` samples also carries and exact-matches the policy capacity-map digest, complete nine-bucket/fourteen-class usage vectors, `measured_unclassifiable_filesystem_allocation_bytes`, and zero unknown/duplicate/cross-bucket plus `unclassified_allocation_bytes=0`, with each class and aggregate bucket within its hard limit. Each sample independently re-proves the dynamic volume predicate and `used_bytes=sum(class_usage.allocated_bytes)+measured_unclassifiable_filesystem_allocation_bytes`; the initial capacity input carries the same exact fields. The watermark recomputes the same growth, minimum free and maximum used. PostgreSQL requires `active<=maximum` in addition to both absolute caps; cumulative WAL is exactly zero in frame 1, never decreases, and is positive in frame 4321. Automation, SSD and page-fault counters begin at zero and never decrease. Heavy report requires `accepted=true`, the exact decision bound and concurrency one. Generated goldens cover equality and one-beyond failure for every scalar and relational predicate, including metadata zero/one-byte required-volume results, checked overflow, wrong map digest, missing/extra class or bucket, a forbidden metadata catch-all class, one anomaly and one allocation byte above each hard limit; no metric is informational or presence-only.
...
- Move: `apps/core-server/build.rs` to `crates/platform/authority-kernel/build.rs`
- Move: `apps/core-server/src/business/ctc01.rs` to `crates/platform/authority-kernel/src/application/business/ctc01.rs`
- Move: `apps/core-server/src/business/mod.rs` to `crates/platform/authority-kernel/src/application/business/mod.rs`
- Move: `apps/core-server/src/config.rs` to `crates/platform/authority-kernel/src/application/config.rs`
- Move: `apps/core-server/src/platform/client_sessions.rs` to `crates/platform/authority-kernel/src/application/platform/client_sessions.rs`
- Move: `apps/core-server/src/platform/control_center.rs` to `crates/platform/authority-kernel/src/application/platform/control_center.rs`
- Move: `apps/core-server/src/platform/employee_api.rs` to `crates/platform/authority-kernel/src/application/platform/employee_api.rs`
- Move: `apps/core-server/src/platform/events.rs` to `crates/platform/authority-kernel/src/application/platform/events.rs`
- Move: `apps/core-server/src/platform/file_routes.rs` to `crates/platform/authority-kernel/src/application/platform/file_routes.rs`
- Move: `apps/core-server/src/platform/identity.rs` to `crates/platform/authority-kernel/src/application/platform/identity.rs`
- Move: `apps/core-server/src/platform/identity_admin.rs` to `crates/platform/authority-kernel/src/application/platform/identity_admin.rs`
- Move: `apps/core-server/src/platform/key_domain.rs` to `crates/platform/authority-kernel/src/application/platform/key_domain.rs`
- Move: `apps/core-server/src/platform/middleware.rs` to `crates/platform/authority-kernel/src/application/platform/middleware.rs`
- Move: `apps/core-server/src/platform/migration.rs` to `crates/platform/authority-kernel/src/application/platform/migration.rs`
- Move: `apps/core-server/src/platform/remote_support.rs` to `crates/platform/authority-kernel/src/application/platform/remote_support.rs`
- Move: `apps/core-server/src/platform/sensitive.rs` to `crates/platform/authority-kernel/src/application/platform/sensitive.rs`
- Move: `apps/core-server/src/platform/static_assets.rs` to `crates/platform/authority-kernel/src/application/platform/static_assets.rs`
- Move: `apps/core-server/src/platform/windows.rs` to `crates/platform/authority-kernel/src/application/platform/windows.rs`
- Move: `apps/core-server/src/probe.rs` to `crates/platform/authority-kernel/src/application/probe.rs`
- Move: `apps/core-server/src/wiring/authn.rs` to `crates/platform/authority-kernel/src/application/wiring/authn.rs`
- Move: `apps/core-server/src/wiring/authority_fence.rs` to `crates/platform/authority-kernel/src/application/wiring/authority_fence.rs`
- Move: `apps/core-server/src/wiring/authz.rs` to `crates/platform/authority-kernel/src/application/wiring/authz.rs`
- Move: `apps/core-server/src/wiring/command.rs` to `crates/platform/authority-kernel/src/application/wiring/command.rs`
- Move: `apps/core-server/src/wiring/context.rs` to `crates/platform/authority-kernel/src/application/wiring/context.rs`
- Move: `apps/core-server/src/wiring/control_center.rs` to `crates/platform/authority-kernel/src/application/wiring/control_center.rs`
- Move: `apps/core-server/src/wiring/db.rs` to `crates/platform/authority-kernel/src/application/wiring/db.rs`
- Move: `apps/core-server/src/wiring/employee.rs` to `crates/platform/authority-kernel/src/application/wiring/employee.rs`
- Move: `apps/core-server/src/wiring/features.rs` to `crates/platform/authority-kernel/src/application/wiring/features.rs`
- Move: `apps/core-server/src/wiring/generation.rs` to `crates/platform/authority-kernel/src/application/wiring/generation.rs`
- Move: `apps/core-server/src/wiring/identity.rs` to `crates/platform/authority-kernel/src/application/wiring/identity.rs`
- Move: `apps/core-server/src/wiring/ipc.rs` to `crates/platform/authority-kernel/src/application/wiring/ipc.rs`
- Move: `apps/core-server/src/wiring/kms.rs` to `crates/platform/authority-kernel/src/application/wiring/kms.rs`
- Move: `apps/core-server/src/wiring/meta.rs` to `crates/platform/authority-kernel/src/application/wiring/meta.rs`
- Move: `apps/core-server/src/wiring/metrics.rs` to `crates/platform/authority-kernel/src/application/wiring/metrics.rs`
- Move: `apps/core-server/src/wiring/mod.rs` to `crates/platform/authority-kernel/src/application/wiring/mod.rs`
- Move: `apps/core-server/src/wiring/office_identity.rs` to `crates/platform/authority-kernel/src/application/wiring/office_identity.rs`
- Move: `apps/core-server/src/wiring/package.rs` to `crates/platform/authority-kernel/src/application/wiring/package.rs`
- Move: `apps/core-server/src/wiring/probes.rs` to `crates/platform/authority-kernel/src/application/wiring/probes.rs`
- Move: `apps/core-server/src/wiring/provider.rs` to `crates/platform/authority-kernel/src/application/wiring/provider.rs`
- Move: `apps/core-server/src/wiring/release.rs` to `crates/platform/authority-kernel/src/application/wiring/release.rs`
- Move: `apps/core-server/src/wiring/storage.rs` to `crates/platform/authority-kernel/src/application/wiring/storage.rs`
- Move: `apps/core-server/src/wiring/support.rs` to `crates/platform/authority-kernel/src/application/wiring/support.rs`
- Move: `apps/core-server/src/wiring/sync.rs` to `crates/platform/authority-kernel/src/application/wiring/sync.rs`
- Move: `apps/core-server/tests/authority_storage_bootstrap_archive_composition.rs` to `crates/platform/authority-kernel/tests/authority_storage_bootstrap_archive_composition.rs`
- Move: `apps/core-server/tests/evidence_input_store_composition.rs` to `crates/platform/authority-kernel/tests/evidence_input_store_composition.rs`
- Move: `apps/core-server/tests/evidence_object_store_composition.rs` to `crates/platform/authority-kernel/tests/evidence_object_store_composition.rs`
- Move: `apps/core-server/tests/gate_run_journal_store_composition.rs` to `crates/platform/authority-kernel/tests/gate_run_journal_store_composition.rs`
- Create: `crates/platform/authority-kernel/src/application/bootstrap.rs`
- Create: `crates/platform/authority-kernel/src/application/platform/mod.rs`
- Modify: `apps/core-server/tests/default_kms_fail_closed.rs`
...
The 47 explicit leaf-file `Move` rows in Files are the complete Authority application relocation set: they include the checked-in application leaves and every additional leaf produced by G1–G5 or Task 10 before this task. G0-01 expands both endpoints under the existing staging grammar; this task adds no directory/glob staging rule and does not amend the frozen registry. All 47 sources must exist at task-begin, all destinations must be absent, and the post-move source paths must be absent. `apps/core-server/src/main.rs`, `apps/core-server/src/platform/mod.rs`, the launcher Cargo manifest and its process-level `default_kms_fail_closed.rs` remain explicit modifications, not move sources. The old main function’s application bootstrap/serve logic is transferred into the new kernel `application/bootstrap.rs`; the new kernel `application/platform/mod.rs` carries the moved application route registrations, while the retained launcher platform module registers only `windows_service`. The moved `application/wiring/mod.rs` is updated in the same move to register both its preserved adapters and the new Task-14 wiring leaves. Rewrite every moved `crate::config|platform|wiring|business|probe` import to its kernel application owner, move the Control Center asset build script and its build dependencies to the kernel, and let the launcher call application behavior only through the kernel’s versioned ABI. The kernel manifest supports its existing component tests through an `rlib` alongside the release `cdylib`; all former application dependencies/features/build settings move there, and the launcher loses those direct dependencies. The retained subprocess KMS test exercises a verified engineering kernel through the launcher and continues to assert fail-closed pre-DB startup, without importing application adapters into the launcher. This relocation preserves already-delivered route/discriminator sets, error semantics, storage-before-DB ordering and the single command gateway; it does not revive any historical disabled endpoint.

The Task-14 locked-metadata/import and composition goldens exact-check these 47 source/destination pairs, four preserved file owners, kernel module/build-script reachability and absence of application paths/imports in the launcher. They rerun the moved four store-composition tests plus the retained KMS subprocess test and rehearse the same G3/G4/G5 public-HTTPS conformance after relocation; these engineering results are not candidate evidence. Task 15 still performs final L2 against its frozen candidate. A missing route, unlinked moved module, lost asset, copied second handler, source left behind or unregistered destination fails before candidate freeze. A later application leaf requires a coordinated pre-G0 plan amendment that adds its exact move, never runtime source discovery or a Task-14 staging exception.
...
No IaaS alternative or provider-power staging set exists in this graph version. `IAAS_WINDOWS_SERVER_HDD_STRICT`, `IAAS_PROVIDER_POWER_SHUTDOWN_EQUIVALENCE`, every `IAAS_*` input discriminator and every provider/VM/vTPM/cache/snapshot ref is rejected before STARTED and cannot appear in a staging plan, carrier result, offline-schema manifest or certificate. **F-63: for `IAAS_WINDOWS_SERVER_HDD_STRICT` specifically, the profile selector's rejection must return the typed pair `PROFILE_NOT_IMPLEMENTED` / `STORAGE_MEDIA_UNVERIFIED` (the named values demanded verbatim by the P340 production-gate checklist item and threat-model §15), and the existing rejection tests must assert these two exact codes for that discriminator; other `IAAS_*` inputs keep the generic rejection.** The future seam must introduce its complete input sets and new graph version together; it may not borrow the current P340/UPS slots.
...
Expected: certificate verification completes first and is still not activation. `EPAuthorityServer` resolves two-person acceptance and exact five-risk/20-user/safeguard closure for the recertified P340 production profile, creates or adopts one activation attempt, captures all four fresh sealed readbacks and persists only `LIVE_READBACK_BOUND` after graph/active/deferred/consumer/item/ACK equality. It additionally requires fresh `HEALTHY` backup state with exact current-root binding; all 30 backup-domain inequalities; exact PostgreSQL-log policy (`max_age=30d`, `max_total=20GiB`, current log undeletable, minimum seven days, legal-hold preservation, typed `EPAuthorityControl` cleanup only, PostgreSQL identity unable to delete history); and no capacity/resilience hold. The current 1 TB development disk can never satisfy this check. The installed production DATA_HDD must re-read at least `2000000000000` raw bytes and satisfy `volume_total_bytes >= max(1589137899520,1481763717120+107374182400+measured_unclassifiable_filesystem_allocation_bytes)`, with the signed capacity-map digest, exact nine-bucket/fourteen-class/fourteen-selector vectors and three-bucket WAL envelope unchanged. The fresh capacity readback must import the same profile §9.1 `PG16_NORMAL_WAL_OBJECT_SET` and cause-ledger partition, prove healthy recycled/future and recognized archive-status/history/backup/partial/temp/Windows-deleted lifecycle objects are charged once, reject unknown names/lifetimes and duplicate ownership, uniquely classify every product-managed canonical `data_root` object and registered VSS extent, carry the independently measured NTFS/BitLocker metadata scalar, prove the exact used-byte decomposition and dynamic volume predicate, keep each local and aggregate usage within its limit, and report zero unknown, duplicate, cross-bucket or `unclassified_allocation_bytes`; a metadata catch-all class, unexplained ordinary volume allocation or legal hold never makes a capacity overrun acceptable. DATA_HDD free space below the stricter of the existing configured yellow limit and 100 GiB pauses batch work; below the stricter of the existing red limit and 100 GiB creates the global hold, while the independent archive-failure/total-live-WAL holds remain `300/650 GiB` and their hard limits `350/700 GiB`. It passes private `VerifiedProductionActivationReadyV1` to the upper admission authority, whose sole joint transaction appends activation `ACTIVATED`, creates the genesis `ProductionGenerationAdmissionV1`, advances its head and `business_api_generation`, then commits once. The gate exact-loads the same signed-set/install-evidence ten-row bypass registry and immediately rechecks current OBSERVED equals admitted and no hold exists. Every subsequent business request is accepted only by same-lock creation/adoption of one `ProductionAdmissionExecutionLeaseV1`; command/query completion or exact-once handoff terminalizes it, and a hold cannot drain/barrier while an intersecting ACCEPTED lease remains except for the exact disaster fence. Response-loss adopts the one joint result or one request lease; every statement/commit/race crash exposes neither side or both and no write after a committed barrier. Stale certificate, expired/one-person acceptance, missing safeguard, shared backup domain, log-retention drift, co-located target, one offline disk, desired-only/mixed readback, admission/lease conflict, ambient/changed bypass registry, any IaaS/provider-power input or graph/resource drift fails closed; repairable live failure remains `FAILED_HELD` and retries only the same activation ID with four new readbacks. A normal current-root rotation must already have created its tagged global hold before the config CAS and must remain held throughout `TRANSITIONING|BOOTSTRAPPING`; only fresh `HEALTHY`, exact cause tuple/new-root binding and the cause-bound final admission CAS append `REOPENED`. A disaster replacement is never smuggled into activation: the dead old disk needs no fresh `HEALTHY`, but release stays held until the sole two-person/off-host-trust/checkpoint/fence/higher-epoch-and-storage-generation/new-BitLocker-volume-and-manifest/clean-restore/PITR-full-reconciliation/fresh-A-B-bootstrap/P340-recertification chain completes. Terminal status is exactly `PRODUCTION_ACTIVATED_SINGLE_DISK_DEGRADED`; it never claims RAID, HA or an IaaS terminal.
