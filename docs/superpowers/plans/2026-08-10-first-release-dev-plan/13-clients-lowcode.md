...
本阶段按贯通线 T0 拆成先后两批，阶段范围归属不变，只改本阶段内部的工作次序。第一批是桌面壳最小切片，与 T0 同批交付，排在阶段 4 与阶段 3b-1 之后、阶段 5 全量开工之前，内容与判据见第 1.5 节。第二批是本阶段其余全部交付物，一律在 T0 已贯通的骨架上加厚，不再承担任何首次贯通性质的判据。阶段顺序为 1 → 2 → 3a → 4 → 3b-1 → T0 → 5 → 9a → 8 → 6 → 7 → 10 → 11 → 9b → 14，阶段 3b-2 不在这条链上，其各项按阶段 3 计划第 3.0 节判定四的下游拉动点排在 T0 之后、各自拉动阶段之前，阶段 12 在阶段 10 之后与阶段 11 并行，本阶段第二批在阶段 12 之后与阶段 9b 并行。本阶段第二批不在关键路径上：它不被任何后继阶段的功能依赖，只被阶段 14 的四端验收矩阵汇总。
...
T0 不要求本阶段的下列各项，它们一律排在阶段 11 之后：白标 `brand.toml` 驱动的四端制品与商店合规门禁、本地加密缓存与离线草稿与增量同步、桌面端两个原生插件、能力矩阵表与其冻结比对、引导下发台账、深色与高对比度两套主题、低代码建模与配置发布的界面。T0 不要求四端、不要求 scale 数据集、不要求分支覆盖。

移动端两端按己-3 的裁定从上面这份清单中移出，拆成两段。`/clients/mobile` 的 Tauri 2 移动壳本体与其 iOS、Android 生命周期与后台任务适配，连同一份可在真机或模拟器上安装运行的开发构建，其交付时点为不晚于阶段 5 退出条件中移动用例的判定；本裁定不要求它排在阶段 5 全量开工之前，也不把它移入 T0 之前的关键路径。其四端制品、白标驱动与商店合规门禁仍留在第二批，排在阶段 11 之后。前移的理由与 T0 判据无关：裁定 A-23 在阶段 5 至阶段 12 各写死一条退出条件，要求本模块移动界面通过 XCUITest 与 Espresso 用例，而这八个阶段在固定链上没有一个排在阶段 11 之后，移动壳不存在时这八条退出条件恒不可达，业务阶段也无处安放 `clients/mobile/src/modules/<module>/` 的代码。移动壳不属于第 1.5 节首段向 T0 贡献的那五项，也不进入 T0 判据，该段末句所定的那条唯一切片判据一字不改。
...
| 6 | V202704060925__platform_meta_alter_config_package.sql | 对阶段 3b 已建的 config_packages 与 config_package_items 做列扩展：追加自动测试相关列，按第 3.2.10 节的十一态放宽 `ck_config_packages_status`，不改写任何既有行 |
...
启动自检的追加项按裁定 C-25 改为命名项，基线第 7.3 节的十项固定项一并按注册名标识，本阶段不再以序号称呼，需回写基线。按裁定 C-25，自检项分 Blocking 与 Degrading 两级，本阶段追加的一项为 Degrading 级：它判读的是数据库里的业务行，而这台服务器没有备节点，用一次数据判读阻断八个进程的启动是把可用性押在一次判读上。
