...
`verify-pipeline-commands.sh` 不再按 xtask 首词元去重，而是带完整参数实跑每一行；负样例已证明同名
子命令的第二套参数不会再被静默跳过。`cargo xtask ci` 与备用 `run-pipeline.sh` 的聚合优先级
...
6. `--check-doc-type-codes` **当时从不在 CI 中执行**：`pipeline-stages.tsv` 只跑不带旗标的入口，
   而退出条件 23 点名的正是这个旗标入口。**该历史缺口已由 F-86 关闭**：独立登记行已进阶段 6，验证器按完整参数逐行实跑。
