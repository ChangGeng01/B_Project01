...
| 辛-24 | **部分承接**：次序一致性已由 `xtask sqlcheck` 的 SQL-031 静态判；活库那一半仍缺执行方（归辛-27） | F-22 立案，F-28 分档，SQL-031 落码 |
...
| 辛-41 | 成立（F-29 新立） | F-29 结论八 |
| 辛-42 | 成立（F-36 新立）：20 张表写进 DO 块 execute 字符串绕开 sqlcheck 语句级规则，判据被移交给零执行方的 db/checks/01 | F-36 结论二 |
