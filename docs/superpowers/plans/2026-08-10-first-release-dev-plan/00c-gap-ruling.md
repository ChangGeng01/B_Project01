...
| `errorcodes` | 1（13 处） | 1（13 处） | 12 条文档已登记代码未实现，1 条取值冲突见结论三 | ~~各码所属阶段实现时~~ **F-66 补记同病更正**：登记册「阶段」列指旧十四阶段（已被 F-57 取代），且登记表 `:99` 明写「G0 后机器真值是 generated error registry」——现行转绿路径为 **G0 生成式再基线＋F-57 各 G 批次实现** |
| `configdoc` | 1（312 处） | 1 | 配置键、指标**与单据类型码**三段未对齐（F-62 补：五个失败用例中两个属单据类型码段判据，不属前两段也不属 the_repository 一族） | ~~首批实施按两份 markdown 对齐~~ **F-67 补齐第四行**：register `:97`/`:101` 明写「G0 后机器真值是 generated config/metrics catalog」且两份 markdown「cannot gate」——现行路径同前三行：**G0 生成式再基线＋F-57 各 G 批次实现**（F-66 曾称「四条判据全部修正」，实落三条，此为漏改的第四条） |
| `eventcatalog` | 1（117 处） | 1 | 事件已登记、无产生点 | ~~各事件所属阶段实现时~~ **同上**：登记表 `:100` 明写「G0 后机器真值是 generated event catalog」——现行路径为 **G0 生成式再基线＋F-57 各 G 批次实现** |
