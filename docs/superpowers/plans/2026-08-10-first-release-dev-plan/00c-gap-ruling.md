...
- **在第 3 步的实测出结论之前，不得把任何配额取值写进新的承载文件**，也不得改写 `01:24`／`:457`／`:507` 的判据文字。
...
这四条不是技术取舍，本方给不出依据。已于本轮明确挂起，**开工后再议**。
（本节原记「3 条」是旧值：己-1 第七节与己-5 两行已分别于其后两轮撤销，计数未随改，本轮一并修正。）
...
| F-08-1 | 裁定 F-08 第十一节第 1 条 | **国产化替代路径与等级保护三级对外表述**。服务端改 Windows Server 后，规格第 2.2、17.5 章登记的国产化认证矩阵在服务端一侧失去可达路径：国产 Linux 不再是「延期项」，而是与首版服务端平台互斥。规格 `1372`、`1534`、`1958` 与 PRD `4417` 四处的国产 Linux 表述**本轮原样挂起，不得顺手删除**——删掉等于悄悄取消一项对客户的能力承诺。三选一：改写为互斥、保留为长期项、另立国产化服务端分支。**本表优先级最高的一条**，它影响对外承诺，其余三条只影响内部形态 | 产品负责人 |
...
### 庚五　等目标平台上的一次实测（14 项，见裁定 F-08 第十二节）

本节不复述条目，只给索引与共同的等待条件。十四项逐条列在裁定 F-08 第十二节，
...
其余十一项不阻断规格修订，但**全部阻断落码**：第 4 项决定 E2E-05 的替身能否保住，
