...
**处置**：规格侧两处须同批改，**但改规格须使用方逐轮授权**（F-18 结论八、F-08 先例）。
决定四那次授权的标的是备份代数，**不自动延伸到本处**。故本卷**不代改**，
登记为待授权项：若授权，`规格:297` 的「覆盖销项发票与进项发票」与 `规格:311` 的
「同时覆盖销项与进项两个方向」须各加方向限定，明写作废只在销项方向成立。

**在授权到位之前，PRD 那八处与规格两处的矛盾是敞开的，本卷不宣称决定一已完整落地。**
