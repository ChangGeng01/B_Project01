...
真正会出事的是另一件：`EgressTarget::parse` 只校验形态、**不做规范化**（末行逐字 `Ok(EgressTarget(raw.to_string()))`），
于是**大写主机与显式 `:443` 都能通过校验并原样存下**；而匹配是精确相等，
运行期一旦产出规范化 URL，这类白名单项**永远匹配不上**——出网被拒，
而启动日志印的是「白名单 N 项，演练通过」。

已把恒真的自匹配换成**规范形判据**（拒大写主机、拒显式默认端口），
并补两条测试：一条钉住 `is_allowed` 的「无规范化精确相等」语义，
一条证明新判据**真的能拒**（四个近似写法一条都不认，规范形通过）。
...
1. 「保护永远够不着」**不成立**。按容量表实算：盘 930 GiB、稳态占用 370 GiB、阻断底线留 100 GiB；
   槽涨到旧上限时占用为 `370 − 40 + 350 = 680 GiB`，剩 250 GiB，**未触底线，槽会先失效，保护当时有效**。
2. 真正的问题是**容量表里没有任何一行给那 310 GiB 留位**，以及**随时间翻转**：
   按表自身记账口径 `470 − 40 + 350 = 780 GiB = 84%`，越过它自己的 80% 黄线；
   且稳态占用一旦超过 `930 − 100 − 350 = 480 GiB`，平台的空间底线就会**先于**数据库失效槽而阻断业务写入，
   故障模式从「归档链断、业务继续」翻转成「业务停写」——与规格逐字选的取舍相反。
   按附件 50 GB/年的增长，这大约发生在第 5 年。~~改成 40 GiB 后该翻转点被推到稳态 790 GiB~~（F-73：该句随 F-72 撤回而失效，值仍为 350GB）。
...
`db/bootstrap/02_cluster_params.sql` 落 `max_slot_wal_keep_size = '350GB'`（＝325.96 GiB，占 931 GiB 盘的 35%）；
容量表 `④a 连续归档本机保留` 记 **40 GiB**（按 R_wal 约 4 GiB/日 × 7 日 × 1.5 余量，占 4.3%）。

该参数存在的全部意义是「归档落点长时间不可写时兜住 `pg_wal`」。按 350GB 实现，
这条保护要成立就得盘上恒有 326 GiB 空闲；容量模型却只按 40 GiB 记账。
...
6. `--check-doc-type-codes` **从不在 CI 中执行**：`pipeline-stages.tsv` 只跑不带旗标的入口，
   而退出条件 23 点名的正是这个旗标入口。本卷**不擅自增流水线行**（那是改门禁面），登记待 G0。
7. `xtask ci` 与 `run-pipeline.sh` **两处**逐字自称「行为逐行对等」（`xtask/src/ci.rs` 与 `ci.yml`；第三处原在 `run-pipeline.sh` 头注，已被本卷改写掉，故不计），实测同一份登记表**一个判 0 一个判 1**（单行登记表把 `sqlcheck` 标未交付：脚本退 0，`cargo xtask ci` 退 1）。
   登记，改法按文档先行——先改那三处措辞再动判定，本卷不动。
