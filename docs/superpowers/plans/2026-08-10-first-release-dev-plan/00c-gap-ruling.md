...
结论：在 ep-foundation 中冻结 Tx、SnapshotCtx、UnitOfWork 三者，ep-adapter-db 只提供实现，契约层的跨模块方法签名一律写 `&mut dyn Tx`。
...
提供方要做什么：阶段 1 在 ep-foundation 增加 `port::tx` 与 `id::marker` 两个模块，在 ep-adapter-db 提供 `PgUnitOfWork` 与 `PgTx` 两个实现类型的声明位，实现落在 ep-adapter-db-pg。写入阶段 1 计划第 5.1 节 foundation 核心类型表与第 7 节并发与事务边界。
...
提供方要做什么：阶段 1 在 ep-foundation 实现该结构体与三个枚举，写入阶段 1 计划第 5.1 节，替换现有的八字段简表。
...
| 表 | 关键列 |
|---|---|
| platform_core.recon_check_definitions | check_code text 唯一、category text CHECK in SUBLEDGER_VS_LEDGER, CROSS_MODULE_LINK, INVARIANT、module_code text、statement_sha256 text、signed_statement_ref text、is_blocking_period_close bool、registered_by_module text |
...
结论：交付确认单归 sales 模块，因此归阶段 6，不归阶段 8。总览第 4.1 节把表建在 sales schema 却让阶段 8 的 ep-app-inventory 去写，直接违反基线第 1.3 节最后一条“一个仓储只访问自己模块的 schema”，且 sales.delivery_schedules 与 sales.return_line_delivery_links 两张已在阶段 6 的表都需要指向交付确认单的同 schema 真实外键，按基线第 3.3 节这两处外键不能是逻辑引用。阶段 6 在调整后的顺序中排在阶段 8 与阶段 9a 之后，库存腿与凭证腿的端口在阶段 6 开工时均已存在，不构成倒挂。
...
结论：两个枚举提前到 ep-foundation，由阶段 1 交付并回写基线第 12 节，各业务阶段在自己的 ep-contract 中为每个用例声明两个常量。

最终归属阶段：阶段 1 定义枚举，阶段 5 至 12 与 3b、4 各自声明常量，阶段 13 只做运行期判定。
...
结论：登记表与登记接口归阶段 9a，登记行由产生该事件的阶段在自己的迁移中插入。库存阶段登记零行，须显式写明。
...
`DegradationKind` 由阶段 2 定义为空枚举加两个初始取值 `OFFSITE_SINK_NOT_CONFIGURED` 与 `WRITER_ROLE_CONTAINMENT_MISSING`，阶段 14 扩展到十八类。指标 `ep_degradation_windows_open` 由阶段 2 注册并填充。
...
每个使用方要改什么。阶段 1 计划的自检项 11 在失败时调用 `DegradationLedger::open`，但阶段 1 早于阶段 2，因此阶段 1 只写 stderr 并留注释 `// TODO(stage-2): write degradation ledger`，阶段 2 补上。阶段 3 计划第 3.13 节依赖九删去 `platform_ops.degradation_windows` 一项。阶段 4、9、11、13 凡登记降级窗口的措辞改为调用 `DegradationLedger`。阶段 14 计划第 73 节明确本表由阶段 2 建立、本阶段只做扩展。
...
结论：阶段 8、9、10 在各自迁移中显式登记，由数据库检查断言登记与触发器一致。
...
回写：阶段 11 计划第 3 节新增该消费者并在退出条件中增加一条；阶段 8 计划第 6.4 节把该事件的消费者由“报表数据集”改写为 `costing.stock_value_adjust`。
...
### C-01 二十四个 schema、七个功能角色、二十四个属主角色、引导脚本与 order.toml

结论：全部归阶段 2，阶段 1 只保留目录约定与空壳。

最终归属阶段：阶段 2。

确切标识符：`db/bootstrap/00_database.sql`、`01_roles.sql`、`02_cluster_params.sql`、`03_role_defaults.sql`、`04_pg_hba.fragment` 五个文件名以阶段 2 的命名为准，阶段 1 的 `B001__cluster_roles.sql`、`B002__database.sql`、`B003__postgres_conf.sql` 三个文件名作废。`db/migrations/order.toml` 的二十四项顺序以阶段 2 第 3.3 节为准。阶段 1 只交付 `db/migrations/<schema>/` 二十四个空目录与一个只含注释与顺序数组骨架的 `order.toml`。

阶段 1 计划第 4.1 节与第 4.2 节的三个迁移文件全部移交阶段 2，阶段 1 的自检项 4 与 5 保留但被测对象由阶段 2 建立，阶段 1 交付时以 `ci_probe` 探针表作为被测对象。
...
回写：阶段 1 计划第 333 行整句改写为 `ep-foundation` 定义 `UnitOfWork`，两个方法为 `transact` 与 `snapshot_transact`，`ep-adapter-db` 提供实现骨架，`ep-adapter-db-pg` 提供实现。基线第 10.3 节在示例之后追加一句：只读快照事务的唯一入口是 `snapshot_transact`，配合 `SET TRANSACTION SNAPSHOT` 使用。阶段 2 计划第 265 行的 `UnitOfWork trait，唯一方法 transact` 改为两个方法。

### C-04 PoolKind、RetryPolicy、SessionContext、ConnectionBudget

结论：类型定义归阶段 1，取值与预算校验脚本归阶段 2。四个类型全部留在 ep-adapter-db，不进 ep-foundation。

最终归属阶段：类型归阶段 1，取值归阶段 2。

确切标识符：`ep_adapter_db::PoolKind { Rw, Ro, Worker, Integ, Ops }`；`ep_adapter_db::SessionContext { legal_entity_id, user_id, request_id, trace_id }`；`ep_adapter_db::RetryPolicy { max_attempts: u8, backoff_ms: [u16; 3], retryable_sqlstates: &'static [&'static str] }`；`ep_adapter_db::ConnectionBudget { resident_max: u16, burst_max: u16, per_pool: [(PoolKind, u16); 5] }`。取值固定为 max_attempts 3、backoff_ms [50, 150, 450]、retryable_sqlstates ["40001", "40P01"]、resident_max 42、burst_max 52。校验脚本名 `scripts/verify-connection-budget.sh` 归阶段 2。
...
结论：统一为 `ep_replication_crosscheck_age_seconds`，归阶段 14 注册，阶段 2 只填充。

最终归属阶段：注册归阶段 14，填充归阶段 2。
...
回写：阶段 2 计划把该指标标注为“由阶段 14 注册，本阶段只填充”，并改名；阶段 14 计划在指标清单中登记该指标。
...
确切标识符：`SelfCheckRegistry` 位于 `crates/platform/runtime/src/selfcheck/registry.rs`，注册项为 `SelfCheckItem { name: &'static str, title: &'static str, severity: Severity, run: … }`，name 为 kebab-case。基线十三项的名字固定为：`config-parsed`、`database-reachable`、`migration-version-matched`、`rls-enabled-and-forced`、`runtime-role-privileges-bounded`、`secrets-resolvable`、`audit-chain-verifiable`、`file-store-writable`、`clock-skew-within-limit`、`cgroup-quota-matched`、`offsite-sink-requirements`、`license-and-modules-consistent`、`current-period-open`。
...
本节按文件归纳，每个文件列出需要改的缺口编号与落点小节。执行修改的 agent 只改本文件，不改其他文件。
...
落点：第 2 节 crate 表（删去 EquipmentQuery，Customer360SectionProvider 由新增改为扩充）；第 3 节追加 project dataset view 一个迁移文件；第 4.7 节派生消费者名写死为 project.contract_derivation；第 4 节退货登记改用 SalesReturnCommandPort 与三个终态事件；第 9.3.6 节三个读取方改写；第 9 节退出条件（新增界面、数据集视图、能力域常量、ServiceReferenceCounter 四条）；第 11 节 R-01 与 R-02 的缓解措辞改写。
