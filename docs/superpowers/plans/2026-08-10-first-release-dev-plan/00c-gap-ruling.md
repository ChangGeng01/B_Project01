...
本文件包含**三类**内容，必须分开解释（F-67 更正——原自述「两类」漏掉了第三类：**F-58 起的现行裁定卷**，含 F-65 的开发授权逐字留证；该类是**现行记录**，不随首行横幅的 SUPERSEDED 分类失效，全仓授权注记的「留证 00c F-65」指向的正是它）：`F-52　最后五项内部开发阻断收口`、`F-53　阶段 14 历史迁移、补丁分发、支持套餐与病毒扫描部署收口`、`F-54　全局登记闭合与合同终止影响面平台补齐` 三个完整专项段，**按本文件首行横幅与 F-57 权威登记为历史裁决证据，不是现行施工口径**（本句原写「已批准的现行后续裁定与唯一实现口径」，与首行逐字「F-52–F-54 只保留历史裁决证据」相反，据首行更正）；其在各自明示范围内曾为已批准的后续裁定；若与更晚的 F-55/F-56 明示范围重叠，按同范围较晚裁定优先。除此三段外，本文件自本次修订起降级为 67 条历史登记表，不构成独立权威；一般权威链固定为规格、PRD、技术基线、各阶段计划，历史登记与这四层冲突时一律以后者为准。各阶段计划中的“按裁定 X-nn”只表示决策出处，取值以已回写的现行正文为准。
...
> **F-62 行号勘误规则：本文件各裁定内写死的行号以该裁定作成时点为准**——后续裁定在本文件与被引文件里插行会使其漂移（已核一批：F-60 引主卷 `:11321` 等因 F-61 插 5 行现实位 +5；`:8330` 现 `:8629`；`00b:128` 现 `:132`），**逐字引文全部属实，查询一律以逐字为键，不以行号为键；漂移不逐处回改。**

除上述 F-52/F-53/F-54 三段（**现为历史裁决证据，见首行横幅**）外，历史登记部分只承担三件事：登记 67 条缺口的原归属、原确切标识符、作废名清单。结论、最终归属阶段和确切标识符都只作长期追溯；一旦落入 F-50 至 F-56 的明示覆盖面，现行取值必须改读后续裁定与已回写计划；**F-57 最高，其权威登记与实施计划是现行唯一口径**（本句原止于 F-56；F-58 结论二第 4 行声称已对本处补记而实际未落，F-62 补落并记此差）。其中提供方要做什么、每个使用方要改什么、顺序约束三段，以及一切原方案作废、该措辞作废、其后编号顺延、总览第 N 节须改写一类句式，都是 2026-08-10 四次回写提交的施工期工单，回写已执行完毕，一律作废：不得引用、不得据以施工、不得据以判定评审阻塞，下一次修订本文件时逐条删除。文末的历史回写清单整节同此处理。
...
| ④a archive staging 故障包络 | 350 GiB | `archive.wal_spool_max_gb=350`；本机 archive staging 的独立硬界 |
| ④b 复制槽 `pg_wal` 保留包络 | 350 GiB | PostgreSQL `'350GB'` 按 1024 进位恰为 350 GiB；只约束 slot retention，不约束 archive failure |
| ④c 归档持续失败新增 live WAL 包络 | 350 GiB | 与 ④b 可并发；二者同处 `pg_wal` 但必须分账，合计硬界 700 GiB |
| ④d 全量备份本机暂存 | 24 GiB | 5 分钟 × 80 MiB/s 流式缓冲 |
| ⑤a 内置搜索索引 | 8 GiB | 可检索文本量 × 膨胀 × 合并期双份 |
| ⑤b 恢复及升级临时空间 | 48 GiB | ③ × 1.2 |
| **数据与三条独立 WAL/归档故障包络小计** | **1380 GiB** | `150+60+40+350+350+350+24+8+48` |
| **自检阻断闸门** | **100 GiB** | 上述 `FREE_SPACE_MIN_BYTES`，阻断级 |
| **NTFS 卷最低总容量** | **1480 GiB = 1,589,137,899,520 bytes** | 小计加不可借用的 100 GiB 自检底线；物理盘另须至少 2,000,000,000,000 bytes |
| **当前 1 TB HDD 可寻址约 930 GiB** | **短缺至少 550 GiB** | 即使尚无增长，也不能同时兑现三条 350 GiB 故障界与 100 GiB 底线 |

**交付前提（写入部署记录，不是运维建议）**：附件正文不超过 **50 GB/年**。
但增长假设不再能证明当前盘可行：930 GiB 已低于 1480 GiB NTFS 硬包络，也低于 2 TB 原始盘
交付下限。开发与非生产验证可以继续；生产认证/激活必须在物理盘和数据卷分别达到上述两项
底线后重测，不能用 744 GiB 告警或 830 GiB 阻断线
把先天短缺改写成可接受余量。
...
- 旧表同一个 470 GiB：占 2 TB 为 **23%**，占 930 GiB 为 **51%**；该旧表漏计三条独立 350 GiB 故障包络，已由 F-86 取代，不再是当前容量结论
...
**本段是当时等待使用方表态的历史处置。F-86 已证明 470 GiB 不是完整实测合计，
而是漏掉三条 WAL/归档故障包络的旧表值；当前只采用 1380 GiB 静态包络、1480 GiB NTFS
最低总容量、2 TB 原始盘底线与当前盘短缺至少 550 GiB 的结论。原「51%／逼近 80%／余量充足」继续撤回。**
...
> 交付机可寻址约 930 GiB；完整静态安全包络为 1380 GiB，另有不可借用的 100 GiB 自检底线，NTFS 最低需 1480 GiB，当前短缺至少 550 GiB，物理盘另须达到 2,000,000,000,000 bytes。
> **本机同时低于 F-86 的安全包络和 A.3 的 2 TB 交付下限 1118 GiB；这两项都阻止生产认证/激活。**
> 744 GiB 复核线与 830 GiB 阻断线只保留为扩容后的运行水位，不能解除交付前的容量失败。
...
> **F-62 注：上句为该段作成时点的状态。按本文件首行横幅与 F-57 权威登记，「可直接开发」已被 F-57 撤销（register RULING-AUTHORITY-01），现行开发授权状态为 `README:8` 逐字 `READY_NOT_AUTHORIZED`（F-62 作成时点；**2026-08-27 起授权已由 F-65 授予**）；F-52–F-54 三段并已降为历史裁决证据。**
>
...
### F-86　全面审阅后的可信入口、四池种子与 P340 容量包络收束

本节只关闭 2026-09-01 全面审阅中已经实证的矛盾，不把未来实现或外部实机结果写成通过。

#### 一　HTTP 来源和内部请求头由服务端单点持有

`http.trusted_proxy_cidrs` 是唯一可信代理入口，默认空。只有真实 TCP 对端命中该闭集时才解析
`X-Forwarded-For`；其余请求一律以 socket peer 为准。重复、过长、过多 hop、非法或全可信链都回落
直接对端。所有入站 `x-ep-*` 在 body、并发闸门、认证和 handler 之前按前缀剥离，request/trace 每次由
服务端新建；身份只能来自已验证会话写入的 `SecurityContext` extension。匿名登录名和来源 IP 的活动键
分别有硬容量，容量满时新键失败关闭，不以淘汰旧键为攻击者换键放行。

#### 二　四池 37 是当前 P340 实现种子，不是产品永恒拓扑

当前代码只保留 `rw|ro|worker|ops` 四种池，种子上限 `20|10|5|2`、常驻 37、临时 10、安全储备 5、
峰值 52、实例上限 64；integration-gateway 没有数据库配置、凭据或连接。该组数字只允许作为当前
P340 代的实现/测量输入。ADR-0019 要求的签名数据库消费者 exact set、同硬件并发实测和容量证书仍是
生产准入真值；拓扑、硬件或配置代改变即重测，不能反向把代码默认值升级为产品不变量。

#### 三　三条 350 GiB WAL/归档故障包络必须分账，1 TB 单盘不满足

PostgreSQL 16 配置单位使用 1024 进位，故 `max_slot_wal_keep_size='350GB'` 是 350 GiB；它只限制
replication slot 在 live `pg_wal` 的保留。`archive.wal_spool_max_gb=350` 限制 archive staging，属于
另一物理目录。归档执行器持续失败还会在 live `pg_wal` 形成独立的 350 GiB 故障桶。三项同值不等于
同一份空间；archive staging 堵塞、接收端停止和归档执行器失败可以并存，后两项合计 live WAL 包络
为 700 GiB。

完整静态包络为：附件 150 + 历史版本 60 + 数据库 40 + archive staging 350 + slot-retained WAL 350 +
archive-failure live WAL 350 + 备份暂存 24 + 搜索索引 8 + 恢复临时空间 48 = **1380 GiB =
1,481,763,717,120 bytes**；再加不可借用的 100 GiB free-space 阻断底线，NTFS 数据卷最低总容量是
**1480 GiB = 1,589,137,899,520 bytes**。物理企业 CMR HDD 标称原始容量同时必须不少于
**2,000,000,000,000 bytes**。当前 1 TB HDD 两项都不满足。因此：

1. 开发、单元/协议测试与不声称生产通过的验证可继续；
2. 当前单盘不得签发完整容量证书、不得通过生产认证/激活；
3. 扩容后同时满足 1480 GiB NTFS 卷包络和 2 TB 原始盘交付下限，并重新实测与签名；
4. 运行水位只会收紧，不能豁免硬容量短缺；
   批量暂停与全局 hold 均至少取 `max(existing threshold,100 GiB)`，100 GiB 不可借底线不得被旧
   50/40 GiB 日志策略下限放宽；
5. 归档失败最多每 30 秒采样；unarchived live WAL 达 300 GiB 或 live `pg_wal` 合计达 650 GiB 时进入
   deployment-wide hold、拒绝新业务写并受控停机，350/700 GiB 为不得越过的终值；
6. 容量证书必须证明峰值 WAL 生成率乘以（采样周期 + 最坏停机时间）小于 50 GiB 反应余量；
7. 不降低三项 350 GiB 安全界来迁就硬件。若未来要改界值，须以新实测、恢复/RPO 影响和新书面裁定同批重算。

本节取代 F-16、F-31/F-32、F-71/F-73 和 F-70 中与 40/350/470 GiB 容量算术冲突的数字结论；
那些段落保留为过程追溯，不再作为现行容量真值。

#### 四　CI 的每个完整命令行都是独立门禁

阶段 6 现同时登记 `configdoc` 与 `configdoc --check-doc-type-codes`；后者不再只出现在规格文字中。
登记表保留每套完整 argv 的独立行，包括同名子命令的第二套参数；`verify-pipeline-commands.sh`
只检查入口可用性、Cargo 解析约束及登记表与文档一致性，不执行这些登记命令。负样例证明的是上述预检边界；
后续完整 `cargo xtask ci` 才逐行执行并提供参数、退出结果与交付状态证据。它与备用 `run-pipeline.sh` 的聚合优先级
也已统一为「不符 1 > 不可判定 3 > 未交付 70 > 全通过 0」，不再对同一登记表给出两个结论。

类型码常量表是未来产品实现，本次不代写；因此该独立门禁当前如实返回 3（判定未做出）。
这是「门禁已接入、被测实现未开发」的预期红，不得改成通过或用已知红基线遮盖。
...
### F-85　收尾批：剩余项逐条了结——两条已修、三条判为「无处可改」、一条判假、三条留待外部条件

使用方逐字「剩下一次性 解决掉」。本批把 F-75／F-76／F-77 之后所有挂着的条目**逐条走到终点**，
不再留「待办」这种没有终点的状态：每条要么改掉，要么给出**它为什么改不了**的实证。

#### 结论一　两条已修，都是「文档自称完整而实际缺件」

1. **主计划补上两个自己用了却从未定义的类型。** `BackupSetIdV1` 与 `BackupCheckpointContextV1`
   在主计划 §3 被引用**四次**、定义数**零**，而该节逐字自称
   「All child plans use these exact names」——定义实际只在扩展发布计划里。
   照主计划实现的人**拿不到 `BackupCheckpointContextV1` 的 variant 集合**，只能自己发明一个，
   于是 `PACKAGE_MAINTENANCE` 分支所需的两个 ref 无处安放；`BackupSetIdV1` 若被实现成裸 `String`
   而非 `UuidV1` newtype，「`backup_set_id` is nonnil」的判据也没有可判的载体。已把两处定义
   逐字补进这份「唯一共享名字表」。

2. **`F57EvidenceSignerBrokerReadinessStateV1::Held` 登记为「未定义即阻断」。**
   该变体全仓**无任何产生条件、无任何消费判据**：三处定义该 readiness 的散文都只给两个状态，
   而它们说的 `NOT_READY` 是管道应答、不是本枚举的取值。后果是写 collector 的人不知道何时置 `Held`，
   写 verifier 的人只能在「`== READY`」（把一个合法闭集值判死）和「`!= WAITING_FOR_DATA_HDD`」
   （把一个语义未定义的值放进签名证据）之间二选一。
   **本卷不替设计方定义它的语义**，只就地规定：在其产生条件被明确之前，verifier **必须**把它当作
   未定义且阻断，**不得并入 ready 分支**；同时写明**不要删除该变体**——删 `deny_unknown_fields`
   闭集的成员是签名变更，须走协调改动每一份消费计划那条规矩。

#### 结论二　三条判为「无处可改」——功能尚未实现，不是缺陷

F-84 曾把这三条记为「只改了措辞、实现侧未落地」。本批实测后更正该表述：**不是「未落地」，是「无处可落」**。

| 条目 | 实测 |
| --- | --- |
| 替代标识的唯一索引 | `db/migrations/` 下 `alternate_identifier` 命中 **0**，`mdm/` 目录只有 **1 个**文件，`mdm.customers` 建表迁移**尚不存在** |
| 开票比例改只读派生 | `issued_ratio`／「开票比例」在 `.rs` 命中 **0** |
| 销项红冲封闭规则 | `invoice_reversal`／`reversal_line` 在 `.rs` 命中 **0** |

三者的规格与计划口径已在 F-84 改对；**索引随建表落、逻辑随功能落**，此刻没有任何文件可改。
这不是欠账，是顺序。

#### 结论三　一条判假——本卷第二个穿过对抗验证的假阳

F-76 丙组第六条称「对账 crate 有一条『不得出现字符串拼接 SQL』的判据窄到几乎不可能触发」，
F-81 曾如实记为「定位不到」。本批做了终局检索：**`xtask/` 下 `recon` 命中 0**；
`crates/platform/recon/src/` 七个文件内**无任何自扫源码的判据**（`include_str!`／`file!()` 全无）；
全仓「拼接」字样只出现在迁移工具、身份 MFA 与构建脚本的正常注释里。

**该判据在本仓不存在。** 这是继 F-81 结论一（`SecurityContext` 20 字段那条）之后，
**第二个通过了「默认驳回」对抗验证的假阳**。两者成因不同：前者是验证方核了文档说什么、
没核那份文档还算不算数；本条是验证方描述了一个它没能给出 `file:line` 的对象，
而**审阅侧当时没有把「拿不出 file:line」直接判为不成立**。记为方法教训：
**凡发现无法给出被测对象的 `file:line`，一律判不成立，不进「待定位」。**

#### 结论四　三条留待外部条件，各自写明转绿判据

- **`DbValue::Null` 绑 TEXT OID**（F-80 留）：`INSERT` 走赋值转换、`WHERE` 走隐式转换，
  两种上下文结论相反，**本机无 PostgreSQL 16 无从判定**。转绿判据：接上真库后分别构造
  「INSERT 空值到 uuid 列」与「以空值比较 uuid 列」两个用例，按实测决定是否需要类型化空值变体。
- **连接池 `after_release` 的改法**（F-82 留）：转绿判据是接上真库后确认
  一次建表加四次 transact **只出现一个 backend PID**。
- **经销商门户是否延期、备份两个保留期的合同措辞定稿**（F-84 留）：属使用方文案面，本卷只给口径。

#### 结论五　F-77 其余条目的处置口径，一次说清

F-77 判真 36 条中，甲组 3 条已修；其余分两类，**本卷不逐条改，理由是改法本身要先被授权**：

- **契约签名变更类**（如域分离判据缺 CUSTODY／LOCATION 字段、备份密文块 nonce 24 vs ADR-0021 的 12 字节）：
  主计划逐字规定「A signature change requires a design amendment and coordinated edit to every consuming plan
  before code changes」。改一个字段要同批改每一份消费计划，**不是可以顺手做的事**。
- **判据面变更类**（如 `--all-targets` 已修、其余恒真判据）：须文档先行，先定判据该长什么样。

F-79 已用「改判据的诚实度而不发明签名变更」的办法处置了域分离那条最要紧的；
同法可推广，但每一条都需要一次单独的授权。

#### 验证

六道门禁 2 绿 4 红、阶段表自检 0——与登记基线一致。本批只动计划文本与两处类型定义，未动代码。

### F-84　第七批：F-75 的规格与 PRD 十九条全部落地——含三条对外承诺

> **编号更正**：本节起初误编为 F-83，与先落的「F-83 第六批：F-76 丁组其余＋乙组漏项」撞号。两节内容互不相同、互不覆盖；按先到先得，本节改为 F-84、批次序号改为第七批。本节正文里出现的「F-83」自指标记同批改为「F-84」，指向另一节的引用不动。

使用方逐字「全面 修复 现有 问题」，即批准 F-75 授权清单的全案（确定改法直接改，带选项者取推荐的 A 案）。
**本批是本卷第一次动规格与 PRD 正文**，此前各批一律「只提出不代改」。

#### 甲组五条：会写进客户合同或对外承诺的

1. **备份保留期由「一个数」改为「两个口径分列」。** PRD 原只写「认证取值 14 天」并作为
   「必须出现在交付说明与客户合同中」的披露，而 F-57 生产档的抗勒索有效最短保留期取
   `max(站点法律期限, 90 天, 2×检测延迟P99＋洁净恢复验证窗口, 2×轮换间隔)`。
   **照原文做保留清理会主动销毁 F-57 要求保留的代际。** 已改为常规 14 天与抗勒索 ≥90 天两个口径同时披露，
   并把无条件句「平台不保留超出保留期的备份集」改为「超出**各自**保留期」。
2. **客户门户从「首版不做」改回首版客户端。** F-57 已把它写进 20 人容量认证的 exact 必测构成
   （15 Workbench＋3 客户门户＋2 供应商门户），不达此构成不签发证书。
   已改甲八行、1.6 表、`:422`、`:1795` 四处断言，**并补上此前缺失的两处**：
   11.6 承载形态表新增客户门户行、11.7 的公网入口前置规则从「供应商门户」扩到「客户门户与供应商门户」。
   反查确认 PRD 内「对外门户只有供应商门户」类断言**残留 0**。
3. **1.6 能力表不再承诺本地分析 AI。** 原写「包含 F-55 本地分析 AI」，与本 PRD 第 7 行、
   RULING-AI-01 裁决体、以及 F-57 逐字「低位文档不得反向恢复……首版本地模型」三处相抵。
   已改为「只保留 provider／MCP／权限／审计／隔离契约，本地分析模型实现按 RULING-AI-01 延期、当前容量档关闭」，
   甲九行同批更正。**起草方提醒了一处陷阱并被采纳**：RULING-AI-01 的标题仍是旧句「首版本地模型必须交付」，
   只有其裁决体才是延期，回写以裁决体为准。
4. **凭证来源封闭「四个」改「五个」**，补入 `HISTORICAL_MIGRATION`——`ledger.md` 冻结表早已把它列为第 19 个
   `VoucherSourceKind`，而规格的封闭句停留在 Stage 14 加入之前。
5. **成本归集 8.2 补毛利侧恒等式。** 原先未分摊差异只为成本侧定义、收入却全额归维度，
   于是各维度毛利之和**恒比真实总毛利多出一个未分摊差异**，而同节要求「可由用户直接验算」。
   已让末行同时给出成本与毛利两个分量，并与 8.3.2 驾驶舱「对收入、成本、利润三项成立」的处理对齐。

#### 乙组十四条：内部一致性

**确定改法九条**：`VoucherSourceKind` 18→**19**；错误码表阶段 13 `30`→**31**（唯有 31 才与本段自报的 128 闭合）；
§13.1 让路顺序里「集成网关的独立池」→「Ops 池」（前者被 ADR-0018／0019 固定为零、不存在，
而真实存在上限 2 的 Ops 池反被漏在保护外）；PRD 5.9 的「业务不变量违反」→「业务冲突」（归入规格 §15.1 五类闭集）；
7.4.2 的两个「第 N 节」→「第 6 节／第 8 节」；error-codes「后五条」→「后三条」（表体为准）；
`TYPE_CODE_NOT_REGISTERED` 的指针从本文件不存在的 §5 改指数据字典 §5；
data-dictionary §6 首段为 `breakglass_activations` 开单据类例外；引导脚本注释「五池 42」→「四池 37」。

**取 A 案五条**：

- **替代标识补唯一索引**。PRD 逐字要求「法人内唯一」，而落地侧只有非空与长度 CHECK；
  按同文件自陈「唯一性一律以数据库唯一索引为最终强制点」的判据，**没有索引即等于没有唯一性**。
  已补 `ux_customers_legal_entity_id_alternate_identifier` 并在校验表增行。
- **开票比例改为只读派生**（＝各行价税合计之和 ÷ 合同金额）。原为独立输入且只校验「≤剩余可开比例」，
  与行明细金额彼此无勾稽：**比例填 100%、金额填 1 元可全程绿灯**，随后申请单被置「全部开具」、
  剩余可开比例归零，**该合同再也开不出票**。改后与申请侧 6.3.2 的只读派生对称。
- **销项红冲对齐进项封闭规则**。原「数量与金额均大于零」直接排除了本 PRD 6.6.3 逐字声称
  「进项红冲的行级价量组合与销项**使用同一封闭规则**」里的 `NONE+ADJUSTED` 纯折让／纯税额更正，
  且与 openapi 共享行 schema、F-50、阶段 10 计划三处相抵。
- **分片上传声明唯一一条路由级 body 例外**。分片 8 MiB 而全局请求体上限 1 MiB，
  不设例外则**首个分块即被拒**、`platform.file.*` 一整组键随之取不到。
- **ops 双键消重**（本条按 A 案，理由是实测支持）：`ops.metrics_listen`／`ops.health_listen` 与
  `http.bind_addr`／`metrics.bind_addr` 各登记一次同一对地址，**无优先级也无相等性校验**，
  实际生效的是先绑定的那个、另一个静默失效。实测**代码侧对这两个键名零命中**，
  故删文档侧两行与代码一致，族计数 328→326。

#### 一处基线收窄，按纪律办

删两个键使 `configdoc` **实测 313 → 311**。按基线表自己的规矩「先实测、再改表、再留证」，
已把 `configdoc` 行改为 311 并写明收窄成因。**这是本卷第一次真正的收窄**（此前只有测试侧的 5→3）。

#### 验证

真全量 **1115/3/5 → 1116/3/5**（净增一条乙3 的索引相关断言）；六道门禁 2 绿 4 红、两道 CI 自检 0/0、
基线对照逐面相等——**无新回归**。

#### 本批未做到的

- **五条 A 案里有三条只改了规格与 PRD 的措辞，实现侧尚未落地**：替代标识的唯一索引在计划文档里补了，
  真实迁移文件里还没有；开票比例的只读派生、销项红冲的封闭规则同理。它们随各自 owner 任务实现。
- 经销商门户是否延期、备份两个保留期的合同措辞定稿，**仍属使用方文案面**，本卷只给了口径。
...
#### 结论一　一条判假，而且它通过了对抗验证——本轮第一个漏网的假阳

报告称 `foundation-frozen-items` 把 `SecurityContext` 期望字段硬编码 19，而**文档四处要求 20 字段
加 `SystemPurpose` 枚举**，故门禁永远绿、下游两个 Executor 到时会编译不过。

**逐处核出处状态，该结论不成立：**

| 出处 | 状态 |
| --- | --- |
| `00b-technical-baseline.md` | **`SUPERSEDED_DO_NOT_EXECUTE`** |
| `01-engineering-baseline.md` | **`HISTORICAL_DO_NOT_EXECUTE`** |
| `f51-development-readiness-freeze.md` | `PARTIALLY_SUPERSEDED` |
| **F-57 现行五份计划与四份规格** | `system_purpose`／`SystemPurpose` 命中 **0**，未规定字段数 |

即：**要求 20 字段的四处出处全部已被取代，F-57 对此没有任何规定**。代码的 19 与门禁的 19 彼此一致。
再加一条旁证：同一段旧基线把 `ClientKind` 冻结为**八个**变体（含 `ServerAdmin`／`Mcp`），
而代码与门禁都是 **6**——**照那份旧基线改会一次改错两处**。

**作成方差点据此改代码。** 这是本轮第一个穿过对抗验证的假阳，成因是验证方核了「文档说 20」，
**没核「那份文档还算不算数」**。记为方法教训：**凡引文档为据，必须同时核该文档的横幅状态。**

**残留的真问题另记**：`frozen.rs` 的期望值确实是**代码现状的转抄，不是权威的转抄**——
而同目录的 `foundation-module-registry` 早把这个反模式写在自己注释里。
但 F-57 下**没有可读的权威**，所以本卷不改实现，只在 `frozen.rs` 就地如实登记：
它今天只能证明「代码没有自我漂移」，**不能证明「代码与权威一致」**；
转绿路径是 G0 生成式再基线为这些闭集建立现行权威后改为从该权威读取。

#### 结论二　`unwired-absent` 漏掉裸名——用旧新判据对拍证明

`UNWIRED_PREFIXES` 是 `["Noop","Stub","Fake","Dummy"]`，而匹配条件逐字要求
「前缀后必须紧跟**大写字母或行末**」。于是最典型的占位实现名全部漏判。**对拍实证**：

| 输入 | 旧判据 | 新判据 |
| --- | --- | --- |
| `pub struct Noop;` | **False（漏）** | True |
| `Box::new(Stub {})` | **False（漏）** | True |
| `    Fake,` | **False（漏）** | True |
| `wire(Dummy)` | **False（漏）** | True |
| `let x = Noopy;` | False | False（仍正确排除） |

该规则的全部理由就是禁止在两个 wiring 目录注入这四种前缀的实现——**漏掉裸名等于漏掉它要拦的主要形态**。
已把 `after_ok` 放宽到「大写字母、**非标识符字符**或行末」，并补五条断言钉住四种收尾。

#### 结论三　`sqlcheck` 的规则表少一条，且把引导目录的规则算进了迁移目录

三处一并更正：

1. **`SQL-030` 根本不在 `RULES` 里**，只作字面量出现在 `checked.push` 与 `line_rule` 中。
   迁移目录为空时它**从三态报告里整条蒸发**——读报告的人看不出它没跑。已补进 `RULES`（14 → 15 条）。
2. 原式 `chain([&RULES[12], &RULES[13]])` 取的是 **`SQL-021`** 与 `SQL-031`，
   而 `SQL-021` 判的是**引导目录**、另有分支处置，于是同一条规则会**同时出现在「已判定」与「判定未做出」两栏**
   ——两栏按纪律「互不合并、任何一件不得被读成另一件」。
3. 索引写死在 `chain` 里，`RULES` 增删条目即静默指错。已改为**按取值语义取，不按下标**。

实测：`sqlcheck` 现在自报「**15 条规则**」（原 14）且通过。

#### 结论四　降级窗口的唯一约束在它本该生效的全部取值组合上不生效

`unique (kind, subject, scope_legal_entity_id, scope_accounting_period_id, closed_at)`——
PostgreSQL 唯一约束默认 **NULLS DISTINCT**，而 `subject` 与两个 scope 列都可空，
三个 `kind` 初值又都是**部署级、无法人无会计期间**的窗口，正常写法就是 scope 两列取 NULL。
于是「同一对象至多一条活动条目」在**最常见的取值组合上完全不成立**：
每触发一次就多一行活动记录，gauge 虚高，`close()` 又按 `is not distinct from` 一次关掉全部同类行。
已改 `unique **nulls not distinct** (...)`（PG15+，本项目目标是 PG16）。
`closed_at` 非空且有 `default 'infinity'`，不受影响。

#### 结论五　集成网关的启动演练自检是拿白名单问它自己

逐字 `for t in allowlist { if !is_allowed(allowlist, t.as_str()) }`，而 `is_allowed` 逐字是
`allowlist.iter().any(|t| t.as_str() == target)`——**纯字符串相等**。
拿白名单里的项去问白名单认不认，**必然命中自身，没有可失败的输入**。

后续复核又证明，仅在 `rehearse` 拒大写主机和显式 `:443` 仍不够：配置层还会接收
userinfo、query、fragment、反斜线、端口 0，以及 `127.1`／`0x7f000001` 这类会被部分 URL
客户端重新解释成 IPv4 的旧式数字主机。现已把唯一判据收回 `EgressTarget::parse`：只接收
ASCII 小写规范 DNS、规范四段 IPv4 或方括号内的小写压缩 IPv6；路径、查询、片段、用户信息、
空白、通配、显式默认端口、非规范端口和数字地址别名一律在反序列化时失败。`rehearse` 不再重复
一套较弱的 URL 判据，只演练 Closed → Open → HalfOpen → Closed 熔断状态机；精确相等匹配由
newtype 的唯一规范形前置保证。正负样例同时钉住 DNS、IPv4、IPv6 与旧式地址别名。

#### 未做到的

- **第六条未定位**：报告称对账 crate 有一条「不得出现字符串拼接 SQL」的判据窄到几乎不可能触发。
  在 `crates/platform/recon` 与 `xtask/` 下按「拼接／concat／静态判据／封闭性」检索**均无命中**，
  **本卷定位不到该判据本体，故不处置**；如实记，不假装已核。

#### 验证

真全量 **1113/3/5 → 1115/3/5**（净增本批两条测试）；六道门禁 2 绿 4 红、两道 CI 自检 0/0、
基线对照逐面相等——**无新回归**。
...
每条发现由独立一路按「默认驳回」核实。**报出 60 条，判真 31 条，驳回 29 条（48%）**。
按类：错了不会当场报错 10、恒假的触发 7、取不到的取值 5、恒真的判据 3、与文档不符 3、错误被吞 2、资源未释放或无界 1。

**本节是本卷迄今最重的一批**，因为它们不是文档账目，是接上真库就会发生的事。

#### 甲　「接上真库必然失败，而全部测试都看不见」——八条同族

这一族的共同形态：单测走 `FakeConn`／进程内注册表，不碰真实列约束与函数解析，
**本地与 CI 全绿，只有接上真 PostgreSQL 那一刻才炸**。

1. **四个数据库角色从未被授予 `ep` 库的 `CONNECT`**（`ep_app_rw`／`ep_analyst_ro`／`ep_ops_ro`／`ep_breakglass`）。
   引导脚本与全部迁移成功、十三项 check 全绿、退出码 0，**而应用第一次建连即在握手阶段被拒**。
   对 `ep_breakglass` 尤其致命——它存在的全部意义就是别的路都断了还能进去。
2. **迁移开窗语句 `make_interval(mins => $5)` 收到 i64 绑参**，真实库 Parse 阶段即报 42883。
   迁移窗口是**结构变更的唯一放行闸**，这条失败把整条运维路径堵死，而错误只说「数据库返回了未预期的错误」。
3. **`decode_row` 只给 INT8/BOOL/UUID/TIMESTAMPTZ/DATE 留了分支，int4/int2 一律落文本解码报 `ColumnDecode`**——
   迁移开关窗、高风险权限判定、密钥域与数据密钥列表、授权快照载入，都在第一次取行时失败。
4. **`data_keys` 插入漏掉 NOT NULL 且无默认值的 `activated_at`** → 密钥域开通与轮换必然 23502。
5. **降级窗口开窗漏掉 NOT NULL 的 `opened_at`** → 台账开窗必然失败，
   而降级窗口是「端口未装配时不得静默成功」的唯一登记机制。
6. **授权校验和语句调用未加 schema 限定的 `digest()`**，而 pgcrypto 装在 `platform_core`、`public` 已被删除 → 42883。
7. **`DbValue::Null` 一律绑成 TEXT** → 落在 uuid/timestamptz/bytea/jsonb 列上被服务端 42804 拒绝。
8. **回填迁移 #27/#28 向 FORCE RLS 表写入却从不设 `app.legal_entity_id`**——
   今天不炸只因法人表为空、循环体一次也不执行；任何「法人先存在」的顺序都会让迁移硬失败。

#### 乙　安全面三条

- **`PRE_AUTH` 白名单用同一个布尔量豁免四条路径，使
  `GET /api/v1/platform/identity/me/legal-entities` 变成匿名端点**：
  带 `X-Device-Id`／`X-Client` 加自造的 `x-ep-user-id`／`x-ep-legal-entity-id`／`x-ep-session-id`／
  `x-ep-duty-classes` 四个头、**不带 Authorization**，即可读任意用户的授权法人清单。
- **登录前限流器的两张 `HashMap` 只重置计数从不删键**，键取自攻击者可控的登录名与 `X-Forwarded-For`。
  **这是唯一一个不需要凭据就能让 core-server 单调涨内存的路径**，而它自己就是防爆破用的；机器只有 32 GB。
- **`auth.x509.trust_anchor_ref` 用裸 `String` 承载 `secret://` 引用**，
  绕过 `SecretRef` 这条被代码注释逐字称为「明文口令写进配置的唯一防线」的 parser；
  文档 `:699` 逐字禁止「退化为普通 `String`」。

#### 丙　门禁自己是恒真判据——六条

- **`foundation-frozen-items` 把 `SecurityContext` 的期望字段数硬编码成 19**，而四处文档逐字要求 **20 字段
  加 `SystemPurpose` 枚举**。期望值抄自代码现状而非基线，于是**漂了一个字段、一个枚举、一个构造器参数仍报绿**。
  下游 `ReconExecutor` 与 `DataMigrationExecutor` 要调四参构造器，到时直接编译不过，**而现在没有任何信号**。
- `integration-gateway` 启动演练的白名单自检**比对两侧同源**，任何白名单都必然「命中自身」。
- `ux_degradation_windows_kind_scope_closed` 声称保证「至多一条活动条目」，
  但三个键列可空加 NULLS DISTINCT，**在它本该生效的全部取值组合上完全不生效**——台账行数随请求量无界增长。
- `unwired-absent` 漏掉裸名 `Noop`／`Stub`／`Fake`／`Dummy`——**最典型的占位实现名恰好在判据之外**。
- 对账 crate 的「不得字符串拼接 SQL」判据窄到几乎不可能触发，大写关键字或换行即静默放行。
- `sqlcheck` 空迁移目录分支取错数组下标：**SQL-030 从三态里整条蒸发**，SQL-021 同时被判「已判定」与「未覆盖」。

#### 丁　其余

- **连接池 `after_release` 的断言恒为假**：每次归还都销毁连接，五具名池退化成「每事务一次建连」。
  **实跑 PostgreSQL 16.14 证实**：一次建表加四次 transact 出现 **5 个不同 backend PID**。
  池只剩限流器作用，`max_lifetime`／`idle_timeout` 与规模表全部失效；在单块机械盘上这是真实的吞吐杀手。
- **`user_grants` 以 `lifecycle_state = 'ACTIVE'` 过滤，而该列 CHECK 根本不允许 `'ACTIVE'`**——
  每个真实用户的 roles 与 duty_classes 恒为空集，方向是 fail-closed，但等于身份域上线即全拒。
- **六个已登记的配置键全仓无取用者**，其中 `http.max_body_bytes` 是唯一的请求体上限登记；
  实际生效的是 axum 的 2 MiB 隐含默认，**比登记值还大**。
- `ep_http_request_duration_seconds` 的 `client` 标签**写死 `"ops"`**，登记的另外六值永不出现，
  而 `X-Client` 就在同一个请求头里且已被强制校验。
- 会话准入等待直方图的标签数与登记表不符，`observe` 恒返 `Err` 并被 `let _ =` 吞掉，**永远收不到样本**；
  而「有 TYPE 行」恰是既有判据，指标门禁也不会响。
- 加密字段取值非字符串时 `unwrap_or_default()` 产出空串，`hex_decode("")` 返 `Some(vec![])`，
  **拿空信封去调 KMS 解密**，那个用来兜底的 `else` 分支永远进不去。
- 迁移文件读不出来时 `unwrap_or_default()` 把 I/O 失败**伪装成「文件被篡改」**，运维会去查供应链。
- 回写 `applied_versions` 的 UPDATE **不检查受影响行数**，窗口中途失效被静默吞掉：库结构已改、台账无记录。
- `config-reference` 与代码默认值**四处取值漂移**（`resident_max` 差 5 直接改变启动拒启阈值、
  `spool.max_bytes` 差 80 倍），而 `configdoc` 只比键名。
- `probe.rs` 的 165 行与 8 个用例**在任何 CI 命令下都不编译**，`-D warnings` 与 clippy 对它从不生效。
- 内存幂等实现声称的两道 CI 防线都不存在，且它逃过了 `unwired-absent` 的前缀名单。

#### 结论　本卷一条未改，全部交使用方定优先级

31 条**全部是代码缺陷，不是规格问题**，按纪律不需要改规格授权；但本卷仍未动手，理由有二：

1. **甲组八条一改，多道门禁的实测计数就会变**，必须与 `known-red-baseline.tsv` 同批走「先实测再改表再留证」；
2. **丙组六条是改门禁本身**，属判定面变更，须按文档先行——先定判据该长什么样，再改实现。

**建议的动手顺序**（按「不修就一定会咬」排）：乙组安全三条 → 甲组一（角色 CONNECT，一行 GRANT）→
甲组其余七条 → 丙组一（`frozen.rs` 的 19→20，连带 `SystemPurpose` 落地）→ 丁组池 → 其余。

#### 本轮未做到的

- F-57 语料那波（36487 行 / 26 段）仍在跑。
- 被驳回的 29 条未逐条复核驳回理由。

### F-75　规格与 PRD 首次全量逐句精读：报出 68 条，对抗验证判真 21 条

对规格（2056 行）、首版 PRD（4866 行）与八份参考文档（合 11196 行）作**首次逐句**精读，
分 12 段并行，每条发现再由独立一路按「默认驳回」核实。**报出 68 条，判真 21 条，驳回 47 条（69%）**——
驳回率本身是这套验证有效的证据，未经验证的原始发现不可直接采信。

按类：闭集算不平 4、内部矛盾 7、指向已取代的世界 3、错了不会当场报错 3、跨文件矛盾 3、取不到的取值 1。

#### 甲　会写进客户合同或对外承诺的三条（最要紧）

1. **备份保留期**：PRD `11.11.11` 把「认证取值 14 天、下限 7 天」作为**必须出现在交付说明与客户合同中**的
   披露，而 F-57 的有效最短保留期公式含 **90 天硬项**并取最大值。按 PRD 写合同即向客户承诺 14 天并声明
   「平台不保留超出保留期的备份集」；按 PRD 做保留清理会**主动销毁 F-57 要求保留的代际**。
2. **客户门户**：PRD 附录甲·甲八把客户门户列入「首版不做」并断言对外门户只有供应商门户一类，
   而 F-57 已把客户门户定义为首版客户端并写进 **20 人容量认证的必测构成**。
   连带 11.6 承载形态表无客户门户行、11.7 只为供应商门户规定公网前置。
3. **本地 AI**：PRD `1.6` 的「首版取值」能力表仍写「**包含 F-55 本地分析 AI** 与双向 MCP」，
   而同一 PRD 第 7 行与 `RULING-AI-01` 均已把本地模型实现延期，
   F-57 总体设计 §1.1 末句更逐字禁止「低位文档反向恢复……首版本地模型」。

#### 乙　闭集算不平四条

- **凭证来源「只由四个来源产生」漏了第五个** `HISTORICAL_MIGRATION`（Stage 14 历史迁移的唯一合法入口）。
- **`VoucherSourceKind` 固定 18 项**，而规格同一句指定为「唯一输入」的冻结表是 **19 项**，CI 已按 19 校验。
- 参考文档 §12 段首自报「阶段 13 = 30 条」，表体实为 **31 条**；三个自报数不能同时成立。
- 成本归集查询 8.2 **收入列与毛利列不闭合**：未分摊差异只为成本侧定义，合同交付节点确认的收入
  从结果表里消失，而 8.2.3 要求恒等关系「可由用户直接验算」。

#### 丙　「错了不会当场报错」三条

- **客户档案的「法人内唯一的替代标识」没有唯一索引**（落地侧只有长度 CHECK 与非空要求），
  而同一计划自陈「唯一性一律以数据库唯一索引为最终强制点」。两条客户填同一标识会双双建档、
  双双进合同与应收台账；PRD 又逐字声明首版**没有合并去重工具**。
- **销项开票的「本次开票比例」与行明细金额彼此独立提交**，全链路无一条校验把二者绑到合同金额上。
  比例填 100% 而金额填 1 元，全程绿灯：申请单被置「全部开具」、剩余可开比例归零，**该合同再也开不出票**。
- **ops-agent 的两个监听地址被两套键各登记一次**，无优先级也无相等性校验；
  实际生效的是先绑定的那个，另一个静默失效——而这两个正是发现其他故障的入口。

#### 丁　内部矛盾与其余

- **`§13.1` 要求保护「集成网关的独立池」**，而 `§7.7` 与 ADR-0018/0019 把 integration-gateway 的
  数据库连接与连接池**固定为零**；同一句还漏掉真实存在的 Ops 池（上限 2）。照它落地会去建一个
  ADR-0019 逐字禁止的「别名池」，而 ops-agent 反倒不受挤占保护。
- **销项红冲行「数量与金额均大于零」**排除了同一份 PRD 在 41 行后声称销项同样适用的
  `NONE+ADJUSTED` 纯折让／纯税额更正，且与 openapi 共享行 schema、F-50、阶段 10 计划三处相抵。
- **分片上传分片 8 MiB，而全局 HTTP 请求体上限 1 MiB**，且文件自称无路由级例外——
  附件上传在首个分片即被拒，`platform.file.*` 一整组键随之取不到。
- 引导脚本的连接预算注释**仍按已裁撤的 integ 池记 42**（登记值 37），同一个峰值 52 有两套互斥构成。
- PRD 5.9 的错误分类「业务不变量违反」**不在规格 §15.1 的五类闭集内**，全仓仅此两处。
- 另有 `7.4.2` 残留「第 N 节」占位符（与冻结项 F-21「不保留名称占位符」冲突）、
  error-codes 的「后五条只登记不返回」与表体「返回方」列冲突、
  `PLATFORM.SEQUENCE.TYPE_CODE_NOT_REGISTERED` 的指针指向本文件不存在的登记表、
  §6 首段「本 schema 各表均非单据类」与 §6.21 自称单据类冲突、
  bootstrap 事务只写 default CONFIG_RELEASE chain 而登记为 37 项。

#### 结论　本卷只修一条，其余全部交使用方

**已修**：`data-dictionary` 的 `§6.21`／`§8.14` 逐字「随编号生成本体（属 3b 同批）一并补齐」——
这是 **F-67／F-68 更正 `:131`／`:137` 时漏改的另外半边**，同一对象两处说法相反，
且「3b」是已被 F-57 取代的旧阶段号。属本卷自己的半改，直接修。

**其余 20 条一律不动**：它们全部落在规格与 PRD——按本卷一贯纪律，
**改规格须使用方逐轮授权，裁定只提出不代改**。甲组三条尤其如此：它们决定对外承诺的内容。

#### 本轮未做到的

- 另两波（F-57 语料 36487 行、Rust 代码 72805 行）仍在跑，未出结果。
- 被驳回的 47 条未逐条复核驳回理由是否成立——只采信判真的 21 条。

### F-74　ADR-0026 的承接挂在一个被现行设计自己标为 legacy 的标识上

消化已落盘结果期间的一处实测发现，**关于本卷自己写的 ADR-0026**。

#### 结论一　`owner_task` 是 `LegacyOwnerTaskIdV1`，而 ADR-0026 把它当现行承接

ADR-0026（F-69 落、F-73 改）第三条逐字「承接改挂 F-57 的 requirement→owner-task 映射」，
并列表把五组不变量的承接写成 `owner_task=F57-20` 等。实测三件事：

1. **收敛主计划 `:365` 逐字 `pub struct LegacyOwnerTaskIdV1(String); /* private; F57-(01..25) */`，
   而 `owner_task` 字段的类型正是它**（`:380`／`:393`／`:404`／`:418`）——
   **现行设计自己把 `F57-NN` 标成 legacy**。全文无散文解释其去留。
2. **`F57-NN` 在现行五份计划里只作为交付波次标签出现**：主计划 `:11247-11248` 的
   delivery-profile 两行，与 G0 计划 `:838` 的一个 `match` 分支（映射到 `G5Integration`）。
   **没有任何一处把 `F57-NN` 对应到一个成文任务。**
3. **唯一存在 `Task N ≡ F57-NN` 对应关系的文件是
   `2026-08-23-f57-governed-automation-fabric-implementation.md`**——
   实测其 `Task 20`（`:2978`）正文 `:3019` 逐字「every row whose `activation_task=F57-20`」。
   而该文件横幅逐字「状态：`HISTORICAL_DETAIL_INPUT`，**永久禁止按本文件执行**」，
   并逐字「本文件任何切片均不构成任务、命令、迁移」。

**所以 ADR-0026 的承接链止步于一个 legacy 标识**：它有波次、有 TestID，
但**没有活的成文任务**；唯一有的那个在一份永久禁止执行的文件里。
这不推翻 ADR-0026 的实质结论（五组义务保留、挂载点已定），
但它自称的「挂在有 owner、有 TestID、有交付波次的现行映射上」里，**「有承接任务」这一层不成立**。

#### 结论二　随之实测到的覆盖数，作为测量登记，不作缺陷断言

| 测量 | 值 |
| --- | --- |
| 现行五份计划的成文任务 | **46 个**（`G1-` 7、`G2-` 7、无波次前缀 32） |
| ~~`G3-`／`G4-`／`G5-` 前缀的成文任务~~ | ~~**0 个**~~ **F-74 当日自我更正：该行字面为真但误导，已作废** |
| seed 的 185 条需求中，在现行五份计划里**一次都没被点名**的 | **88 条（48%）** |
| 其中落在 G5_INTEGRATION／G4_CTC01 的 | 63／13（合计 76 条） |
| 落在更早波次的 | **12 条** |
| 185 个 TestID 中未被现行计划点名的 | **184 个** |

**F-74 当日自我更正（重要）**：本节原据「`G3-`/`G4-`/`G5-` 前缀任务为 0」暗示后期波次没有成文任务，**该暗示是错的**。逐个读完 46 个任务标题后确认：波次命名方式在计划间变了，覆盖并没有缺——

| 波次 | 承载 | 收尾 |
| --- | --- | --- |
| G0 | `g0-bootstrap` Task 1–6 | issue `G0_BOOTSTRAP_GREEN` |
| G1 | `authority-spine` `G1-01…G1-07` | Issue `G1_AUTHORITY_SPINE_GREEN` |
| G2 | `authority-spine` `G2-01…G2-07` | — |
| G3 | `ctc01` Task 1–5 | issue `DEV_SLICE_GREEN` |
| G4／G5 | `expansion-release` Task 1–15 | Aggregate `INTEGRATION_GREEN`；issue `RELEASE_CERTIFIED` |
| 各波受理 | `converged-program` Task 1–4 | 逐波 execute and accept |

**所以「五份计划集是否覆盖到全部波次」这个口径问题不存在，无需使用方裁定**——它覆盖了，本卷原来的提法作废。留下的仍成立的测量只有一条：**185 条需求里 88 条、185 个 TestID 里 184 个，在现行计划正文中未被其 ID 逐字点名**。这只说明计划与 seed 之间没有以 ID 为键的双向可查性，**不说明内容没被覆盖**——ctc01 与 expansion 的任务正文大量按能力与场景描述，不按 RequirementID 索引。

**不据此断言缺陷**：「没被 ID 点名」不等于「内容没被覆盖」。
但要并记一处张力——就绪评审 `:11` 逐字称其为「一套**可直接执行的**设计与五文件计划集」，
权威登记 `:13` 逐字「现行可执行计划；**内容已就绪**」，
而 G3 以后没有成文任务、近半需求行与几乎全部 TestID 未被点名。
~~是否属实质缺口，需要一次口径裁定~~ **已由本节当日自我更正取消：波次覆盖不缺，该裁定问题不成立。**

#### 本轮两次险些报错，记为方法教训

- 首次统计用 `grep -l "$t" $LIVE`，**zsh 不对未加引号的变量做词分割**，
  `$LIVE` 被当成单个文件名，于是「21 个 owner_task 在现行计划里命中 0 份」——
  差点据此报出「全部需求都没有实现计划」的巨大假发现。与本卷早先的 `awk $6` 同类。
- 第二次即将把「88 条需求无计划」定性为缺陷时，读到字段类型名含 `Legacy`，
  结论随即收窄为本节结论一。**类型名救了一次误判。**

### F-72／F-73　对 F-69…F-71 的复演验证：本卷自己造成的十余处，已修

复演四路（G0 Task 1 端到端、本批传导面二次反查、判据复核、新缺陷猎扫）出 43 条，
**其中 15 条是本批引入的新缺陷、15 条是本批漏改**。本节合并记 F-72（当日撤回）与 F-73（本次修复）。

#### 结论一　F-69 插一行，把 ADR-0026 自己的四处行号引用全推错了

F-69 在收敛主计划 §2.1 按字母序插入 `inventory-costing`，落点第 86 行，**其后每行 +1**。
而 ADR-0026 第五条那张挂载点表引的正是这些行号：`:87`／`:86`／`:94`／`:88` 全部失准，
**其中「库存数量与金额守恒 → `inventory-fulfilment`（:86）」现在恰好指着 F-69 自己新增的那一行**。
该表是转绿判据二取 `crate_root` 的唯一出处，而判据二比对的是**路径前缀不是行号**——
**错了不会当场报错**。同因失准的还有 `:11247`（实位 `:11248`）与 `:11461`（实位 `:11462`）。

**修法不是改数字，是换引用方式**：该表已改为**按 `feature_owner_id` 名引用，不写行号**。
名字是闭集成员、增删行不漂移；行号会。这是本卷第三次在同一病灶上返工，
前两次都只改了数字，这次断掉引用方式。

#### 结论二　F-69 制造了一处两份现行权威各说一套的事实归属

业务执行契约 §0.2 逐字「| 库存数量、**金额**、批次和序列事实 | `inventory-fulfilment` |」，
而 F-69 把金额与成本层事实划给 `inventory-costing`。**两份 `CURRENT_SUBJECT` 权威对同一笔事实给出不同 owner**，
且 G0 不会当场报错——单一 writer 校验只在 `Activation` 闭包内生效，G0 是 `BootstrapImport`，
库存要到 G5 才进闭包。已改契约 `:53` 拆成两行；`:67` 的共享 schema 闭举补上 `inventory`
（`inventory-fulfilment` 与 `inventory-costing` 自 F-69 起共用它，而原闭举只列 `invoice`／`finance`／`portal`）。

ADR-0026 第五条的解析表同批补齐：原表只有三行、后两组解析不到 `crate_root`，
与本篇「两组可即刻开工」自相矛盾；「库存数量与金额守恒」一组按核销守恒的先例**一分为二**。
解析式本身也改了——原写「其 CapabilityOwner 对应 `FeatureOwnerIdV1` 行」，
而 CapabilityOwner 取值如 `inventory`／`costing` 根本不是该闭集的成员，
且 `inventory` 在增行后同时是两行的 DB schema，**「可机械求值」不成立**。

#### 结论三　F-72 虚构了一个冲突——出处记错

F-72 曾把 `max_slot_wal_keep_size` 标成「待裁定」，称脚本上一行的「等于附录 A.3 连续归档本机保留子项」
与 `config-reference` 的「同值等于 `archive.wal_spool_max_gb`＝350」不可能同时成立。
**那是把出处记错了**：附录 A.3 逐字是「连续归档本机保留约 **350 GB**」，
本行取 `'350GB'` 与它一致、与 config-reference 也一致——**三处本来就是一致的**。
那个 40 GiB 出自 00c 自己为 P340 单盘重算的容量表，不是 A.3。已撤销该标注与整段虚构冲突。

真实存在的差额比那轻，且属容量账不属本行取值：**P340 重算表把 ④a 记为 40 GiB，
而本行按 A.3 取 350 GB——P340 容量表里没有任何一行给这 350 GB 留位。** 登记于此，不改取值。
F-71 结论二里「改成 40 GiB 后翻转点推到 790 GiB」一句随撤回失效，同批划掉。

#### 结论四　本卷新造的工具有三条静默假绿路径，全部实跑构造出来并已修

`compare-red-baseline.sh` 存在的全部意义就是不让人误判「没有回归」，而它自己：

1. **基线表末行没有换行时，该行被 `while read` 静默丢弃**，脚本照常判「无新回归」退出 0。
2. **`expect_count` 写成非数字时**，`[[ -gt ]]` 报语法错、两个分支都取假，
   直接落到 else 打印「与基线一致」退出 0。而 `-` 正是本脚本自己用来表示「计数读不到」的记号。
3. **`archcheck` 的汇总行是「违反明细（N 处）」**，不是「不符／不一致」，
   原正则读不到 → archcheck 一旦回归被判「未覆盖」（3）而不是「新回归」（2）。

另修一条方向相反的：门禁被**真正修绿**时退出码变好，原判据只判「不等」，会把修绿报成新回归。
四处已修，三条假绿路径各构造反例复验（分别得 2／3／0）。

#### 结论五　本卷自述账目三个版本互不相同，且都没数对

同一批传导面：提交信息说**十一处**、00c 说**九处**、ADR-0026 说**八处**，实际还漏了
`converged:10258`（逐字「the real **17/35**-registry owner」——`35` 是 F-61 改对的，`17` 这次漏了，
同一处复合计数被连续两批各改一半）与 ADR-0026 影响范围缺 `inventory-costing`。**已补。**
自述账目不可信这件事本身要记：**本卷三次声称「已逐处改并反查」，三次都有漏。**

#### 本轮未做到的

- `crates/features/inventory-costing` **在五份计划里既无 `Create` 也无 `Modify`**——
  F-69 增了 registry 行，却没有任何任务会去建这个 crate。属承接缺口，本卷只登记。
- P340 容量表与 A.3 的 350 GB 差额未裁（见结论三）。
- 三波全量精读（F-57 语料 36487 行、规格与参考文档 11196 行、Rust 代码 72805 行）
  **因使用方叫停而中止**。**已完成的 agent 结果已落盘保留**（分别 10／15／3 条，共 28 条），
  三份脚本连同 runId 留在 session 目录，可用 `resumeFromRunId` 原样续跑——
  已完成的部分直接命中缓存，只重跑未完成的那些。这批结果**本卷尚未消化**，属未做完的账。

### F-71　使用方裁定「改成 C，前三条都动手」——三处已改，并更正 F-70 自身两处说重的话

使用方逐字「改成 C，前三条都动手」。三处已落，另更正 F-70 自身两处过重的表述与作成方一处口头错误。

#### 结论一　建库排序规则改回 `libc`/`C`——这条不是取舍，是执行已有裁定

本卷此前把它列为「三处互斥」其实说轻了。裁定 00c 早已逐字写明两条：
「数据库 locale **已冻结，不再二选一**：固定为 `LOCALE_PROVIDER libc`、`LC_COLLATE 'C'`、`LC_CTYPE 'C'`」，
以及「旧 `LOCALE_PROVIDER icu`、`ICU_LOCALE 'zh-Hans-CN'`……只作历史证据，**不得实现**」。
而 `db/bootstrap/00_database.sql` 实现的恰是被明令不得实现的那一支，`db/checks/12` 还在断言它是对的。
**已落盘的脚本在执行一条被撤销的决定，且建库参数一经使用只能重建库才能改。**

已改两处：建库语句改为 `locale_provider libc lc_collate 'C' lc_ctype 'C'`；
`checks/12` 改为断言 ADR-0003 逐字要求的四项 `datlocprovider='c'`／`datcollate='C'`／`datctype='C'`／
`daticulocale IS NULL`。**同批删掉该检查里的 collation 版本比对**——`datcollversion` 与
`pg_database_collation_actual_version()` 在 C 排序下都不参与版本追踪、恒为 NULL，比对它们是一条恒真判据；
而「不参与版本追踪」正是选 C 的理由本身。

代价照 ADR-0003 原样记，不粉饰：中文按 UTF-8 字节序而非拼音序，需要中文阅读序的列表
**由应用层生成并持久化显式 `sort_key` 列**承担。作成方曾口头对使用方说「需要拼音序的地方写
`COLLATE "zh-Hans-x-icu"`」——**该说法错**：首版不要求 PostgreSQL 构建带 ICU，那个写法不保证可用，
且不是本语料定的方案。已当场更正，记此。

#### 结论二　~~`max_slot_wal_keep_size` 从 `'350GB'` 改回 `'40GB'`~~ **该改动已由 F-72 当日撤回；本节保留为过程记录，结论以 F-72 为准**

本卷此前把这条判断成「两个冻结数字互斥、需二选一」，**其实不需要判断**：
`02_cluster_params.sql` 就在那行值的上一行逐字写着
「复制槽的本机事务日志保留上限：**等于附录 A.3 连续归档本机保留子项，不得高于**」，
而 A.3 的 ④a 连续归档本机保留是 **40 GiB**。脚本违反的是它自己上一行写的规则。
按 PostgreSQL 单位约定 `GB = 2^30`，`'40GB'` 即 40 GiB，与 ④a 相等。

**F-72 撤回本条**：上述推理只读了脚本上一行的绑定，漏了另一条同样现行的绑定——`docs/config-reference.md` 已删除键一节逐字「`archive.max_slot_wal_keep_gb`：由 `archive.wal_spool_max_gb` 取代；**同值由部署自检与 PostgreSQL 参数读回保证**」，而 `archive.wal_spool_max_gb` 现行取值是 **350**。单方面改一处会让部署自检在安装时就判不符。值已恢复 `'350GB'`，冲突改为就地写明并交使用方裁定，详见 F-72 结论一。

**同时更正 F-70 结论二两处说重的话**：

1. 本段原算术只把 replication slot 的 350 GiB 替换进旧表，漏掉 archive staging 自己的 350 GiB，故「剩 250 GiB」结论由 F-86 撤回。
2. 当前完整算法为 `150+60+40+350+350+350+24+8+48=1380 GiB`，再加 100 GiB 不可借底线得 1480 GiB；930 GiB 单盘从初始包络即短缺至少 550 GiB，不存在等到第 5 年才翻转的安全窗口。三条故障预算必须分别取证，不能因同值、同目录或常见时序而互借。

#### 结论三　`archive_mode=on` 的失败路径已在冻结要求处登记，阶段 14 的「净收益」结论已标失效

**先更正 F-70 结论四的一处事实错误**：F-70 称「全仓无 `archive_command`」——**不实**。
`archive_command` 在冻结 `critical_setting_rows` 闭集内，且收敛主计划逐字含
「`archive_mode=on`, **the signed archive executor**」，执行器是计划里有的。

站得住的是另一件：阶段 14 逐字写过「该改动对单机形态是**净收益**：`pg_wal` 不再因落点不可写而增长，
数据库因复制槽滞留失去写入能力这条路径被移除」——**`archive_mode=on` 把这条路径装了回来**，
因为该模式下 WAL 段在归档成功之前不被回收，而 `max_slot_wal_keep_size` 只约束复制槽保留、
不约束归档失败导致的滞留。仓内现有两条堆积用例（本机暂存触界、`pg_receivewal` 停止）
**都不覆盖「归档执行器持续失败」**。

已落两处：阶段 14 那句「净收益」结论就地标注在 F-57 下不再成立；
收敛主计划的冻结要求处登记该义务——执行器的持续失败路径必须显式有界并由一条注入用例证明，
并写明 `max_slot_wal_keep_size` 不是它的界。**本卷不代选界值、不代指派承接任务**，
那属该义务的属主。

#### 本轮未做到的

- 界值本身与承接任务未定，见结论三。
- `shared_buffers` 仍无取值，绑硬件实测，本卷不代算（F-70 结论一未变）。
- 三处改动都在 plan 阶段的文档与引导脚本层，**未在真实 PostgreSQL 上执行过**——
  本机无 Windows/PG16 环境，`sqlcheck`／`archcheck` 与两道 CI 自检通过只证明形态合规，不证明可执行。

### F-70　运行可行性实算：按 plan 阶段重筛后的五条真问题

使用方问「有什么点没有注意到，会导致整个系统无法运行」，随后逐字澄清「当前还是 plan 阶段，没有继续开发」。
四路实算出 32 条，**该澄清作废其中大半**：缺 `.ps1`、缺 `ServiceMain`／`JobObject`、代码里全是 POSIX 路径、
无 `data_root`、无 Windows 密钥保护——实测 F-57 五份计划对 Windows service／SCM／installer／DPAPI／CNG／
named pipe／data_root／Job Object **全部有任务承接**（分别命中 3／3／6／2／5／4／5／3 份），
故这些是「还没写」不是「写错了」，一律不作发现。

**留下的只有一类：已冻结、且互相矛盾或与硬件对不上的数字与决定。**
这类不会被实现修好——实现会忠实地把矛盾建出来，且其中两条建出来之后只能重建数据库才能改。

#### 一　32 GB 的内存预算**不存在**，而禁用它的话已经写下了

P340 档案逐字「| RAM | 32GB | 需要有界连接、worker 和缓存；**不得按旧 64GB 预算** |」——
**禁用了旧值，却没有给替代值**。仓内唯一一套绝对字节在被隔离的 Linux slice 里，
合计 **54.71 GiB**（按 64 GiB 算定），是物理内存的 **1.71 倍**。
指名承载替代值的 `deploy/resource-limits.toml` 不存在。

更要紧的是同档案要求「pagefile 移至 HDD 或禁用，**并验证 32GB 内存档下不会因禁用导致不稳定**」——
**这条验证的输入不存在**：没有内存预算就没有可验证的对象。按本仓自己的三类缺陷分类，
这是「取不到的取值」，且它守的是一条禁用分页文件后 32 GiB 变成硬顶的路径。

#### 二　两个冻结数字差 **8.15 倍**，而它们守的是同一条路径

`db/bootstrap/02_cluster_params.sql` 落 `max_slot_wal_keep_size = '350GB'`；PostgreSQL 16 明确其内存/容量单位按 1024 进位，所以该值恰为 **350 GiB**，不是 325.96 GiB。
旧容量表 `④a` 只记 40 GiB，且把 archive staging 与 slot-retained `pg_wal` 合成同一用途；F-86 已将二者拆为各 350 GiB 的独立故障包络。

该参数只限制复制槽导致的 `pg_wal` 保留；archive command 失败和 archive staging 堆积不受它限制。
按 350GB 实现，slot 路径本身要有 350 GiB；另一个 archive 路径也必须按自己的 350 GiB 上限独立记账。
**真走到那条路径，终点不是「槽失效、实例继续可写」，而是 `pg_wal` 涨到盘满、单实例 PANIC 停写。**
且落点不可写正是规格列为必测的常规场景，而使用方当前无法增盘。

#### 三　建库参数三处互斥，而它**只能重建库才能改**

- ADR-0003（已接受）逐字：`LOCALE_PROVIDER libc`、`LC_COLLATE 'C'`，并明写落地脚本应写成 `LOCALE_PROVIDER libc`；
- `db/bootstrap/00_database.sql:24` 实际写：`locale_provider icu icu_locale %L`；
- `db/checks/12_collation_conformance.sql:4` 断言：`datlocprovider 为 i、daticulocale 为 zh-Hans-CN`。

ADR-0003 自己写明该参数「此后只能重建库才能更改」。这是三处已落盘的互斥，不是未实现。
**本卷记一处自省**：作成方最初据 ADR-0003 单独判定「collation 处理对了」并如此告知使用方，
是只读了规则、没读承载规则的脚本——与本仓头号复发病同源。

#### 四　`archive_mode=on` 把阶段 14 刻意拆掉的无界 `pg_wal` 路径装了回来

阶段 14 的设计是 `pg_receivewal` 加复制槽，并论证过把背压从 `pg_wal` 挪到本机暂存是净收益，
理由逐字是「`pg_wal` 不再因落点不可写而增长」。F-57 的冻结 GUC 向量另外要求 `archive_mode=on`，
而全仓无 `archive_command`、无归档积压上限、无该路径的混沌用例。
`max_slot_wal_keep_size` 只约束复制槽保留，不约束归档失败导致的 WAL 滞留——两者不是一回事。

#### 五　`wal_sync_method` 停在 `EP-PENDING`，冻结要求是 `fsync_writethrough`

**现行状态（F-69 后续收束）：已关闭。** 本节以下文字只保存发现当时的历史证据；现行
`db/bootstrap/02_cluster_params.sql` 已显式设置 `wal_sync_method='fsync_writethrough'`、
`effective_io_concurrency=0` 与 `huge_pages=off`，并由 `xtask` 单元测试对去注释后的可执行 SQL
作精确断言。不得再把下述 `EP-PENDING` 描述解释为当前实施指令。

`02_cluster_params.sql` 只有注释与 `EP-PENDING` 标记，从未 `alter system set`。
Windows PG16 默认落 `open_datasync`，**脚本自己写明该默认值配合磁盘写缓存不安全**。
本条已被诚实标记为 pending，故不升格；登记于此是因为它与第一条同属「实现时没有数可用」，
且它的缺省后果落在数据面。

#### 本轮的判定边界

- 上述五条**都不阻塞 G0 开工**：它们是计划面的账，G0 Task 1 不碰其中任何一处。
- 第一条属**缺数**，须由使用方或一次实测给出 32 GiB 档的预算；本卷不代算。
- 第二、三、四条属**已冻结值之间的互斥**，可在计划面直接消解，不需要等实现。
- 「Windows 侧一件可执行物都没有」经复核**不是发现**，计划已逐项承接。

### F-69　使用方两条裁定落地：CI 单一取 GitHub；两组不变量按「加一行」关闭

使用方逐字「1 只在 github」「2. 选最好的」。两条都已落地，各立一篇 ADR，并把传导面逐处改完。

#### 结论一　CI 平台收敛为 GitHub Actions 单一平台（ADR-0027）

F-67 登记的那条**自始无决策归属与期限**的开放项就此关闭。要点：

- **窄取代 ADR-0005 决定一**（内网自建 Forgejo 加 Woodpecker）。ADR-0005 的薄适配器、
  Rust 唯一判定、Authenticode 生产门禁三条**继续有效，未触碰**；ADR-0022 的既有取代范围不变。
- **执行器一律自托管**，这是保住 ADR-0005 原始理由的要害：编排面在 GitHub，
  而**离线依赖仓库、制品与签名材料全部留在自有 runner 上，不出网**；托管 runner 明文禁用。
- **一处不一致如实记，不掩饰**：ADR-0005 原文的保护面逐字含「源码」，
  而源码事实上已在 GitHub（`origin` 指向 GitHub 远端）。ADR-0027 把保护面收缩为
  **离线依赖、制品、签名材料**三项并写明该差异，而不是假装它不存在。
  若日后要求源码也不出网，须另立取代 ADR。
- `ci.yml` 头部的「待裁定」注记同批撤销——**它是活的指令，必须改**；
  其余提及 Woodpecker 的六处属历史叙述，按行号勘误通则不逐处回改。
- **未解决且如实说**：自托管 runner 的存在性仓内仍查不到任何痕迹。本篇选平台，不解决执行器是否就位。

#### 结论二　两组不变量的挂载点：取「给 `FeatureOwnerIdV1` 加一行」，17 → 18

ADR-0026 第四条的 `UNRESOLVED` 关闭。新增行：
...
1. **它是与矩阵对齐，不是发明新概念。** 追踪矩阵 `:174` 早已把 `INV-007` 的 CapabilityOwner
   逐字定为 `costing`，`:423` 的来源绑定逐字为 `[costing; inventory]`——
   **矩阵这一层一直认为存在一个 costing 承接方，缺的只是 §2.1 登记表里没有对应行**。
   加行是让两份现行权威对上；另一案要推翻矩阵。
2. **并入既有 owner 会撑破一条逐字只到数量的 scope。** `inventory-fulfilment` 的 authoritative scope
   逐字是 `receipt/delivery/return and quantity/batch/serial facts`——只有数量、批次、序列，没有金额。
   金额与数量生命周期不同：一次重估改金额不改数量，一次价差拆分只动金额。把两族事实塞进一个写者，
   正是 §2.1 逐字「Sharing a physical schema never permits sharing a fact or repository writer」要拦的事。
3. **一 schema 两 owner 是本表既定模式。** `payable-cash`／`receivable-cash` 共用 `finance`、
   两个 portal 共用 `portal`、两个 invoicing 共用 `invoice`。新行沿用 `inventory` schema 与
   `inventory-fulfilment` 共存，事实与写者各自独立。

**传导面九处已逐处改并反查**：收敛主计划 §2.1 表体与 `:78` 计数、
G0 计划 `:56`／`:141`／`:261` 断言／`:512` 断言／`:691` 注释／`:917` 计数、权威登记 `:53`。
**其中 `:141` 那处险些漏掉**——它把计数藏在一句长复合句的中段，前一轮 grep 的模式没兜住，
是补一次更宽的反查才捞出来的。这正是本仓头号复发病「半改」的典型现场：
**闭集计数散落在多处不同措辞里，改一处不改其余，机器不会当场报错。**

#### 本轮未做到的

- `crates/features/inventory-costing` 与 `inventory::costing_repository` **尚不存在**，属 G0 之后的实现内容；
  本卷只改登记与计划，不建 crate。
- 自托管 runner 仍是外部前置，仓内查不到痕迹。

### F-68　「把能解决的全部解决」：一件工具、一篇取代 ADR、九处判定件修复、十处登记

使用方逐字「将你能解决的全部解决」。本卷把 F-67 留下的四条未做逐条处置，并落一件新工具。
四路调查（五红判真、ADR 起草、词表普查、覆盖洞补扫）出 47 条，处置如下。

#### 结论一　五条失败测试逐条判真：甲三条不许碰，乙两条已修，失败数 5 → 3

独立一路的判定与本卷各自做出、结论相同：

- **甲（诚实的红）三条**——`codecheck::the_repository_itself_passes`、`configdoc::the_repository_passes`、
  `eventcatalog::the_repository_is_clean_with_registered_events`。三条都直接断言真实仓库
  `problems.is_empty()`，**只能靠删除或放松才过**，故一字不动，等门禁真绿。
- **乙（陈旧的测试）两条**——两条单据类型码用例都栽在同一个陈旧前提
  `assert!(dict_type_codes(&doc).is_empty(), "阶段 1 登记条数为 0")`：该表**已由 0 行变成 43 行**
  （测试写于表空的时期，43 行落于其后）。按 `configdoc.rs` 自陈的谓词「该表出现第一行即自动生效」，
  比对**早已激活**，现行真值是 `uncovered=1 / Outcome::Uncovered`，而非「推迟」。
  已改断言为现行真值，并**新增一条测试保住推迟分支的覆盖**（用合成空表走到该分支），
  以免改断言等于丢覆盖。**这条本身也返工过一次**：我最初写的版本只断言「合成空表取到 0 行」，那只证明了触发条件成立、没证明分支被走到——**一条永远不会因为分支坏掉而失败的测试就是恒真判据**，自查时抓出并改成在临时根目录里真调 `run_doc_type_codes` 并断言推迟段确实落下一条。**修后更紧不更松**：断言由 `Clean` 改成 `Uncovered`，
  `--check-doc-type-codes` 退出码仍是 3，本门禁不因此变绿。

因此 `--no-fail-fast` 真全量由 **1107/5/5** 变为 **1110/3/5**。这是**收窄**，已按新基线表的纪律
更新 `cargo-test` 行并在此留证。同时更正 F-62 补记的一处归类：它把这两条与 `the_repository` 一族
同列为「等转绿」，实测**它们不是等实现，是断言过时**。

#### 结论二　新造一件对照工具，把「登记基线红」与「新回归」分开——且它不是门禁

登记红存在期间流水线整体报红，开发者分不清哪一处红是自己造成的。本卷落
`.github/ci/known-red-baseline.tsv` 与 `.github/ci/compare-red-baseline.sh`：逐面实测、逐面对照，
只回答「比基线多出来的红有没有」。**它不在 `pipeline-stages.tsv` 里，不进 `ci.yml`，不使任何门禁变绿**；
退出码沿用既有那一套（0／2／3／64），**跳过即未覆盖**。

三件事值得记：

1. **基线取实测，不转抄裁定。** F-58 登记的 `errorcodes` 13 处、`configdoc` 312 处，
   今天实测是 **12 与 313**（F-63 修掉一条取值冲突；F-62 增段）。裁定卷里的数会漂，实测不会。
2. **首版注的归因就是错的，一小时内被抓。** 我写「配置键、指标名与单据类型码三段」，
   实测 313 的构成是配置键两向 252＋已删除键 3＋指标名 58＋路由能力元组 0，
   **单据类型码段走另一入口、对该数贡献为 0**。已按实测口径改写。
3. **工具自身两处缺陷，一处当场发作、一处潜伏。** 写完首跑就踩了
   `pipefail` 下 `grep -q` 提前退出给上游 SIGPIPE、整条管道返回 141 的陷阱——
   判定被误报成「读不到」。**它失败在安全方向**（报未覆盖而非蒙成一致），但仍是缺陷，已修。
   潜伏那处是拿**非序数的退出码作大小比较**：`-gt` 抓得到 1→3，抓不到 3→1
   （未覆盖恶化成确凿不符），已改 `-ne`。
   负样例 N11–N15 与对照组 P4 已并入既有套件，**证明它的 0 不是恒真、非零也不是恒非零**。

#### 结论三　九处判定件缺陷已修——其中三处是「错了不会当场报错」

按严重度排：

1. **`verify-pipeline-commands.sh` 真跑 xtask 时把参数丢了。** 登记表里
   `e2e --profile=t0` 实际跑成 `cargo xtask e2e`，那是一次**缺必填选项的用法错误（退出码 2）**，
   而判定只看是不是 70，于是**每次 CI 都在静默放行它**。实测：带参 exit 0、丢参 exit 2。已修。
2. **`run-pipeline.sh` 无条件打印「结论：11 个阶段全绿。」**——只跑一个阶段时同样这么说，
   而负样例对照组 P1 正是用单行登记表跑它并断言 0，**等于每轮都在为这句假话背书**。已改为按实测条数。
3. **`verify-connection-budget.sh` 的头注宣称与规格逐项比对，而比对两侧都是本文件里的字面量**
   ——恒真判据。本卷取「改口径不改行为」：头注更正为「本表是规格第 7.7 章的冻结转抄」，
   外部化留给 G0；其非数字分支同批标注为「外部化之前恒不可达」。
4. `scripts/dev-up.sh` **首次运行必然以未文档化的 141 中止**（`tr | head -c` 的 SIGPIPE，
   与本卷自造工具踩的是同一个陷阱）。已改为先取够再截，实测 rc=0、长度 32。
5. `run-pipeline.sh` 登记表失真时**同一行被计两次**（未交付 1 ＋ 不符 1），已在改判时退回前一次计数。
6. `run-pipeline.sh` 头注称 `cargo xtask ci` **「目前不被受理」且「一旦交付本脚本连同登记表一并作废」**
   ——二者都已被现实取代（ci 早已受理，且 ci.yml 称二者互为备用）。已改写。
7. `verify-release.sh` 的「哈希不是小写十六进制」**只校验首字符**（`[0-9a-f]*`），已改取反匹配全串。
8. `verify-release.sh` 在缺 SHA 命令时**照常印出一行空指纹**，而带外核对正是靠这行文字承载
   ——把「未做出」显示成「已给出」。已改为记未覆盖。
9. `verify-resource-limits.sh` 的权重合计判据在读不到文件时**静默整体放弃且不记未覆盖**，已改为先记再返回。

**一处证实了却按文档先行撤回**：`configdoc` 的「已删除键不得再引入」判据只取每条 bullet 的
第一个反引号词元，三条多键 bullet 的其余 8 个键从未被它读到——**读取缺陷属实**。
但改后实测是**重复计数而非新发现**（这 8 个键已被「代码里有、文档没登记」那一向报过），
且 `config-reference.md` 第 6 节逐字写明「『已随裁定删除』指**文档登记面**，代码侧的移除随同批实现裁定」，
照该判据的措辞报成「不得再引入」与该逐字相抵。**已撤回行为改动，就地把缺陷与正确读法写进代码注释**，
待先定这批键在代码侧的去留口径，再同批改读取规则与措辞。

#### 结论四　ADR-0026 落地：五组不变量的义务保留、承接换世界

F-67 登记的取代 ADR 已落 `docs/adr/ADR-0026`，窄取代 ADR-0006 第 17—27 行的整张挂载表。
两列都已失效且成因不同：**承接阶段**列指向永不退出的旧十四阶段；**挂载点 crate** 列指向的四个
`ep-domain-*` 正是 ADR-0025 逐字「不得新增业务规则」的 facade——那不是一段作废的历史文字，
**而是一份照着执行就会违反上位 ADR 的现行指令**。新承接改挂 requirement→owner-task 映射，
其 seed 引用经**逐字节复核全对**（行号 48/53/58/64、`F57-20`、TestID 全中）。

**两处诚实**：其一，移动加权平均单价重算与价差拆分两组**在现行权威中取不到挂载点**
（当时 17 行 `FeatureOwnerIdV1` 闭集里没有 `costing`，也没有任何一行声明库存金额／成本层／价差事实；**该状态已由 F-69 增行关闭，现为 18 行**），
已固定为 `UNRESOLVED` 并交回使用方二选一，**不就近安置**——就近安置正好制造第三类缺陷。
其二，起草方自报了三处缺逐字支撑的判断（核销守恒的挂载点一分为二、五个相邻证据行的归并、
SRV-006 的排除禁令），**已在正文逐处标注为「本篇判断，不是引用」**，不冒充引用。

#### 结论五　十处登记（含三处根治性的）

1. **词表连补两码**：`SUPERSEDED_FOR_IMPLEMENTATION`、`HISTORICAL_LINUX_RESEARCH_ONLY`。
   更要紧的是**根治**：F-66／F-67／F-68 连续三轮各被动发现一个缺码，缺的是**判定边界**而非勤勉——
   已在登记 `:34` 立**显式排除清单**（旗标块键名／开发状态字段取值／闭集枚举成员一律不属该表）。
2. **一个恒不发生的取值**：`GENERATED_PROJECTION` 全仓 0 持有者，且其唯一外部引用 ADR-0025 第 5 条
   与登记的 `CURRENT_SUBJECT_IMPORT` 行相抵（后者与 G0 计划都规定 seed 往返后转
   `HISTORICAL_IMPORT_SNAPSHOT`）。**seed 是导入前像，不是机器投影**，ADR-0025 侧已更正，两处成对做。
3. **一个 0 持有者但合法**：`HISTORICAL_IMPORT_SNAPSHOT` 是 G0 后的转入目标，已加注，
   免得下一轮普查把它误判成恒假。
4. `config-reference.md` 横幅双值（自称 `REGISTRY_PENDING_REBASELINE`、登记给 `CURRENT_SUBJECT_INPUT`）已消歧：
   **文件分类**与**内容再基线状态**是两根轴。
5. 收敛主计划 Global Constraints 那条**现在时断言**被它点名的文件当场证伪
   （称 `ci.yml` 有 `windows-f57-release-precommit` job，实为一个 `pipeline` job 的 D-07 Linux 流水线），
   已加 `APPROVED_TARGET` 限定。
6. `--check-doc-type-codes` **当时从不在 CI 中执行**：`pipeline-stages.tsv` 只跑不带旗标的入口，
   而退出条件 23 点名的正是这个旗标入口。**该历史登记缺口已由 F-86 关闭**：完整 argv 独立行已进阶段 6；验证器只做可用性与登记一致性预检，后续完整 `cargo xtask ci` 负责逐行执行及参数、退出结果、交付状态证据。
7. `xtask ci` 与 `run-pipeline.sh` **当时并不对等**：实测同一份登记表对如实未交付行得到两个聚合结论。
   **该历史缺口已由 F-86 关闭**：两入口统一为「不符 1 > 不可判定 3 > 未交付 70 > 通过 0」。
8. `configdoc.rs:1` 自陈「共三段判据」而正文四段（00c 早已实测、代码侧一直未改），已改。
9. **本卷自己制造的一处半改已修**：F-67 改了 `data-dictionary:137` 的前半句，
   **末句仍停在「阶段 1 只判本节存在」的旧态，与前半句自相矛盾**；`:131` 的「阶段 3a」同批更正。
   半改仍是本仓的头号复发病，这次是我犯的。
10. 非 Rust 面的**真实清单**：首方 `.ps1` 为 **0**（唯一一个在 `.venv-docx` 里，属 pip vendor），
    受版本控制的是 **11 个 `.sh`**（含本卷新加的 `compare-red-baseline.sh`；扫描时为 10，此数随本批变动，已重算）加 **6 个首方 `.py`**——后者**本轮未扫**，其中
    `verify-orchestration-equivalence.py` 承载 13 条规则。不得因 `.sh` 已扫就声称非 Rust 面闭合。

#### 结论六　六个首方 `.py` 首次被扫（2109 行全读、实跑变异验证）

F-67 与本卷前半都把这块登记为未覆盖，本轮补上。**三个编排脚本带
`HISTORICAL_LINUX_RESEARCH_ONLY` 横幅、在隔离区内，故只登记不修**——修隔离区的代码本身就是越界：

- **最重一条（隔离区内）**：`ep_orchestration_facts.py` 的 `REQUIRED_FACTS` 只列 6 种事实，
  而两套编排共有 16 种。**两侧同时删掉 `ReadOnly`／`cap_drop`／`security-opt`／整个健康检查，
  判定输出「不符 0 项，未覆盖 0 项，判定：通过」、退出 0**——经实跑变异逐项证实。
  该表的存在理由正是防「两边同时漏写」，**它偏偏在最要紧的加固项上不设防**。
- 隔离区内另两条：`.slice` 文件不可读时 `OSError` 未被接住 → 退出码 1（不在其自声明的四码集合内）；
  非 UTF-8 字节触发的 `UnicodeDecodeError` 是 `ValueError` 子类、绕过 `except OSError` → 同样退出 1。
  **与本卷在 shell 侧发现的 141 属同一类**：未文档化的退出码。

**两个无横幅、是活工具的脚本已修**：`mark_original_requirements_snapshot.py`
文档字符串自称幂等，实际两条代码路径给标题**两种颜色**（插入路径琥珀＋底纹、更新路径蓝色无底纹），
同一份文档跑两次得到两种样子；且 `props.modified = datetime.now()` 加 `sanitize` 不带包时间戳，
而 `OUTPUT` 默认就是 `INPUT`——**每次运行都改写一个受版本控制的 docx，产生永久性伪差异**。
已把标题样式收进单一函数、并照兄弟脚本 `build_product_intro.py` 的既有做法冻结时间戳；
**实测两次运行逐字节相同**（改前差在第 11 字节）。

**如实报告扫描方自己的一次翻车**：它第一次的加固测试因 shell 函数没转发 `$@` 而静默空跑，
返回了一个虚假的「通过」，被它自己抓出并重做——上述结论出自重做后的验证。

#### 本轮未做到的

- **ADR-0005 平台冲突仍未裁**（Woodpecker vs GitHub Actions）：决策归属与期限自始为空，**属使用方**。
- **两组不变量的挂载点 `UNRESOLVED`**：须使用方二选一（给 `FeatureOwnerIdV1` 加一行，
  或把该事实范围并入某个既有 owner 的 authoritative scope）。
- 收敛主计划 §3 的 9880 行 Rust 块与 1135 行散文，**本轮未逐句**——只读到第 10087 行为止，如实记，不声称已扫完。（六个首方 `.py` 已于结论六补扫完毕。）
- **提交前对抗验证抓出本卷六处账目错，已全部修正**：五处 `file:line` 引用错——**其中四处是本批自己造成的**（改动了被引文件却没重算行号），另有 `:102` 一处**逐字引错了词**（把「generated error catalog/OpenAPI」引成「generated error registry」）；`ci-pipeline.md` 的基线数字停在改前的 5；「三处自称对等」实为两处；ADR-0026 自己开的三项同批义务只做了一项；一条断言空洞（只证明 `String::replace` 生效）。**其中两处正是本卷宣称的头号复发病「半改」，犯在宣称它的同一批里。**机械成因值得单记：**凡本批改动过的文件，其行号引用必须在改完之后重算一遍**——本卷已把这条列为下一轮的自检项。
- 「39／20」两数**正式退役**：其承载产物已被轮转清理，**独立复核做不出且以后也做不出**。
  自此**不得再作为证据被引用**，只作 F-65 的自述留档。这是对 F-66 所立纪律的落地执行。

### F-67　终扫 23 条：七处直修、五处加注、五条登记；「还剩什么」自此有一张如实的表

使用方问「还有什么问题吗」。四路全新猎扫（不看近期改动、专找未登记项）出 **23 条**：
开工后会咬 4、登记欠账 9、仅记录 5、**好评 5**。本卷处置如下。

#### 结论一　四条「开工后会咬」，三条当轮修死、一条已由本卷同批先修

1. **`g0:271` 测试字面量仍断言 30 行**——与上一行「等于主卷 §2.2 集合」（35 行）互斥，
   照字面 Task 1 的 GREEN 永远到不了。**F-62 的「六处全改」实为第七处漏网**，已改 35 并留注。
2. **`principal.rs` 被列为 `Create` 而它已存在**，承载 A-02 冻结的 `SYSTEM_PRINCIPAL_ID`、
   被三处在用代码依赖——照字面新建当场破坏全工作区编译。g0 与主卷两处均改为
   「**Modify／extends existing，保留既有内容**」。
3. **PRD 自排了一套四级权威顺序**，违反最高权威逐字禁令「**不得复制、缩写或另排一套顺序**」，
   且与 §1.1 相反（把 PRD 细节排在实施计划之上）、漏掉两份契约。已改为链接 §1.1，原表述存历史。
4. CI「基线红 vs 新回归」无区分机制——见结论三登记。

#### 结论二　「跨世界判据」同病的全景，本轮扫尽

F-66 修了三道门禁判据后，本轮系统性搜同类，又得七处，已分别直修或加注：
F-58 的 **configdoc 行**（第四条判据，F-66 曾称「四条全部修正」实落三条，记此差）；
`data-dictionary:137` 的「阶段 3a 生效」**与 `configdoc.rs` 已实现的「出现第一行即自动生效」谓词相反**
（本卷唯一一处**代码先于文档**完成跨世界修正的实例，文档已对齐）；
`metrics-catalog` 的「阶段 13c 十五项」分派（被测对象是被 ADR-0019 撤销的进程）与 **pool 行引已撤销的
ADR-0018 冻结为现行权威**；`impact-catalog` 的阶段退出点／`ep-app-*` 属主／固定岗位三列；
**ADR-0006** 的阶段挂载表（义务现行、挂载失效，实质冲突须另立取代 ADR，登记待 G0）。
**好评侧同录**：F-57 现行规范层自身（五计划＋四规格＋三评审＋ADR-0019…0025）经同法扫描**零同病**。

#### 结论三　五条登记（不代裁、不越权造机制）

1. **CI 无「基线红 vs 新回归」区分**：聚合取最重，登记红存在期间每次推送整体红，
   开发者分不清新旧。已登记不符计数唯一落点是 F-58 散文表。**过渡纪律**：开发者以 F-58 表
   （codecheck 2／errorcodes 12／configdoc 313／eventcatalog 117／测试失败 5）为基线人工对照，
   **计数增长即新回归**；机制化归 G0 的 f57check 收据体系，本卷不自造闸。
2. **self-hosted runner 存在性仓内查不出**（无任何 runner 预备痕迹），CI 实际执行性待环境证实；
   本地 `xtask` 全套是现行有效判定面。
3. **ADR-0005（Woodpecker）vs GitHub Actions 冲突**：原登记于 ci-pipeline 旧 §7，
   **该节已被 F-50…F-57 重写删除、ci.yml 头注指针悬空**——指针已修，两条可选处置
   （迁 `.woodpecker/` 或修订 ADR-0005）迁记于此；**该待裁定自始无决策归属与期限，仍开放**。
4. 实现状态词两套并行（`IMPLEMENTATION_NOT_STARTED` vs `NOT_IMPLEMENTED`）与
   `DEVELOPMENT_AUTHORIZATION_GRANTED` 新 token 同筐：**词表归一归 G0**。
5. `cpq`/`mdm` 分册「以后每次结构变更」的对照物与失败时点仍锚旧阶段——两册已有历史状态注，
   替换状态说明时**须连这两句一并改**，记此防漏。
   另：g0 Step 2 的 delivery-registry RED 命令在包未建时报「package not found」而非测试失败，
   RED 语义以 Step 1 先建 crate 骨架为前提，仅记录。

#### 结论四　登记表与 00c 自身的两处收口

登记表「唯一状态词表」**原缺的在用状态码不止两码**——又补 `HISTORICAL_NON_NORMATIVE`
（挂在 10+ 份早期文件上），并更正 F-65 时「本表原缺后两码」的完备性表述；
**00c 自述「两类内容」改「三类」**——F-58 起的现行裁定卷（含 F-65 授权留证）是第三类，
**不随首行 SUPERSEDED 横幅失效**，全仓「留证 00c F-65」指的正是它。

#### 本轮未做到的

- ADR-0006 的**取代 ADR 未立**（登记待 G0，register :89 规则）。
- CI 基线机制**未造**（过渡纪律代替，归 G0 收据体系）。
- ADR-0005 平台冲突**未裁**（决策归属与期限仍空，属使用方）。
- 猎扫四路自报未覆盖面照旧（`.ps1`／TS／Dart、主卷 §3 大部未逐句）。

### F-66　对 F-65 批次的二次对抗验证：修复全部落地、命题判真；F-65 自述六处账目错已更正

使用方问「当前是即刻开发状态吗；检查近期改动是否正确」。本卷对 F-65 提交（18 文件 +117/−33）
做四路对抗验证，**63 项判定**。两问的答案：

#### 结论一　近期改动：**20 处修正实质全部落地，未发现修复引入的语义或格式破坏**

逐项复核过的硬项：附A 11 条数据行与表头对 `a98cacc` **逐字节 diff 为空**；
阶段表自检实跑 **0** 且以 `EP_CI_DOC` 指旧版**反向复现退出码 3**；负样例 harness **0**；
`config-reference:716` 三条禁令齐；`expansion:76` 与 `:747` 的 `Cargo.lock` 行 **od 逐字节同形**；
六道门禁实跑**恰四红两绿与 F-58 登记一致**；两个冻结 SHA-256 重算相符。

#### 结论二　命题「可即刻开发」：**按仓库自身定义为真**

一致性路的原文结论：「**未发现任何越权解锁或实质放宽；『可即刻开发』（＝G0 可启动）按仓库自身定义为真**」。
三处状态字段块逐项一致；全仓七类「未授权／不可开发」断言模式扫描无残留现行断言；
G0 启动条件逐条实测在位（clean HEAD、种子在位、185 行注册表）。
G0 **转绿**另差已登记前置（如证据信任密钥仪式）——那属 G0 执行内容，不挡启动。

#### 结论三　被抓出的六处全在 F-65 自己的叙述里，已就地更正

| # | 错 | 更正 |
|---|---|---|
| 1 | 「全仓**十九处**」与自身括号枚举（合计 27）自相矛盾 | 写明两个口径：改前树 grep 得 **19 行**断言，落成 **27 处**清扫位置 |
| 2 | 「本卷**第一次**把对抗验证放在提交之前」——被 F-64 结论二自己的原文「拦在落盘后、提交前」推翻 | 改「自 F-64 始，本批第二次且规模更大」 |
| 3 | 「F-58 那批被抓 27 处」——27 是 F-62 对 **F-58…F-61 四批合计** | 归属改准 |
| 4 | 引 config-reference「原句」时句中删字未标省略 | 补 〔……〕 标记 |
| 5 | ai_mcp 判据注行号被本批自身插行漂移未登记 | 照 register 先例登记 `:281→:282` |
| 6 | 「各处留『F-65 复核注』」按字面不成立（逐字仅两处，余为异名注） | 措辞改准 |

另两处同批补：`readiness:381` 启动条件 1 补「已满足」注（该文件唯一漏注处）；
`ai_mcp:7` 历史段「F-55 四张」与 `:9`「五张」并立无注——补注（F-55 时点确为四张，第五张系 F-57 再基线新增）。

#### 结论四　一处**查不出**，如实记，并立一条纪律

验证方要对表的「39 项判定、20 处非正确」两个数**无从复核**——承载它们的任务产物文件已被轮转清理，
指定路径不存在。两数**未被证伪**（F-65 与提交信息同源一致），但**独立验证做不出**。

**由此立一条纪律**：临时任务产物不是档案。**凡裁定要引用的验证结果，其可复核的实质
（判定清单或至少逐条要点）必须落进裁定正文或仓内文件，不得只留 /tmp 路径指针。**
本教训与 F-65 指针悬空同类——那次是引了不存在的节，这次是引了会消失的文件。

#### 补记（同日，使用方再问「可即刻开发吗，你想想」引出）

重新推敲时发现一处**此前两轮判真都没看见的**：F-58 给 `codecheck` 的转绿判据写「阶段 13c 落 `apps/ai-inferer`」，而 ADR-0019 已撤销 `ai-inferer` 的强制性——**判据指向一个被上位设计取消的义务，照它等永远等不到**。已就地更正。成因：F-58 写判据时按旧十四阶段口径取承接方，F-57 收敛后未复核——**转绿判据的归属跨在新旧两个世界**的实例。

#### 本轮未做到的

- 「39／20」两数维持**存疑**（见结论四），不改写。
- 全量测试 1107/5/5 本轮未重跑（验证方声明成本考虑；六道门禁与两道 CI 自检已实跑）。
- Windows 侧门禁在 macOS 无法实测，照旧属发布面外部前置。

### F-65　开发授权授予与「完全可即刻开发」批次：授权翻转、CI 自检回绿、六处遗留修复；提交前对抗验证抓出 20 处并全部修正

#### 结论一　授权动作与射程（**本节即全仓各处「留证 00c F-65」指针的落点**）

使用方于 **2026-08-27** 以逐字指令授予开发授权：

> **「需要 完全 可即刻开发 状态 最谨慎」**

本卷据此裁定并执行：

1. **授权表达照仓库自定义机制**——晋升状态机（converged §7）的节点名 `READY_NOT_AUTHORIZED`
   **保持不改**（依机制该名保持至 `G0_BOOTSTRAP_GREEN`）；翻转的是标志位：
   `DEVELOPMENT_AUTHORIZED=false→true`、`blocking_reason→NONE`。
2. **射程**：授权覆盖 **G0 全部任务**（含 Task 1 的 configdoc 扩展面——
   `config-reference` 原句「只有用户另行明确授权后，〔……〕Task 1 才可…」（句中省略「现行 G0 bootstrap 子计划的」，F-66 补标）的条件视为随本授权满足）；
   **不解锁任何 BLOCKED_BY 依赖门**（G1…G6 的次序锁原样），**不放宽任何门禁与 UNVERIFIED 纪律**。
3. 若本读法有误，使用方一句话即可撤销：回翻两个标志位并作废本节。

全仓现行断言点同批清扫——**改前树按七类断言模式 grep 得 19 行，落成的清扫位置为 27 处**（README、ci-pipeline 标志块、readiness 七处、converged、g0 两处、另三份子计划 Status、规格 `:11`、p340、config-reference 两个改行内三个断言、fabric-design、登记表两处、00c 四条 F-62 注）。**F-66 更正：本句原写「十九处」且与括号枚举（合计 27）自相矛盾——19 与 27 是两个口径（断言行数 vs 清扫位置数），原文混为一谈。**

#### 结论二　CI 阶段表还原：自检由退出码 3 回 0

`docs/ci-pipeline.md` 在 F-50…F-57 批次被整篇重写时删掉了阶段表，而互检真值
`.github/ci/pipeline-stages.tsv` 与脚本改动为 0——`verify-pipeline-commands.sh` 落入退出码 3
（「判定未做出，不得视为通过」）。本卷自 `a98cacc` **逐字还原 11 条数据行与表头**为附A，
写明：**附A 是当前实际执行的真值镜像；上文多 lane 形态是已批准目标（APPROVED_TARGET）、
尚未在 `.github/` 落地，其生效须与真值表、脚本同批更新（此为本注要求，非 ADR-0022 载文）**。
实跑：阶段表自检 **0**、判定件负样例 **0**。

#### 结论三　六处遗留误导修复

ai_mcp 分册「四张」改五张并补清单第五项与 `:281` 判据适用注（该注经本批自身插行现落 `:282`，F-66 照 register 先例登记漂移）（迁移落点为**推断**已明标）；
`clm_sales` 的 `NORMAL` 行按契约 `:272` 改 `STANDARD` 并留历史痕；converged §1 行 3 补
`G3_CLIENT_SHELL_GREEN`；业务契约 §8.2 的十五行断表接回一表（插段挪至表后）；
expansion Task 1 补 `- Modify: Cargo.lock`（**裸行**——Files 清单被 G0 机械展开、
unknown annotation 即拒，理由记于此不记于行内；缺此行则 `task stage` 白名单外路径 fails closed
且 Step 6 clean-HEAD 判负）；`openapi/README` 加 UNVERIFIED 状态注（照 `metrics-catalog:136` 样板）。

#### 结论四　提交前对抗验证：39 项判定、20 处非正确、全部修正后才提交

**本批把对抗验证放在提交之前**（F-66 更正两处：其一，「第一次」不实——F-64 结论二逐字已写「拦在落盘后、提交前」，提交前验证自 F-64 始，本批是第二次且规模更大；其二，「F-58 那批被抓 27 处」归属失准——27 处是 F-62 对 **F-58…F-61 四批合计**的判定数）。四路抓出的要害：

1. **「00c F-65」指针悬空**——13+ 文件引它而本节当时不存在。**本节即其落地。**
2. **一处实质放宽闸门**：`config-reference:716` 初改时误删「后续业务任务」禁令——
   **该禁令与授权无关（由 BLOCKED_BY 门锁定），已恢复并注明**。这正是第四路「越权与误读」要抓的东西。
3. README 同行自相矛盾、readiness 三处漏清、register 分类定义未注、规格 `:11` 粗体失衡、
   附A 表头非逐字与 ADR-0022 引证失实、expansion 尾注会被机械展开器拒、openapi 横幅落位错、
   ai_mcp 清单四项对五张——**逐处修正并留注**（F-66 更正措辞：逐字「F-65 复核注」仅两处，其余为「F-65 更正／补／状态注」等异名注；expansion 的 `Cargo.lock` 行按设计为裸行、无行内注，理由在结论三）。

#### 本轮未做到的

- `DEVELOPMENT_AUTHORIZATION_GRANTED=2026-08-27` 是新造 token（该标志块本就用异名字段，沿袭而非新错）——
  **词表未统一**，随 G0 生成式登记再基线时归一。
- 四道门禁红与 5 个诚实失败**照旧**（转绿判据见 F-58）；Windows runner 仍是发布门禁面的外部前置。
- 附A 是**真值镜像**不是新真值：`.github/` 与多 lane 目标的再基线归 G0/ADR-0022 执行批。

### F-64　RULING-F63-02 种子再基线落地：49 路由 117 组件，对抗验逮回一处漏改

F-63 登记待批次的种子再基线本轮执行完毕，**F-63 的两条遗留（种子行、部分收货半问）至此全部关闭**。

#### 结论一　落地内容

`f57-api-direct-routes.seed.tsv` 插入两行——
`POST /employee/v1/session/reauth`（`F57-18`／`EMPLOYEE_SESSION_DEVICE`）与
`POST /portal/v1/sessions/reauth`（`F57-22`／`PORTAL_SESSION_BINDING_CSRF`），
均镜像 CONTROL reauth 族并按目标面惯例取列值；连带 11 处计数与契约清单更新
（登记表 `:68` 47→49、111→117；两契约 16→17 对；ctc01 两处）。

**落盘后在真实文件上重跑机器闭集,全绿**：49 行＝CONTROL 12＋EMPLOYEE 17＋PORTAL 20；
排序键、双唯一键、schema 正则、严格组件并集恰 **117**、逐码对 `error-codes.md` 登记齐全。

**一处裁定级取值留证**：PORTAL 路径落 `sessions/reauth`（复数）——RULING-F63-02 括注原写单数,
但 PORTAL 面既有惯例是 `sessions/*`（`sessions/end`／`sessions/renew`），按 F-63 纪律「镜像照目标面惯例」取复数，非静默偏离。

#### 结论二　对抗验证三路：一处真漏改，两处枚举失实

- **机器闭集路 reject 逮回一处真漏改**：起草自称「`ctc01:19` 是五份现行计划中唯一硬编码 16 对之处（全五计划 grep 证实）」——
  **该断言为假**，`ctc01:222` 逐字「Consumes: exact 16-route Employee API」是第二处。已改 17 并留证。
  **同一份文件里的第二处命中，恰是本卷 16 处半改的标准形态——这次被验证层拦在落盘后、提交前。**
- **清扫核路 reject**：起草的豁免清单对 fabric-implementation（`HISTORICAL_DETAIL_INPUT`）**漏列五处、错引一处**（`:508` 不含受影响计数）。
  豁免结论本身经独立复核**为真**（该文件横幅禁止执行、现行计划零引用其计数），**无需改文**——失实的是枚举完备性断言，本节如实记。
- **镜像依据路 accept，附两条措辞修正**：「PORTAL 已认证 POST 7/7 全带」计数口径不成立（实为十行、其中三行不带幂等码），
  但真正依据（`sessions/end`／`renew` 两行 13 码族逐字节相同）成立，取值不受影响；
  「同式剥除＝两码」低估了被镜像行的实际差集（实为四码，另两码各有独立处置依据），结果值均正确。

#### 结论三　顺带修掉本卷自己一处过时行

F-63 表格第 3 行仍写「部分收货态改单**仍待裁**」——半问当日已答、边已加，该行没跟。已修。
**这是「结论变了承载文件没跟」在同一份裁定内的最小形态，与 F-62 记过的同类。**

#### 本轮未做到的

- 起草留证里那两句失实的支持性论据（7/7、两码剥除）**未逐句改写登记表正文**——
  经核落盘文本未含前者，后者影响的是解释不是取值，本节记正即止。
- 两个新端点只有种子行，**实现与用例随其属主任务（F57-18／F57-22）落**。

### F-63　使用方对十条悬项的裁定：全按推荐，八条已落、两条登记待批次

使用方对 F-62 后清点的十条悬项逐条表态「全按推荐的来」。本卷逐条执行，两条因执行面超出当轮如实登记。

#### 已落的八条

| # | 裁定 | 落点 |
|---|---|---|
| 1 | **删 g0 规则里「a probe at/after first due」拒绝项**——11 行两段式证明合法化，数据与三个冻结目标值全保 | `g0:907` 删项并留证 |
| 2 | **迁移「干活可并行、提交须按版本序」**——最小改动消掉 17 对冲突 | 主卷 §5 调度句后加 F-63 句 |
| 3 | **契约 §14.6 补两条边**：`SUPPLIER_CONFIRMED→PENDING_SUPPLIER_CONFIRM`（改单回退，PRD:1582）与 `COMPLETED→SUPPLIER_CONFIRMED`（红冲重开，07:1104）；部分收货态改单起初无逐字依据（**同日已由使用方补答并加边**，见本节末）；07 边表挂「不完全摘录」注；PRD:1567 的取代登记为 RULING-F63-01 | 契约 `:1477`、`07` 边表、登记表 |
| 4 | **上线硬门的具名值由计划补产出**：`IAAS_WINDOWS_SERVER_HDD_STRICT` 的拒绝须返回 `PROFILE_NOT_IMPLEMENTED`／`STORAGE_MEDIA_UNVERIFIED` 并由既有拒绝测试断言两码 | expansion Task 14 `:8758` |
| 6 | **UPS 生产认证收严**：只认 `SIGNED_VENDOR_ADAPTER`（与 RULING-UPS-01、threat-model §296 一致）；`WINDOWS_STANDARD_POWER_STATUS` 仅供开发与非生产 | 契约 `:251` |
| 7 | **本地 AI 的 10% 份额不重新分配、整体留作未分配余量**；`0.095` 公式休眠，13c 落地时恢复原义 | 规格配额表加注行（**本裁定即改规格授权**） |
| 8 | **`RATE_LIMITED` 取 429**：registry 503→429，不变量测试开**精确例外**（仅 `.RATE_LIMITED` 后缀允许 429）；顺带查出并删掉一处幽灵设计——`codes.rs:35` 旧注声称「429 的运行时映射在封套层」，**实测封套层无此映射、该设计从未实现**（core-server 内 429 命中 0，`identity.rs:141` 映的是指标标签） | `codes.rs` 三处，31 用例全绿 |
| 9 | **`HISTORICAL` 横幅豁免逐句更新，判别项＝「文件是否仍被现行文档引用为取值来源」**——`00b` 仍被引用故已修，`README:45`／`00-overview:62` 纯历史索引故免；F-58「仅有的两处」之误以此关闭 | 本节即登记 |
| 10 | **F-51／F-55／F-56／F-57 改规格的授权追认**：判定批准 F-57 时已整体覆盖，本节留证 | 本节即登记 |

#### 登记待批次的两条

- **5（reauth 端点）**：裁定已定——EMPLOYEE 与 PORTAL 各补一条，镜像 CONTROL 族。
  **本轮不手造种子行**：落行需新增 shape／component 编号，牵动三份封闭种子表与登记表 `:68` 的机器闭集计数
  （47→49 行），**须在 API 种子再基线批次一并执行**，已登记为 RULING-F63-02。
  （2026-08-27 注：该批次已执行——两行已落、47→49；实测组件并集只在路由表内 111→117、与 component-shapes
  零交集，「牵动三份种子表」实为路由表一份；PORTAL 行按该面 `sessions/` 复数惯例落
  `/portal/v1/sessions/reauth`。详见登记表 RULING-F63-02。）
- ~~3 的衍生半问~~ **已答（同日补裁）**：使用方裁「允许，回待供应商确认」——契约 §14.6 已补第三条边 `PARTIALLY_RECEIVED→PENDING_SUPPLIER_CONFIRM`，守卫取 07:664 既有句，已收货事实保持，重新确认后经既有边回部分收货态。**至此本表待裁项清零。**

#### 本轮顺带查出并修掉的

- 裁 8 执行中发现 `codes.rs` 注释描述的封套层映射**从未实现**——文档化了一个不存在的机制，
  恰是本卷第三类缺陷；已随裁 8 一并删正。
- 规格配额注行初插为 4 格、与表 5 格不符，**当场自查修齐**，规格破表行回 0。

#### 本轮未做到的

- 裁 5 的种子行未落（见上，待批次）。（2026-08-27 注：已随 API 种子再基线批次落地，见登记表 RULING-F63-02。）
- 裁 4 只加了计划级要求与断言指令，**测试代码本身随实现批次落**。
- 裁 2 的「提交按版本序」是约束句，**没有机检**——fresh-pg 的 gap 硬失败是它的事后判负口，事前无闸。

### F-62　对 F-58…F-61 自身改动的对抗验证：61 项判定，27 项非正确，全部修正

使用方指示「确保所有改动正确无误」。本卷对自己 F-58…F-61 的全部改动（14 文件 35 行加四段裁定正文）
做六路对抗验证：**61 项判定，正确 34、半改 16、有错 5、存疑 6——非正确的全部出在本卷自己手上。**
修正明细与逐字依据见提交 `7905671` 的提交信息与本节下述四条结论；
**该提交因锚点笔误先带走了全部修正而漏插本节正文，本节由紧随其后的提交补落，记此不讳。**

#### 结论一　16 处半改，全部补齐

共同形态：**改了点名处，同义邻处没动**——本卷一路抓他人的「半改」，这轮被抓出自己 16 处。
要害五处：规格 `:5` 标签改历史而正文仍把已判 `HISTORICAL` 的 F-50 实施计划当执行入口（已改指 F-57 主计划）；
`00c:11` 是 **F-58 声称已补「F-57 最高」而实际未落**的那一处（本轮落，并记此差）；
`01:81` 同一句内前半被本卷改九、后半仍「八个落在**七个**资源单位」（照 `00b:37`/`:273` 改九/八）；
`01:119`/`:263` 两处复述仍用已被 `14:842` 撤下的符号判据（该权威行逐字要求「两处复述须同批改」，照办）；
**§2.2 的 30→35 有六处反侧仍冻结 30**——含主卷 `10257` 同文件内一处，照字面 G0 建出的 35 行 TSV
会被 30-row 断言判非法，六处全改。另 F-51 `:342` 的 44 改 45——按其自订冲突规则，错值原本反而胜出。

#### 结论二　5 处有错，已修或已勘

最重的一处：**`codecheck.rs:3` 的判据引文被本卷改成了嵌合体**——改九之后它不匹配该句的任何版本
（现文是「Windows 服务名」，历史版本才有「systemd 单元名、cgroup slice 名」且写八）。
已改为现文逐字并注历史版本。**本卷自订「不得把改写后的话当逐字引」，这次犯在自己代码注释里。**
其余四处：F-60 甲「6 行」实为 **7** 行；F-59「23 处；F-54 有」皆不实；
`configdoc`「312」现值 **313**（F-59 自己补的五键各生成一条不符而未回改计数）；
F-58 测试数是 fail-fast 跑法，**真全量 158 套件、1107／5／5**，已加注。

#### 结论三　6 处存疑的处置

§2.2 两行 scope 系本卷自拟措辞、无逐字依据，已换有据措辞
（powershell-trust 照 g0 `:1935`「reusable Windows PowerShell trust boundary」，
ups-contract 照主卷 `:10739` 逐字面，避开与 release 证据 carrier 的术语冲撞）；
F-61「P1-2 起草被驳」更正为「未产出替换对」；F-58 的 configdoc 定性补单据类型码第三段；
F-60 三组目标值摘要原文已如实标注，不改；
行号漂移立勘误规则（见前注）：**裁定内行号以作成时点为准，查询以逐字为键，漂移不逐处回改。**

#### 本轮未做到的

- F-58「仅有的两处逐层排序条款」原话仍错（`README:45` 与 `00-overview:62` 同类且止于 F-56），
  但两文件整卷挂 `HISTORICAL` 横幅，与同挂横幅却已修的 `00b` **标准不一**——横幅是否豁免逐句更新，须一并裁。
- 六路自报的未验面（`.ps1`／TS／Dart、主卷 §3 大部）依旧。
- 真全量 5 个失败仍是 the_repository 一族与单据类型码负样例，转绿判据不变（F-58 结论一）。

### F-61　更正 F-60 对甲丙的归类；剩余 14 项起草 31 条**通过 0 条**，如实记

使用方指示「剩余部分一次性解决掉」。本卷更正一处自己的归类、修两处、
并**如实记录这一轮起草全军覆没及其原因**——那些原因多数是对的。

#### 结论一　**更正 F-60 结论三对甲、丙的归类，并记本卷漏的那一步**

F-60 把甲（11 条 slice-probe）与丙（30 行注册表）登记为「**冻结值与规则互斥，本卷不代裁**」。
本轮核准后**该归类不成立**：

| 依据 | 实测 |
|---|---|
| `docs/f57-platform-mechanism-registry.v1.tsv` | **文件不存在**（G0 待建） |
| `docs/f57-slice-probe-execution.v1.tsv` | **文件不存在**（G0 待建） |
| §2.2 该段的 `sha256`／`digest`／`hash` | **命中 0** |
| `docs/f57-requirement-delivery-profile-overrides.v1.tsv` | **已存在 57 行**，第四列 `rationale` 逐字写着「G1 证机制、CTC01 证真实处理器」一类两段式证明的理由 |

**即：那些「30 行」「78 行 / `2e2ba21c…`」是目标值不是已验值，改它们不作废任何已提交的字节；
而那 11 行带逐字理由，是有意设计不是录入错误。异类是规则，不是数据。**

**本卷漏了一步，记下来。** 上述核准做完之后，本卷**只在对话里改了口径，没有改裁定**——
而 F-60 的「不代裁」白纸黑字还在。随后派出的起草被对抗核当场驳回，理由逐字：
「**这条发现已被 F-60 结论三（丙）当作「本卷不代裁」登记在 open 里，
本改动是在越过裁定单方面执行其中一侧。**」

**这条驳得对。** 正确次序是**先改裁定、再动手**；本卷跳了这一步。
与 F-45 那次「只改 PRD 没查规格」同类：**结论变了，承载它的文件没跟。**

#### 结论二　据此已修两处

- **丙**：§2.2 表由「canonical-sorted **30** rows」改 **35**，
  按 canonical sort 插入 `platform.authority-kernel`、`client-common`、`delivery-registry`、
  `powershell-trust`、`ups-contract` 五行。复核：实数 **35 行**，排序位逐个核过。
- **P1-4 的最重一处**：`delivery-registry` 的两个 test target 在 G0 全程无执行路径，
  而 `g0-bootstrap:143` 逐字称它是 `MigrationClosureIdentityV1` 的「sole production-linkable owner」——
  **其黄金件在 G0 全程不会被比对一次，`G0_BOOTSTRAP_GREEN` 仍可签发。**
  已在 **RED（Step 2）与 GREEN（Step 4）两侧同批**补上。
  **上一轮起草只补 RED 被驳**，理由逐字「该计划全部 `cargo test` 严格 RED/GREEN 成对……
  **发现没有消失，只是换了地方**」——本卷照该驳回意见改。

#### 结论三　起草 31 条**通过 0 条**，但这一轮驳出了两条**深审自己的错**

**驳回理由多数成立**，其中两条**推翻了 F-60 转述的深审结论**：

1. **F-50 实施计划「17 migrations」不该改成 18。** 深审给的对侧口径 17/18/17 里，
   18 是「invoice 目录全部迁移」的口径，而该文件 `:303` 逐字自订「final **17**-file migration roster」
   且其 roster 内部自洽、全文不含那条多出来的迁移。**改 302 会与同文件 303 直接矛盾。**
2. **Task 2B 的 `f57_four_platform_contract.rs` 不属「建了不跑」。**
   深审括注称它同类，实取现文否掉：`expansion:912`（Task 3 Step 4）逐字
   `cargo test -p ep-testkit … --test f57_four_platform_contract` 是**无分支条件的下游执行路径**，
   而 2A/2B 的互斥只及于它们自身。

**记此**：F-60 转述深审时未逐条复核到这两点，本卷据本轮实测更正。

#### 结论四　起草方**自己拒绝改**的五条，理由成立，登记

| 条 | 起草方拒绝的理由（逐字要点） |
|---|---|
| **P0-2** | 「追不到依据支持『把连续性判据限定在已标 CREATED 的行之间』，**反而查到三处独立、互相一致的相反明文**，且其中一处专门为这一情形设了独立错误码」——**本卷指的方向是错的，起草方拒绝编造，对。** |
| **P1-3** | 「改硬门措辞」追不到依据且有反证；「补产出路径」是新增任务须授权 |
| **P2-3** | `REAUTH_REQUIRED` 两条修法都是安全面／API 面设计判断 |
| **P2-4** | 本地 AI 判不启动后那 10% 归谁，属资源分配的业务判断；且 `CERTIFIED_HOST_RAM_BYTES` 的唯一产出链已被 `converged-program:10673` 关闭 |
| **P2-5** | 严侧三处一致、松侧一处，异类明确；**但改松侧等于删掉 `WINDOWS_STANDARD_POWER_STATUS` 的生产路径**，改变首发对客户既有 UPS 硬件的支持面与采购前置——属范围判断 |

**这五条须使用方裁**，本卷不代拟。

#### 本轮未做到的

- **31 条起草通过 0 条**，实际只落了本卷自己按驳回意见重做的 2 处。
- **P1-2（采购订单状态机缺两条边）未落**：该组未产出任何替换对（F-62 复核更正：原写「起草被驳」，实测驳回表 23 条中无 P1-2 记录，它只以两条衍生 open 项出现），且起草方另提两个衍生问——
  部分收货订单改单后落哪个态、阶段计划边表是否须与 F-57 §14.6 逐行对齐——**两者都无逐字依据，属业务与文档层级判断**。
- **P2-1／P2-2／P2-6 其余项未落**，驳回理由各异，未逐条重做。
- 深审本身**仍是抽样**（主卷 §3 独占 94% 篇幅只深读约 1400 行，`.ps1`／TS／Dart 一概未读）。

### F-60　四万行未覆盖面深审：修两处，登记三处**冻结值与规则互斥**

F-59 点名的最大未覆盖面（五份 F-57 实施计划约 3 万行、`threat-model` 正文、
7 份 OpenAPI 约 1 万行、12 份数据字典分卷）已深审，出 **47 条**
（阻断发布 3、阻断某阶段 6、影响取值 8、仅需登记 15、**好评 15**）。

#### 结论一　这批文档的成色，先说清楚

**它是本仓迄今最能被机器复算的一批。** 审阅方实算复现了五组冻结摘要——
`docs/f57-migration-baseline.v1.tsv` 的 `52930d7a…`、legacy seed 的 `06566ca3…`、
`first_due_map_sha256`、L2／L3 两个黄金件、47 行预留表的 `98270807…`，
**连 `version` 取字符串型才命中、取整型不命中都对上了**，说明键型也被钉死。

计数交叉也闭合：47 行迁移表 ↔ 42 行 DAG（`migration_versions_json` 摊平恰 47）↔
27 行 fresh-pg 表 ↔ 22 条门面／185 符号 ↔ 89 行签名注册表，**五方无一错位**；
探针直方图实算恰 `2/6/26/5/36/3` 合 78。**附录戊设立的计数高发区，这一轮没有复发。**

另有三处主动封堵本仓第一类缺陷，其中 `docs/data-dictionary.md:135` 逐字
「**恒真的判据不作为通过判定**，因此该逐项比对整条推迟到阶段 3a 生效」——
这是把本卷的纪律用在了自己身上。

#### 结论二　已修两处（异类明确，不动任何冻结值）

**其一，`G6-14` 迁移文件名三方不一致。**

| 处 | 逐字 |
|---|---|
| 主卷 §4.1 `:11321` | `V20261025092530__platform_core_backfill_f57_unpoliced_table_registry.sql` |
| **扩展计划 `:68`／`:4815`** | `V20261025092530__platform_core_create_production_activation_and_admission.sql` |
| 不可改 seed（摘要已核） | 前者命中 **5**，后者命中 **0** |

**扩展计划是唯一的异类**，其余 46 行 version→path 与主卷逐字一致。
已把扩展计划两处改为与主卷及 seed 一致。**不改 seed**——它的摘要 `06566ca3…` 已被冻结且实算相等。

**其二，数据字典根册与分卷在建表级相反。** 根册 `:931` 自称「本节登记 F-50 的最终物理语义，**优先于**……」，
但 `:964` 逐字「关键列**全部 NOT NULL**」，而分卷 `invoice.md:52` 逐字
「两组来源 invoice/line id 是按方向条件可空，**不能声明为全部 `NOT NULL`**」，
F-50 设计 `:352` 与分卷一致。**且根册漏列两列**：
`linked_purchase_return_id`（F-50 设计 7 处、阶段 7 计划引用 5 处，根册 **0**）与 `source_effect_seq`。

**根册是异类**——一份自称优先的文件，其口径与它所依据的设计相反、还少两列。已补两列并更正口径。

#### 结论三　登记三处：**冻结值与规则互斥，两侧不可同时成立，本卷不代裁**

三处的共同形态是：**同一份文件既冻结了一组数值，又写了一条会判该数值非法的规则。**
修任一侧都会作废已冻结的摘要，**故属设计裁决不是文字修复**。

**（甲）11 条 slice-probe 被同一份 G0 的解析规则判为非法，而同批冻结的三个值又必须含它们。**
`g0-bootstrap:907` 逐字规则含「**a probe at/after first due**」即拒；
同句紧接冻结「初始哈希 `2e2ba21c…`、profile counts `2/6/26/5/36/3`」。
审阅方按 7 档 profile 序数实算：57 行共 **78** 个 (RequirementID, probe) 对，
其中 **67 个严格早于 first_due、11 个不早于**，全集中在 **7** 行（AUT-001/002/005/007、AUTH-001/004/005，计 2+1+1+1+2+2+2=11；原写 6 行，F-62 复核更正）。
**删掉这 11 个则为 67，78 与 `2/6/26/5/36/3` 与那个哈希同时作废。**
语义侧同样恒假：`converged-program:10431` 要求父需求评估返回 `NOT_COVERED` 才算过，
而 `:11446` 要求 G2/G4 必须在同一候选上重新取得 G1 收据（父需求为 PASS）——**取不到那个 `NOT_COVERED`**。

**（乙）迁移版本号给 47 个任务强加了一条全序，而 §5 DAG 明文许可的并行在 fresh-pg 下必然硬失败。**
`:11442` 逐字「only G2-01 may overlap the still-independent G1-05/G1-06 work」，
而 `:11271` 逐字要求「contiguous `CREATED` F57 prefix……**gap** 或 count mismatch 是 **hard failure**」，
落到 `g0-bootstrap:3816` 的 `F57_MIGRATION_SEQUENCE_GAP`。
审阅方求祖先闭包得 **17 对**「较早版本的属主不是本任务的 DAG 祖先」，
**其中恰含 `:11442` 明文允许并行的 `(G2-01, G1-05)`／`(G2-01, G1-06)`**。
五份计划中查不到任何一句把提交次序绑到版本次序。

**（丙）§2.2「exact 30 行」平台机制注册表漏登 5 个由现行子计划亲手创建的目录。**
本卷实数该表**恰 30 行**，并逐个反查：`platform.delivery-registry`、`powershell-trust`、
`client-common`、`ups-contract`、`authority-kernel` **五个全部不在表内**，
而 `:104` 逐字「**Planned roots are registry-reserved before their owning task creates them;
an unregistered platform directory or graph owner is rejected.**」
且该表已为 `approval`／`support`／`endpoint-policy` 等尚不存在的目录做过预留，
**说明「预留 planned root」正是它的既定用法**。
**「30 行冻结」与「未登记即拒绝」只能保一个。**

#### 本轮未做到的

- 深审**仍是抽样**：主卷 §3 独占 94% 篇幅（约 11000 行），只定点深读约 1400 行加机器扫描；
  扩展计划 Task 14 的 3800 行只读了首尾与依赖边；`.ps1`／TypeScript／Dart 侧**一概未读**。
- 审阅方**明确报告两处复现不出**：`:10441` 的 78 行探针初始哈希按文中公式试了 8 种约定组合均未命中
  （但文中未钉死末尾 LF、排序键与 wire 形态，**属欠定，不能据此断言不符**）；
  `:10893` 的「exact 137-byte `shutdown_message`」在五份计划中查不到该字面量。
  **两处均只登记未报为缺陷，本卷照此处理。**
- 其余 42 条（阻断某阶段 6、影响取值 8、仅需登记 15、好评 15）**未逐条落实**，
  本轮只处置了 3 条阻断发布与 1 条最要害的建表级分歧。

### F-59　审阅剩余项修复：四处补齐，另登记三处**本卷不代裁的**

F-58 之后继续修剩余项。本卷修四处、登记三处，其中一处是**实跑出来的、此前无人测过的**。

#### 结论一　实跑查出：一道原本绿的自检已掉到「未覆盖」，全仓无登记

`docs/ci-pipeline.md` 本批整篇重写，**删掉了第 3 节阶段表**。
本卷**实际执行** `.github/ci/verify-pipeline-commands.sh`：

```
退出码: 3
未覆盖　文档 …/docs/ci-pipeline.md 中找不到阶段表，一致性判定未做出
结论：未覆盖 1 处，判定未做出，不得视为通过。
```

而 `.github/ci/pipeline-stages.tsv` 仍逐行登记阶段并自称「这张表是流水线的唯一真值」，
`.github/` 本批改动 **0 行**。**审阅方按解析式复算得出同一结论但自标「未在机器上验证」，本卷实跑证实。**

**这条尤其要紧**：本仓的头号纪律逐字是「**未覆盖 ≠ 通过**」，
而这正是一道从「通过」掉进「未覆盖」的自检，**且掉下去这件事在 172 个文件里一字未提**。
转绿判据：`docs/ci-pipeline.md` 恢复可被该 `sed` 解析的阶段表，或同批改写脚本第五节的比对口径。

#### 结论二　F-50 自己的关闭前置**当前不成立**，而标题已宣告「全部关闭」

- `2026-08-21-f50-financial-consistency-design.md:12` 逐字
  「本设计经书面复核并**完成第 12 节回写后**，一次关闭 F-49 的 9 条未决事项」
- 同文件 `:916`（第 12 节完成定义第 8 条）逐字
  「文档静态检查、引用检查、**错误码检查**与 Markdown 表格检查全部通过」
- 而 `errorcodes` 现返 **1**（13 处不符），且第 12 节点名要回写的正是
  `docs/error-codes.md`、`docs/config-reference.md`、`docs/event-catalog.md` 三份

**即：关闭前置的第 8 条当前判负**，而 `00c` 的 F-50 标题逐字「F-49 九项**全部关闭**」。

**本卷不改那个标题**——九项的**实体处置**经 F-58 审阅确认为真（不是搬家），
标题所述的业务结论成立；不成立的是**它自定的形式前置**。
**两者须分开说**，故只登记不改判：
**在 `errorcodes` 返 0 之前，不得声称 F-50 已满足其第 12 节完成定义。**

#### 结论三　四处已补齐

| # | 原状（实测） | 处置 |
|---|---|---|
| 1 | 就绪评审 `:56` 把 `windows-f57-release-precommit` 记为「闭合但未执行」，而 `ci.yml` 中该 job **命中 0**（只有 `pipeline` 一个 job） | 就地更正为「该 job 当前不存在」，并写明**不存在的门禁不会失败、也不会在合并时挡任何东西** |
| 2 | `docs/adr/` 实数 **25 篇**，`README.md:54` 叙述段逐篇点名 0019–0024，**0025 零处提及** | 补入 ADR-0025（单一能力图与 feature-first 边界） |
| 3 | `config-reference.md` 第 6 节漏收本批实删的 5 个键；其中 `admission.max_concurrent_users` 与 `admission.queue_max_len` **代码仍在用** | 五键补入并写明后两键须随准入控制口径一并裁定去留，**未裁定前不得视为已作废** |
| 4 | 附录庚三索引标题止于 F-53，而 `:8330` 自订纪律逐字「本表加一行」 | 扩到 F-58 并逐条补入 F-54…F-58 |

#### 结论四　两处**本卷不代裁**，登记

- **F-50 至 F-54 五段都没有「本轮未做到的」欠账小节**（F-62 复核更正：原写「23 处、F-54 有」皆不实——按严格标题计当时为 22 处，且 F-54 段只有一至五个小节、无欠账小节）。
  本卷**不代拟他人裁定的欠账清单**——那要由作出该裁定的人自己承认没做到什么。登记为格式欠账。
- **本卷自订的必报项被跳过**：`00c:4668` 逐字
  「此后各轮的验证口径改为：报六道静态门禁各自的退出码，并同时报 `xtask ci` 的聚合码，**不得只报前者**」。
  F-50…F-54 五段与两份新评审**一处未报**。本卷自 F-58 起恢复照报。

#### 本轮未做到的

- **五份 F-57 实施计划正文（约 3 万行）、`threat-model.md` 正文、7 份 OpenAPI（约 1 万行）、
  12 份数据字典分卷仍未逐条核**——这是当前最大的未覆盖面。
- `configdoc` **313** 处（原记 312；F-59 自己往 `config-reference.md` 第 6 节补的五键各生成一条「已删除键」不符项，计数随之 +1，F-62 实跑更正）、`eventcatalog` 117 处**仍未逐条消化**，只有 F-58 的状态登记。
- `RATE_LIMITED` 的分类模型例外（F-58 结论三）**仍未裁**。
- **F-51／F-55／F-56／F-57 各自改规格的逐轮授权留证仍未找到**，证据不足未立条。
...
| `archcheck` | 0 | **0** | — | — |
| `sqlcheck` | 0 | **0** | — | — |
| `codecheck` | 1（3 处） | 1（**2 处**） | `ai-inferer` 无 crate、无 systemd 单元 | ~~阶段 13c 落 `apps/ai-inferer/`~~ **F-66 补记更正**：ADR-0019 逐字「取代……强制 `ai-inferer` 结论」，该义务已被撤销、照原判据等不到转绿；现行路径为 **G0 Task 4 process-truth 再基线**或 `ai-inferer` 真实落地，二者其一 |
| `errorcodes` | 1（13 处） | 1（13 处） | 12 条文档已登记代码未实现，1 条取值冲突见结论三 | ~~各码所属阶段实现时~~ **F-66 补记同病更正**：登记册「阶段」列指旧十四阶段（已被 F-57 取代），且登记表 `:102` 明写「G0 后机器真值是 generated error catalog/OpenAPI」——现行转绿路径为 **G0 生成式再基线＋F-57 各 G 批次实现** |
| `configdoc` | 1（312 处） | 1 | 配置键、指标**与单据类型码**三段未对齐（F-62 补：五个失败用例中两个属单据类型码段判据，不属前两段也不属 the_repository 一族） | ~~首批实施按两份 markdown 对齐~~ **F-67 补齐第四行**：register `:100`/`:104` 明写「G0 后机器真值是 generated config/metrics catalog」且两份 markdown「cannot gate」——现行路径同前三行：**G0 生成式再基线＋F-57 各 G 批次实现**（F-66 曾称「四条判据全部修正」，实落三条，此为漏改的第四条） |
| `eventcatalog` | 1（117 处） | 1 | 事件已登记、无产生点 | ~~各事件所属阶段实现时~~ **同上**：登记表 `:103` 明写「G0 后机器真值是 generated event catalog」——现行路径为 **G0 生成式再基线＋F-57 各 G 批次实现** |

全量测试：**通过 1064、失败 5、忽略 5**（修前 1063／6／5，`codecheck` 常量修复净救回一个）。**F-62 注：该数为默认 fail-fast 跑法**——`ep-xtask` 套件失败后中止，少计其后 43 个目标；`--no-fail-fast` 真全量为 **158 套件、通过 1107／失败 5／忽略 5**，失败与忽略两项两种跑法一致。
...
### 庚三　全部已裁定（含 F-50 至 F-58）

> **F-59 补**：本表原标题止于 F-53，而 `:8330` 自订纪律逐字要求「本表加一行」。F-54 至 F-58 五条按该纪律补入索引：F-54 全局登记闭合与合同终止影响面平台补齐；F-55 本地 AI／MCP／ServerAdmin 与九进程冻结；F-56 许可四态与签名模块包终态；F-57 可治理自动化底座（现行最高设计）；F-58 对 F-50…F-57 的审阅修复与门禁状态登记。
