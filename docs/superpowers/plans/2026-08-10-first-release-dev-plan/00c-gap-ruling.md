...
2. **射程**：授权覆盖 **G0 全部任务**（含 Task 1 的 configdoc 扩展面——
   `config-reference` 原句「只有用户另行明确授权后，Task 1 才可…」的条件视为随本授权满足）；
   **不解锁任何 BLOCKED_BY 依赖门**（G1…G6 的次序锁原样），**不放宽任何门禁与 UNVERIFIED 纪律**。
3. 若本读法有误，使用方一句话即可撤销：回翻两个标志位并作废本节。

全仓十九处现行断言点同批清扫（README、ci-pipeline 标志块、readiness 七处、converged、g0 两处、
另三份子计划 Status、规格 `:11`、p340、config-reference 三处、fabric-design、登记表两处、00c 四条 F-62 注）。
...
ai_mcp 分册「四张」改五张并补清单第五项与 `:281` 判据适用注（迁移落点为**推断**已明标）；
...
**这是本卷第一次把对抗验证放在提交之前**（F-58 那批是提交后验、被抓 27 处）。四路抓出的要害：
...
3. README 同行自相矛盾、readiness 三处漏清、register 分类定义未注、规格 `:11` 粗体失衡、
   ...
   ai_mcp 清单四项对五张——**逐处修正，各处留「F-65 复核注」**。
