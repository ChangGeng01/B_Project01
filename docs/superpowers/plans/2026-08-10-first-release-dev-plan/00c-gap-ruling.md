...
| 3 | **契约 §14.6 补两条边**：`SUPPLIER_CONFIRMED→PENDING_SUPPLIER_CONFIRM`（改单回退，PRD:1582）与 `COMPLETED→SUPPLIER_CONFIRMED`（红冲重开，07:1104）；**部分收货态改单无逐字依据、不加边仍待裁**；07 边表挂「不完全摘录」注；PRD:1567 的取代登记为 RULING-F63-01 | 契约 `:1477`、`07` 边表、登记表 |
...
- **3 的衍生半问（部分收货态改单回不回待确认）**：正反两侧均无逐字，不加边，仍待裁。
