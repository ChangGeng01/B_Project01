...
第三，跨模块同步调用只有一种形态。调用方 ep-app-A 依赖被调方 ep-contract-B 的 trait，实现由被调方的 ep-app-B 提供，在 apps/core-server/src/wiring/ 与 apps/job-worker/src/wiring/ 两个目录内按模块分文件注入。被调方必须与调用方同批交付，否则调用方本轮不做该调用：承载该调用的用例整体推到被调方所在批次，不得先注入空实现再回头替换，不得把验收顺延到被调方阶段。空实现这一形态整体撤销，任何以 Noop、Stub、Fake、Dummy 为前缀的实现类型不得出现在两个 wiring 目录中，由 xtask archcheck 断言，出现即构建失败。唯一例外是规格把交付时点冻结在末期的三项平台能力，即 WasmComputePort、RuleEvaluator 与 DisposalPort：三者在其交付阶段之前不注入任何实现，改由 platform_ops.degradation_windows 承载，DegradationKind 增加 WASM_COMPUTE_NOT_DELIVERED、RULE_EVALUATOR_NOT_DELIVERED 与 DISPOSAL_NOT_DELIVERED 三个取值，能力缺位时开一个降级窗口，界面与健康端点显式呈现该能力未交付，指标 ep_degradation_windows_open 自动计数；三者在能力缺位时返回可重试错误或直接拒绝，不得静默按成功路径放行。

第四，阶段顺序固定为 1 → 2 → 3a → 4 → T0 → 5 → 9a → 8 → 6 → 7 → 10 → 11 → 9b → 14，阶段 12 在阶段 10 之后与阶段 11 并行，阶段 13 在阶段 12 之后与阶段 9b 并行。T0 是插在阶段 4 与阶段 5 之间的最薄贯通线，不新增任何范围，只从阶段 5、6、9a、10、11 各取一个最小切片，定义见第 2 节总表 T0 行与第 5 节 MT0 行。T0 的体量是这五个阶段各自最小子集的当量之和，不是一个新增阶段的当量，排期按这五个子集分别估算并从相应阶段本轮工作量中扣除。阶段范围归属不动，与规格第 19 章模块映射一致，T0 只改阶段内部的工作次序。第 3 节的依赖矩阵、拆环、关键路径与并行建议全部以这条链为基准。
...
| T0 | 最薄贯通线 | 让一条合同从建单走到管理层看到一个数 | 1 至 4 | 从阶段 5、6、9a、10、11 各取最小切片：mdm 一个客户与一个产品、clm 一份单审批节点合同、sales 一张销售订单、invoice 一张销项发票与 invoice.tax_rate_options 的最小税率行、ledger 一个打开的会计期间与一张凭证、finance 一笔到款登记与最小应收台账及 finance.unbilled_ar_entries 并含 UnbilledArPort 与 ReceivableExposureQuery 与 InvoiceReversalStatusQuery 三个真实实现、reporting 一张收入报表 | 一条合同从建单走到管理层看到一个数，全程经接口或桌面端完成，不用测试夹具写库；用 ep-datagen 最小样本，不要求 scale 数据集、不要求分支覆盖、不要求四端，只要桌面端；两个 wiring 目录中不出现任何 Noop 前缀类型。体量为阶段 5、6、9a、10、11 五个最小子集的当量之和，其工作量从这五个阶段本轮扣除，不按一个新增阶段估 |
...
| 5 | 1、2、3a、4、3b | 无 | 6 补 ClmProductUsageProbe 与 SalesProductUsageProbe；8 补 InventoryMaterialUsageProbe；6、7、8、10、12 补 MasterReferenceCounter 与两个 TradeHistoryProvider；税率最小行与 TaxRateOptionQuery 已由 T0 建立，MdmTaxRateStub 撤销 |
| 6 | T0、5、9a、8 | 无 | 无。UnbilledArPort、ReceivableExposureQuery 与 InvoiceReversalStatusQuery 三者在 T0 已真实存在，交付确认三腿在本阶段一次全真接线；直运退货勾稽与采购需求派生本阶段不做，整条推到阶段 7 |
| 7 | 5、9a、8、6 | 无 | 10 把 GrniSubledgerBalanceQuery 包装为 SubledgerBalanceProvider；11 补成本退货标注。采购退货的采购发票已登记分支与供应商对账本阶段不做，整条推到阶段 10；本阶段只实现未登记分支，对已开票收货行在提交时硬阻断并返回 PROCURE.PURCHASE_RETURN.INVOICE_STAGE_PENDING，该错误码由阶段 7 登记进 docs/error-codes.md |
| 8 | 5、9a | 无 | 10 把 InventorySubledgerBalanceQuery 包装为 SubledgerBalanceProvider 并建两个勾稽视图；11 补 costing.stock_value_adjust 消费者 |
| 9a（科目、期间、凭证、过账端口、对账框架） | 5 | 无 | posting_trigger_event_types 的 13 行登记由本阶段种子迁移一次写全，业务阶段不再回填；7、8、10、11 与 9b 补各自的 ReconCheck 实现与注册 |
...
1 → 2 → 3a → 4 → T0 → 5 → 9a → 8 → 6 → 7 → 10 → 11 → 9b → 14

这条链上共十四个环节。T0 是其中最短的一环，体量为阶段 5、6、9a、10、11 五个最小子集的当量之和，其工作量从这五个阶段本轮扣除，不作为新增阶段计入。第一次跨模块贯通因此从第十一环移到第五环，阶段 5 至 11 全部改为在这条已贯通的骨架上加厚。阶段 3b 因承载许可、全文检索与最小发布通道留在关键路径。阶段 13 不在关键路径上：阶段 13 之前任何阶段都不得经配置包通道发布配置，一律用迁移与端点写入，该通道在阶段 13 之前没有消费者，阶段 13 因此在阶段 12 之后与阶段 9b 并行。不在关键路径上的是阶段 12 与阶段 13。
...
| A-21 | 各模块把事件类型名登记到 ledger.posting_trigger_event_types | 阶段 9a | 阶段 6、7、8、10 均未列为交付动作 | 表、登记接口与全部 13 行登记行均归阶段 9a，业务阶段不再回填，阶段 8 显式登记零行 | ledger.posting_trigger_event_types 与 ep_contract_ledger::PostingTriggerRegistry::assert_registered；阶段 9a 的第 14 号种子迁移一次写全 13 行并直接填入 event_type 与 registered_by_module：11 个 ledger_event_kind 各一行，INVOICE_REVERSED 与 REFUND_REGISTERED 各再加一行，YEAR_END_PL_CLOSING 一行的 event_type 保持为空；阶段 6、7、10 一律不新增 backfill 迁移，改为启动自检经 PostingTriggerRegistry::assert_registered 做只读断言比对，缺行或 ledger_event_kind 或 registered_by_module 不符即以退出码 78 启动失败；阶段 8 在计划中明写不登记任何行。原裁定的只 UPDATE 不新增行与两个 kind 各承载两个事件在算术上不可同时成立，该措辞作废；三个回填迁移原落在 procure、invoice、finance 三个早于 ledger 的目录，空库上必然失败，一并作废 | 已裁定 |
| A-22 | 处置流程对 DisposalPort 的实现 | 阶段 3b（预留桩） | 无人实现。规格要求物理删除只能由处置流程经专用路径与专用账号发起 | 阶段 14，与密钥销毁、备份保留期一并处理 | crates/platform/file/src/port/disposal.rs 的 DisposalPort 与 DisposalRequest、DisposalReceipt 由阶段 3b 定义，阶段 3b 至阶段 14 之间不注入任何实现，物理删除请求在此期间直接拒绝并开 DISPOSAL_NOT_DELIVERED 降级窗口；实现类型 OpsDisposalService 位于 crates/platform/obs/src/disposal.rs，只由 ops 专用路径与专用账号触发，执行前校验双人审批与重新认证凭证，执行后写审计并生成销毁证明；阶段 2、3、13 的物理删除路径一律指向该实现 | 已裁定 |
...
| C-11 | 税率字典 | 阶段 5（mdm.classification_items 承载税率预置）与阶段 10（invoice.tax_rate_options） | 两处取值 | 阶段 10 | 唯一出处为 invoice.tax_rate_options，取用入口为 ep_contract_invoice::TaxRateOptionQuery 的 default_rate 与 list；阶段 5 的 classification_items 去掉税率一类，税率最小行与 TaxRateOptionQuery 由 T0 建立，桩类型 MdmTaxRateStub 撤销；阶段 10 交付税率迁移文件并撤销该桩；阶段 6 取默认税率一律经 ep-contract-invoice，不经 ep-contract-mdm | 已裁定 |
...
| MT0 | T0 结束 | 最薄贯通线。桌面端建一个客户与一个产品、录一份单审批节点合同并生效、派生一张销售订单、开一张销项发票、登记一笔到款、生成一张凭证并落在打开的会计期间、在收入报表上看到这笔收入 | 一条合同从建单走到管理层看到一个数，全程经接口或桌面端完成，不用测试夹具写库；用 ep-datagen 最小样本，不跑 scale 数据集；不要求分支覆盖，不要求移动端、门户与四端矩阵；跨模块调用一律为真实实现，两个 wiring 目录中不出现任何 Noop 前缀类型；收入报表上的数与 ledger 科目余额差额为零 |
