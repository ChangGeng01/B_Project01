...
第 4 节的三类缺口已按归属裁定表逐条定死，下列五条通则对全部 67 条生效，各阶段不得再解释。
...
第五，迁移文件的目录归属以 db/migrations/order.toml 的 schema 位次为准，不以写入方所属模块为准。跨 schema 读写的迁移一律放在两个 schema 中位次靠后的那个目录下，使其两侧的前置对象在执行时都已建立。order.toml 的二十四项顺序仍在阶段 2 冻结后不得调整。阶段顺序与迁移执行顺序是两条不同的序，任何顺序约束都要分别核对。
...
| 2 | 1 | 无 | 3a 补审计写入；3b 补配置发布通道的接入；4 补重新认证与审批判定；9a 补 current-period-open 自检项 |
...
| A-06 | ep-platform-recon 对账框架本体与执行器 | 阶段 7、8、9、10、11 | 五个阶段都在向它注册语句集与校验项，无人交付 crate 本体、分批执行器、快照传递与系统安全上下文 | 阶段 9a | platform_core.recon_check_definitions、recon_runs、recon_discrepancies 三表，后两张带法人并按基线第 3.8 节建策略；ep-platform-recon 的 ReconCheck、ReconRegistry、ReconExecutor 与 BatchWindow、ReconRunOutcome；执行器逐法人遍历，只在单一法人上设置 app.legal_entity_id，快照由 UnitOfWork::snapshot_transact 导出并逐批传递；注册方固定为阶段 7、8、9b、10、11 五个，一律实现 ReconCheck 并在 wiring 注册，校验项数依次为六、二、四、一、三共十六个，其中阶段 10 的一个 check_code 取 FIN_CROSS_MODULE_LINK、category 取 CROSS_MODULE_LINK；阶段 14 只调用 ReconExecutor::run 不注册，阶段 13 不涉及对账；阶段 3b 的附件孤儿收敛改称 job-worker 内的幂等收敛任务，不使用该框架 | 已裁定 |
...
| A-20 | 各业务用例的能力域码与动作类别常量声明 | 阶段 13 | 十八项能力域码表与五项动作类别在阶段 13 才定义，各业务阶段无法提前声明 | 阶段 1 定义枚举并回写基线第 12 节，阶段 2 与 3b 至 14 共十三个阶段各自声明常量，阶段 13 既声明自身两段路由的常量也做运行期判定 | crates/foundation/src/capability.rs 的 CapabilityDomain 十八项与 ActionClass 五项（Read、Write、Submit、Approve、Export），序列化取值与阶段 13 第 4.4 节能力域码表逐项一致；常量落在承载该路由处理器的 crate 的 src/capability.rs：业务模块落 `crates/contract/<module>/src/capability.rs`，`/api/v1/platform/` 下的平台路由落 A-20 逐阶段指名的 platform crate 并一律取 CapabilityDomain::PlatformAdminLowcodeOps；每个用例声明 `<USECASE_SCREAMING>_DOMAIN` 与 `<USECASE_SCREAMING>_ACTION` 一对常量，xtask configdoc 断言每个 /api/v1/ 路由都能解析到一对常量，缺失即构建失败，ci-probe 门控的探针路由与 /internal/v1/ 端点不参与 | 已裁定 |
...
| C-22 | 复制交叉核对指标名 | 阶段 2 的 ep_db_replication_crosscheck_age_seconds 与阶段 14 的 ep_replication_crosscheck_age_seconds | 两个名字 | 注册与填充均归阶段 14 | ep_replication_crosscheck_age_seconds，类型 gauge，标签 channel（archive、backup）；阶段 2 的 ep_db_replication_crosscheck_age_seconds 作废，阶段 2 只交付 ep-adapter-db-pg 的复制交叉核对取数函数与只读分析池中划出的独占连接，不登记也不填充该指标，其计划第 7.2 节的指标条目由五个减为四个 | 已裁定 |
...
十四个阶段划出十二个可演示节点。演示的判据是能在一台服务器上从客户端或接口跑完，不允许用测试夹具直接写库来构造前置数据。里程碑按阶段编号命名，其实际达成次序按第 3.4 节的链条，即 M1、M2、M3、M6、M5、M4、M7 之后 M8 与 M10 并行，随后 M9、M11、M12。唯一例外是 M5，阶段 8 按其硬边界只提供十个只读端点、不提供任何库存写端点，其写入侧一律经 ep-testkit 的 InventoryPostingDriver 调用过账端口驱动，仍不得直接写库，读出侧必须经只读端点与四端界面验证；本例外逐条列举，未被列举的里程碑不得援引。
...
| M5 | 阶段 8 结束 | 库存内核。经 InventoryPostingPort::post_inbound 的收货过账与移动加权平均单价重算、并发出库、结存归零出清、经 InventoryVariancePort::split_variance 的价差拆分三种覆盖情形、序列号扫码校验（A6 与 A7 的即时反馈经移动端真实调用） | ep-testkit 的两账同源、数量守恒、存货勾稽三组断言全绿；并发出库场景下单价重算无丢失更新；真实采购收货单登记路径顺延到阶段 7、真实进项发票价差拆分路径顺延到 M7，由 ep-app-purchase 与 ep-app-invoice 各自补验 |
...
演示节奏的三条纪律。其一，M7 之前不允许对外承诺闭环，因为在阶段 10 之前发票与应收应付缺位，第 8 章第 6 至 10 步无法连通。其二，M5、M7、M8、M9 四次演示必须在 ep-datagen 的默认 scale 数据集上跑，不允许用小样本，因为规格附录 A.1 的度量结论只在该规模下成立。其三，判据中标注顺延的项一律在其顺延到的时点补验，不得默认通过：M3 的档案停用引用计数与两个使用探针分别顺延到阶段 6、阶段 8 与阶段 12 结束，M4 的过渡科目腿净额与发票红冲判定顺延到 M7、直运退货勾稽顺延到阶段 7，M5 的收货来源单据路径顺延到阶段 7、价差拆分调用方路径顺延到 M7，M8 的 project 数据集视图顺延到 M10。
...
R14 迁移顺序与跨模块逻辑引用。基线禁止跨 schema 外键，跨模块引用不由数据库外键兜底，只由 application 层在写入时经对方模块契约校验。这意味着迁移顺序错误不会立即报错，而是在数据回填时才暴露。应对是 order.toml 的二十四项顺序在阶段 2 冻结后不得调整，任何阶段需要调整必须走基线修订。跨模块逻辑引用的周期性核对不按阶段普遍摊派，ReconCheck 的注册方与校验项数以 A-06 为唯一出处，固定为阶段 7、8、9b、10、11 五个共十六项，本条不另设注册义务；其中 category 取 CROSS_MODULE_LINK 的三项分别是阶段 7 的 R-PROC-04 覆盖 procure 与 portal、阶段 10 的 FIN_CROSS_MODULE_LINK 覆盖 finance 与 invoice、阶段 11 的第三项覆盖 costing 与 reporting。阶段 5 的 mdm 与 cpq、阶段 6 的 clm 与 sales、阶段 8 的 inventory、阶段 12 的 project 与 service，其跨模块逻辑引用在首版只由 application 层经对方模块契约做写入时存在性校验，不建周期性对账校验项，这是首版的已知边界。阶段 9a 的 ledger.vouchers 的 source_document_type 与 source_document_id，以及 release_package_id 与 approval_ref 两类平台侧逻辑引用，同属跨模块逻辑引用且同样不建周期性对账校验项；其存在性由过账入口与发布审批入口的同事务写入保证，不依赖写入时查证，9b 段自带并注册的四个校验项无一取 CROSS_MODULE_LINK。以上即全部未覆盖面，除 procure、portal、finance、invoice、costing、reporting 六个模块外，其余业务模块的跨模块逻辑引用在首版一律没有周期性核对。跨 schema 读写的迁移按通则第五条放在 order.toml 中位次靠后的那个 schema 目录下，迁移执行顺序与阶段顺序分别核对。同 schema 内的引用一律建真实外键，交付确认单与销售退货勾稽两处按 A-09 由逻辑引用改回真实外键。

R15 阶段 13 的关键路径效应。裁定后阶段 13 已在关键路径上，它自己实现六个自定义类 ConfigItemApplier，另外九个分别由阶段 3b、4、11 交付；能力域码十八项与动作类别五项由阶段 1 冻结，各阶段按 A-20 的两类落点在承载该路由处理器的 crate 的 src/capability.rs 中声明常量；受治理数据集十二个由阶段 5、6、8、10、12 各自发布。任何一个阶段漏做，阶段 13 都会直接顺延。应对是把 applier 写进阶段 3b、4 与 11 的退出条件清单，把能力域码常量写进 A-20 指名的十三个阶段即阶段 2、3b、4 与阶段 5 至 14 的退出条件清单，把受治理数据集写进阶段 5、6、8、10、12 的退出条件清单，并由 xtask 的 configdoc 与 archcheck 在每个阶段结束时校验完整性。
