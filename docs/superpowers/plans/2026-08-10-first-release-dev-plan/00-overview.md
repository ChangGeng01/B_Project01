...
R14 迁移顺序与跨模块逻辑引用。基线禁止跨 schema 外键，跨模块引用只由 application 校验并由对账周期核对。这意味着迁移顺序错误不会立即报错，而是在数据回填时才暴露。应对是 order.toml 的二十四项顺序在阶段 2 冻结后不得调整，任何阶段需要调整必须走基线修订。跨模块逻辑引用的周期性核对不按阶段普遍摊派，ReconCheck 的注册方与校验项数以 A-06 为唯一出处，固定为阶段 7、8、9b、10、11 五个共十六项，本条不另设注册义务；其中 category 取 CROSS_MODULE_LINK 的三项分别是阶段 7 的 R-PROC-04 覆盖 procure、阶段 10 的 FIN_CROSS_MODULE_LINK 覆盖 finance 与 invoice、阶段 11 的第三项覆盖 costing 与 reporting。阶段 5 的 mdm 与 cpq、阶段 6 的 clm 与 sales、阶段 8 的 inventory、阶段 12 的 project 与 service，其跨模块逻辑引用在首版只由 application 层经对方模块契约做写入时存在性校验，不建周期性对账校验项，这是首版的已知边界。跨 schema 读写的迁移按通则第五条放在 order.toml 中位次靠后的那个 schema 目录下，迁移执行顺序与阶段顺序分别核对。同 schema 内的引用一律建真实外键，交付确认单与销售退货勾稽两处按 A-09 由逻辑引用改回真实外键。
