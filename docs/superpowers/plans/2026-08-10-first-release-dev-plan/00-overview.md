...
| A-21 | 各模块把事件类型名登记到 ledger.posting_trigger_event_types | 阶段 9a | 阶段 6、7、8、10 均未列为交付动作 | 表、登记接口与全部 13 行登记行均归阶段 9a，业务阶段不再回填，阶段 8 显式登记零行 | ledger.posting_trigger_event_types 与 ep_contract_ledger::PostingTriggerRegistry::register；阶段 9a 的第 14 号种子迁移一次写全 13 行并直接填入 event_type 与 registered_by_module：11 个 ledger_event_kind 各一行，INVOICE_REVERSED 与 REFUND_REGISTERED 各再加一行，YEAR_END_PL_CLOSING 一行的 event_type 保持为空；阶段 6、7、10 一律不新增 backfill 迁移，改为启动自检经 PostingTriggerRegistry::register 与种子行比对，不一致即以退出码 78 启动失败；阶段 8 在计划中明写不登记任何行。原裁定的只 UPDATE 不新增行与两个 kind 各承载两个事件在算术上不可同时成立，该措辞作废；三个回填迁移原落在 procure、invoice、finance 三个早于 ledger 的目录，空库上必然失败，一并作废 | 已裁定 |
...
| B-02 | platform_core.append_only_registry | 阶段 2 | 阶段 8、9、10 有大量仅追加表，但三个阶段的 needs 均未提到要登记 | 登记表归阶段 2，登记行归阶段 7、8、9a、10 | 登记列以阶段 2 实建表为准，为 schema_name、table_name、mode、mutable_columns，原写的 immutable_columns 在该表上不存在且语义相反故作废；本条全部登记行一律 mode 取 APPEND_ONLY、mutable_columns 取空数组；阶段 7 登记 procure.goods_receipt_line_costings，阶段 8 登记 inventory.stock_movements 与 stock_qty_entries 与 stock_value_entries 与 variance_splits 与 stock_movement_serials 五行，阶段 9a 登记 ledger.vouchers 与 ledger.voucher_lines 与 platform_core.recon_runs 三行，阶段 10 登记 finance.unbilled_ar_entries 与 finance.cash_ledger_entries 两行；ledger.general_vouchers 全库无同名对象故删除，finance 的 receivable_entries 与 payable_entries 与 advance_receipt_entries 与 advance_payment_entries 与 overbilling_entries 五张是可更新台账故退出登记清单；检查脚本 db/checks/append_only_consistency.sql 由 xtask sqlcheck 执行 | 已裁定 |
...
R14 迁移顺序与跨模块逻辑引用。基线禁止跨 schema 外键，跨模块引用只由 application 校验并由对账周期核对。这意味着迁移顺序错误不会立即报错，而是在数据回填时才暴露。应对是 order.toml 的二十四项顺序在阶段 2 冻结后不得调整，任何阶段需要调整必须走基线修订，且每个阶段新增的跨模块逻辑引用由该阶段实现一个 category 取 CROSS_MODULE_LINK 的 ReconCheck 并注册到 ep-platform-recon 的 ReconRegistry，一个阶段一个，不按引用逐条建校验项；跨 schema 读写的迁移按通则第五条放在 order.toml 中位次靠后的那个 schema 目录下，迁移执行顺序与阶段顺序分别核对。同 schema 内的引用一律建真实外键，交付确认单与销售退货勾稽两处按 A-09 由逻辑引用改回真实外键。
