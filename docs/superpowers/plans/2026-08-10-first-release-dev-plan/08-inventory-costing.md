...
目录 `db/migrations/inventory/`，历史表 `inventory.refinery_schema_history`。`db/migrations/order.toml` 中 inventory 的位次已由基线第 3.9 节固定在 procure 之后、costing 之前，本阶段不改动该文件的模块顺序。
