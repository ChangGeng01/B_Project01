...
27. 规格第 7.6 与第 7.10 章的历史数据迁移在本阶段的承接面已判定为通过：
    历史系统只读冻结、增量追平与正式切换三个动作**各产出一条不可篡改审计证据**；
    迁移对账的借贷平衡、库存守恒与期初余额衔接三项**差异为零**（零容差，不设容差窗口）；
    数量、金额、关系、附件与哈希五项对账在迁移前后各执行一次并一致；
    校验不通过的记录**进入错误队列且未进入正式数据**（须以一条必然失败的记录实测确认）。
    **本条今日不可能通过**——其承接物尚未交付，见第 11 节所列缺口，本条即为该缺口的判负口。
...
- 第 7.6 章与第 7.10 章：历史数据迁移。**本阶段承接其切换侧与对账判负口，承接物尚未交付**，
  退出条件 27 为判负口，缺口与其成因见第 11 节缺口一。此前全部 14 份计划对本两章零承接物。
...
- 第 3.3 章与第 15.3 章的厂商部署管理通道：**首版不交付**（裁定 F-44 决定三）。
  ...
  「未启用该通道的客户按支持套餐定期回报」由本阶段交付物 10 的本地导出承接，判据见退出条件 28。
  **仍缺一项非本阶段可补的**：第 17.4 章要求的「定期回报」须写入支持套餐成为客户义务条款，
  属商业条款，本阶段不代拟，见第 11 节缺口三。
...
- 第 18 章：升级、版本与生命周期。四档时长上限与切换窗口按退出条件 24 逐档实测判定；
  ...
  「也可以通过受控更新网关在线获取」一条的范围归属待表态，见第 11 节缺口二，本阶段不认领。
...
缺口一（本阶段承接，承接物尚未交付，退出条件 27 为其判负口）。**规格第 7.6 章整节与第 7.10 章的
历史数据迁移，在全部 14 份计划中零承接物。** 本裁定实测：`字段映射`、`增量追平`、`只读冻结`、
`迁移模板`、`哈希对账`、`分批迁移`、`错误队列`、`期初余额衔接` 在 14 份计划中**各 0 次命中**。

这个缺口能存活至今有一个具体原因，值得写下来：**计划里「迁移」一词几乎全指数据库 schema 迁移**
（`ep-migrate`、`schema_history`、`-- rollback:` 段），`迁移窗口` 一词在 14 份计划中命中 35 次，
**指的是 DDL 窗口，与规格第 7.10 章的历史数据迁移窗口同名不同物**。按关键词计数会得到「覆盖充分」，
**逐处核对则一处不是**。这是本卷第三类缺陷在文档侧的形态：错了不会当场报错。

裁定 A-24「不设独立数据迁移阶段」只落了总账、往来、资金账户、库存四条**期初**通道，
**无一条对应第 7.6 章的旧系统迁移**（字段映射、清洗、试运行、错误修复、分批迁移、切换）。
两者不是一回事：A-24 管的是期初余额录入，第 7.6 章管的是从历史系统搬数据。

后果不静默：规格第 22 章第 8 条逐字要求「数据迁移、备份、升级、回退、白标和许可证完整可用」，
会在发布门禁上判不通过。**退出条件 27 今日必然判负，这是有意的**——在承接物交付前，
本阶段不得以「未覆盖」的形态通过发布门禁。

缺口三（商业条款，本阶段不代拟）。裁定 F-44 决定三选定「只做本地清单导出」后，
规格第 17.4 章的替代路径「未启用该通道的客户按支持套餐定期回报」**要求支持套餐里存在一条客户义务条款**，
约定回报周期。该条款不存在时，第 17.4 章「对超过 7 个自然日安装时限……仍未安装严重漏洞补丁的实例
发出风险告警」这一控制**仍然没有输入**——本地导出只解决「能不能导出」，不解决「客户会不会按期交回」。
另按规格第 131 行逐字「关闭部署管理通道且未按套餐约定周期回报版本与补丁状态的客户，
可承诺的最高支持等级为尽力而为级」——**支持等级阶梯也依赖该条款成立**。

缺口二（范围待表态，本阶段不认领）。**受控更新网关**在 14 份计划中 0 次命中，
而规格第 17.4 章逐字「补丁**同时提供**受控更新网关在线获取与离线补丁包两种分发方式」——
措辞不是可选。但同句又写「**完全离线客户**按约定介质渠道交付」，且该网关的运营方是厂商而非客户实例，
**它可能属于厂商侧发布设施、不属于本平台交付物**。附录甲非目标清单中亦无此条。
范围归属须由使用方表态，本阶段**不认领也不代为裁定**，只登记：
若判为平台交付物，则第 18 章「也可以通过受控更新网关在线获取」一条至今无承接方。
