...
> 开发状态：`READY_NOT_AUTHORIZED`（即 `DEVELOPMENT_AUTHORIZATION_REQUIRED`）/ `IMPLEMENTATION_NOT_STARTED`
...
本文唯一规范的项目开发状态值是 `READY_NOT_AUTHORIZED`；`DESIGN_READY=true` 只描述文档完整度，`DEVELOPMENT_AUTHORIZATION_REQUIRED` 只描述阻断原因，二者都不是另一个可执行阶段，也不授权开始实现。
