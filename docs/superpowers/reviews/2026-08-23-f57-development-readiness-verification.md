...
> **容量再基线（2026-09-01）。** 现有 1 TB 盘固定为开发/验证且 `production_eligible=false`；单盘降级生产候选要求 DATA_HDD raw `>=2,000,000,000,000` bytes；NTFS authority volume 以 `1,589,137,899,520` bytes 为名义 floor，且 checked 满足 `volume_total_bytes>=max(1,589,137,899,520,1,481,763,717,120+107,374,182,400+measured_unclassifiable_filesystem_allocation_bytes)`。生产静态包络是九桶 exact-set `161061273600,64424509440,42949672960,375809638400,375809638400,375809638400,25769803776,8589934592,51539607552` bytes，checked-sum `1,481,763,717,120` bytes，再加不可借 `107,374,182,400` bytes；十四 capacity class 与十四 canonical selector 只唯一覆盖产品管理 `data_root` 对象与登记 VSS extent，卷元数据独立实测且 `unclassified_allocation_bytes` 必须为零。RAID1 的每块成员盘也必须各自满足 2 TB 档，旧 1 TB 盘不得复用为成员或备份目标。现行判据以 Windows/P340 profile 和收敛主计划为准；本报告不声称已经实机通过。
...
| 物理业务边界 | **18** 个 exact `FeatureOwnerIdV1`（原 17，F-69 增 `inventory-costing` 后同批更新）＋ 35 个 exact `PlatformMechanismIdV1`（原 30，F-61 扩表后同批更新，F-62）；七个 CTC owner 只是首片子集；sales/purchase invoice、receivable/payable cash、operating ledger 各自单 writer | 闭合 |
...
| P340 硬件认证 | 固定 P340/i5-10500/32GiB/≥240GB runtime SSD/CMR DATA_HDD raw ≥2,000,000,000,000 bytes；NTFS 名义 floor 1,589,137,899,520 bytes，并按 `bucket sum + 100 GiB + measured filesystem metadata` 动态验卷；九桶/十四类/十四 selector 只覆盖产品管理对象与登记 VSS extent，map digest、metadata scalar、used-byte 分解及零 unknown/duplicate/cross-bucket/`unclassified_allocation_bytes` 在 4321 个一分钟样本中 exact-match；25/25 指标、十个 nested + 七个 supporting readback；内存/SSD 洁净恢复、Boot/BitLocker、签名 UPS outlet-group→P340 power-path、写缓存、SMART、温度、水位均为 typed 证据。BitLocker DATA_HDD 必须 exact `{PUBLIC_KEY,RECOVERY_PASSWORD}`、auto-unlock false；ordinary reboot 的 restricted-LocalSystem broker unlock与 clean-SSD 双人八步 reenrollment必须真实通过。POWER 还固定 BIOS `POWER_ON`、五个 Authority permanent `AUTO_START` role、无 activation continuation dormancy、exact 18-object/action-row SDDL、1074/clean-stop/outlet-off-on/clean-start 与 Authority recovery proof。SSD residency probe 必须按 Set A/Set B、八行 persistent policy、七行 telemetry policy确认最终句柄/卷/大小/保留/镜像/重建与零客户字节 | 闭合但未执行 |
...
- 对一家约 20 名活跃用户、附件量受控的合同型企业，现有 1 TB HDD 即使一年净数据看似可能装下，也因九桶故障包络、不可借空闲量和生产证据门而永久不合格。必须先使用满足 raw/NTFS 双门的 2 TB CMR 档，再由 G6 的真实 72 小时混合负载和恢复演练决定；净数据估算不能替代 fault-envelope 认证。
...
1. 用户另行明确授权开始开发（**该条件已于 2026-08-27 满足**，F-65／F-66）；
