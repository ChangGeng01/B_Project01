...
| DBP-001 | F57 | 第一阶段只认证 PostgreSQL 16 为权威数据库；保留稳定 `AuthoritativeDatabaseProvider` seam，其他数据库只能作为外部数据源或类型化命令提供者。Windows 安装由 Task 11 sole owner/schema 的 strict `Postgres16WindowsPackageLockV1`、`Postgres16WindowsInstallContractV1`、`Postgres16WindowsInstallReadbackV1` 固定；artifact set 认证 contract，contract typed-load package lock 并回指同一六组件集合，22-field service-install evidence 认证 readback；`ep-postgres16` demand-start/no-recovery，DATA_HDD gate 前零进程，engine 仅 SSD Set A，全部 PGDATA/live WAL/temp/archive/log/TLS/config 在 HDD，loopback TLS/SCRAM/checksums/durability/PITR 参数 exact-readback，禁 tablespace/reparse/trust/external CIDR/ambient override | EXPANDED | authority-database | PG16 conformance、signed install/readback parent、SCM/account/ACL/start ordering、路径/配置/TLS/防覆盖、PITR、非 PG 权威启动拒绝、外部数据源隔离和 provider seam 测试 |
...
| SEC-004 | F57 | 勒索恢复同时具备服务器外连续只追加层、至少两块 distinct `media_id` 离线 HDD 和独立恢复材料。active-config current head 与 revision/predecessor 防回滚的签名 `BackupTopologyV1` 固定按序六角色、一台 off-host target、exact A/B、writer/target SPKI join、live 管理/凭据/故障/保管/位置域、每盘两个分域 human custodian、保留和容量；strict `StorageSafeguardReadbackV1` 在 install/PITR/activation/retry 每次以新 challenge/session/object、精确 <=300s expiry 与 binding-specific checkpoint 验证 target mTLS、物理 total/free/quota/reserve、partial optionality/history、六角色权限矩阵和 A/B physical disconnect/custody/no-recovery-material。closed `StorageSafeguardSupportEvidenceV1` 使 target refs 单签、介质 transition/观察双签并 exact-bind parent projection；未知即 `NON_SUPPRESSIBLE_RISK`。writer 的 just-written read 只持一次性 affine capability；target-agent 无 direct storage operation；retry 不复用旧读回。介质序列/前驱/hash 与闭合合法边为 `BLANK→ENROLLED→ACTIVE_APPEND→VERIFIED_DISCONNECTED`，随后按 current topology 允许 active/rotation/seal/retire/destroy，不得跳边或分叉 | EXPANDED | backup | current-head/revision/fork、typed support kind/media/signer/quorum/domain、retention/capacity不等式与 overflow、mTLS/freshness/checkpoint、六角色删除/覆盖/改保留/二次读取/历史枚举、partial optionality/flood、quota/reserve、两块介质/窗口外断开、完整介质链/复用/销毁、污染点与洁净恢复测试 |
...
| NFR-004 | F57 | 当前单 HDD 是降级生产；上线需服务器外备份、UPS、恢复演练 | EXPANDED | ops | go-live gate 证据 |
| NFR-005 | F57 | UPS 与两层备份是当前上线门；唯一 UPS schema 拥有 `UpsAdapterManifestV1|UpsStatusReadbackV1|UpsOutletCycleCommandV1|UpsOutletCycleCommandAckV1` 与分离 status/control ports。Windows 标准 carrier 只可监测且 UNKNOWN 保真，最高安全档必须用候选绑定 signed vendor adapter；实现仅在 `EPAuthorityControl`，USB/网络/credential 按 service SID 和 exact endpoint 最小授权。同 identity+command ID+digest 必须 query/adopt byte-identical ACK，不同 digest conflict，未知不重发，boot change 前无 composite ACK 不补造；低电量按停止长任务、持久 checkpoint、PostgreSQL 安全停机、Windows 关机固定顺序执行。后续升级为 RAID1、64GB、增强备份设备和可选暖备 | EXPANDED | ops | standard/vendor carrier、UNKNOWN、manifest/service/config/transport/credential 漂移、同 ID adopt/conflict、response-loss/unknown/boot-change、实际断电/通信断开、900 秒 crossing、PG/Windows 顺序关机与再认证测试 |
...
| NFR-004 | [F57 §13.6「单磁盘降级生产上线门」] | [旧恢复口径「未绑定目标可作为承诺」] | [RULING-HW-01] | [ops; backup; recovery] |
