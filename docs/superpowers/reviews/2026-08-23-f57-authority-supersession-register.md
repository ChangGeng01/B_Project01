...
| PostgreSQL 16 Windows package lock、安装与读回 | F-57 汇合主计划的 canonical Task 11 三 strict root → expansion Task 11/15 实现与实机 gate → Windows/P340 生产档案；`docs/config-reference.md` 的 `db.*` 只作 G0 导入输入，不能改 package/service/path/security contract |
| 备份拓扑与防勒索现场保障 | F-57 汇合主计划的 `BackupTopologyV1|StorageSafeguardReadbackV1|StorageSafeguardSupportEvidenceV1` → expansion Task 11/15 → Windows/P340 生产档案 → threat model；ADR-0021/0024 只分别补 ciphertext/key-envelope，不替代 current-head/revision、typed target receipt、介质状态链、权限/容量/现场读回 |
| UPS 适配器与跨重启断电幂等 | F-57 汇合主计划的 UPS common contract → expansion Task 13/15 → Windows/P340 生产档案 → threat model；Windows standard power status 只作监测输入，不能替代最高档 signed vendor adapter/control ACK |
| 商业产品化 | F-57 §14 |
...
- **裁决**：package lock、服务、账户、启动/恢复、ACL、依赖、路径、有效配置、TLS、网络和 pre-HDD 零进程均由 sole-owner strict package-lock/install-contract/install-readback 固定；artifact set 认证 contract，contract typed-load lock 并回指同一六组件集合，service-install evidence 认证 readback。V1 禁止降级，package/SBOM/扩展 exact-set；九路径与读回按 enum 双射，GUC/HBA/ident 投影与 effective vector 逐字节相等。
- **实现后果**：不得新增 PostgreSQL installer/service-configuration PowerShell、独立 signer 或 backup component；已批准的 archive/PITR operational/test wrappers 保持 closed registry rows。现有 trusted installer 只解释已签 contract，different-lock build、列表顺序/大小写/重复/缺多、路径碰撞或任一现场偏差使 final-installed generation 不可达。

### RULING-BACKUP-SAFEGUARD-01：有异机目标和两块盘即可证明抗勒索

- **旧句**：off-host Boolean、两个介质 ID 和“不可覆盖”声明足以通过上线门。
- **裁决**：必须同时具备 active-config current-head/revision 防回滚的 signed topology、本 attempt 新鲜 strict safeguard readback，以及 topology-pinned target 单签/介质双签 support evidence，证明六角色与 writer/target SPKI、live 凭据/故障/管理/保管/位置域、一次性写后读、六角色权限负探针、真实 total/free/quota/reserve、partial optionality、A/B 状态链/无恢复材料/物理断开/健康/exact 两人保管；未知即不可抑制风险。
- **实现后果**：install、PITR、activation 分别 exact-bind 同一 current topology 和 binding-specific checkpoint；retry 使用新 challenge/session/object，expiry checked 且旧 readback 即使未过期也不能复用。任意 support ref kind/media/signer/projection、topology predecessor 或容量不等式漂移失败。
...
- **实现后果**：status 固定 5 秒采集/15 秒有效；adapter 在 provider 调用前耐久化同 boot/source 的 monotonic start marker，ACK 观察与 query/adopt 都受 `min(start+30s,command deadline)` 约束，UTC 不参与授权。POWER 600 秒只作 User32/composite/preshutdown 对账。同 ID 不同 digest 冲突，未知状态禁止重发，boot change 前缺 composite ACK 不得事后补造；adapter 不增加服务/vendor DLL/子进程，设备、网络和凭据只按 `EPAuthorityControl` 最小权限开放。
