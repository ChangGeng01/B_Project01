...
PostgreSQL 16 必须按唯一的 `Postgres16WindowsPackageLockV1 -> Postgres16WindowsInstallContractV1 -> Postgres16WindowsInstallReadbackV1` 认证链运行，并由 contract 内的 scan contract 与 readback 内的 coverage ref 分别闭合 `Postgres16WindowsEventLogFixtureSetV1 -> Postgres16WindowsEventLogScanCoverageV1`；唯一 schema 因而拥有 19/13/4/19/17 字段五个 strict root。package lock 的 `installed_files` 是 engine root 下全部普通文件的 canonical path/长度/SHA-256 向量，必须与离线包、SBOM 和 final-handle 独立重枚举形成完整双射；`.control` 派生的扩展集合也必须与 SBOM、available/installed/enabled 投影逐项相等。V1 只允许全新安装，或现存 package-lock、包摘要、版本/control/catalog 与 installed-file-set 全部 byte-equal 时的同锁采用/修复；既有版本无论更旧还是更新，只要不同锁就必须在任何服务或数据变更前返回 `MAINTENANCE_UPGRADE_REQUIRED`，不得靠版本比较自动升降级。

服务固定为 `ep-postgres16`，显示名 `Enterprise Platform PostgreSQL 16`，账户 `NT SERVICE\ep-postgres16`，`UNRESTRICTED`、按需启动、无自动重启恢复动作；ready 只接受 typed `RUNNING`，不能用任意 SCM 字符串或 pending/stopped 冒充。九个路径角色按固定顺序唯一出现并与 live final-handle readback 双射；每个签名路径策略保存含服务/恢复/备份占位符的未解析 `canonical_sddl_template` 及其摘要，只有服务和账户创建/采用后才从同一安装证据解析 SID、应用 ACL，并把 live canonical DACL/摘要逐角色精确比对。SSD Set A 只放 package-lock 固定的 engine，`pg_ctl runservice -N ep-postgres16 -D <signed-PGDATA> -e EnterprisePlatform.PostgreSQL16 -w` 的 PGDATA/config、live WAL、TLS、日志、archive staging、`temp/process` 和 `temp/restore` 均落在 DATA_HDD。依赖 unlock broker 只保证顺序，不代表 ready；Authority 必须在同一 boot 的卷解锁、storage manifest、vault、配置和 TLS 读回全部通过后显式启动。
...
UPS 是当前上线硬门，不是未来升级项。`crates/platform/ups-contract` 与 `docs/evidence/f57-ups-contract.v1.schema.json` 唯一拥有 `UpsAdapterManifestV1`、状态、命令、ACK 和分离的 `UpsStatusPortV1|UpsOutletControlPortV1`；P340/发布只导入。Authority 只通过现有 `EPAuthorityControl` 内静态链接、候选绑定的第一方 Rust adapter 调用这些端口，不新增服务，不加载厂商 DLL，不启动厂商子进程，不让 UPS 软件直连 PostgreSQL。manifest 的 `implementation_binary_ref` 必须 byte-equal 当前 `WindowsAuthorityArtifactSetV1.authority_kernel_binary_ref`，其重开摘要必须再与运行中 held implementation binary 相等；任何替代实现字节都失败。首发 carrier exact-set 为：
...
outlet 命令是严格 `UpsOutletCycleCommandV1`，不携带 endpoint、credential、path、argv 或厂商 raw payload。首次 provider 调用前必须耐久化 monotonic start marker；provider 返回的 `provider_operation_id` 是 1..128 字节 canonical ASCII，精确匹配 `[A-Za-z0-9][A-Za-z0-9._:-]{0,127}`，并在 ACK 前持久绑定 `(ups_adapter_identity,command_id,command_sha256)`。schedule response、exact-ID query 与 operation-log readback 必须逐字重复该 ID；空值、规范化别名、变化或跨命令复用一律进入 `COMMAND_STATE_UNKNOWN`，不能据此重发。同一 `(ups_adapter_identity,command_id)` 且同一 command digest 只能 query/adopt 并返回字节完全相同的 `UpsOutletCycleCommandAckV1`；同 ID 不同 digest 冲突，设备状态无法查询即 `COMMAND_STATE_UNKNOWN` 且禁止重发。boot 已变化但 preboot composite ACK 未落盘时仍失败关闭，不能用事后可见事件重建 PASS。
