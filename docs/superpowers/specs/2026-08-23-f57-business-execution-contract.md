...
第一阶段业务接线逐字复用客户端契约 §1.1 的 17 个 method/path pair：

- `POST /employee/v1/session/start`、`POST /employee/v1/session/handshake`、`POST /employee/v1/session/renew`、`POST /employee/v1/session/reauth`、`POST /employee/v1/session/end`
...
| `PURCHASE_ORDER_V1` | 持久生命周期；`DRAFT` | `DRAFT→PENDING_APPROVAL\|VOIDED`；`PENDING_APPROVAL→CLOSED\|ISSUED\|REJECTED`；`REJECTED→CLOSED\|DRAFT`；`ISSUED→CLOSED\|PENDING_SUPPLIER_CONFIRM`；`PENDING_SUPPLIER_CONFIRM→SUPPLIER_CONFIRMED\|SUPPLIER_RESCHEDULE_PROPOSED\|VOIDED`；`SUPPLIER_RESCHEDULE_PROPOSED→PENDING_SUPPLIER_CONFIRM\|SUPPLIER_CONFIRMED\|VOIDED`；`SUPPLIER_CONFIRMED→CLOSED\|COMPLETED\|PARTIALLY_RECEIVED\|PENDING_SUPPLIER_CONFIRM`；`PARTIALLY_RECEIVED→CLOSED\|COMPLETED\|PENDING_SUPPLIER_CONFIRM`；`COMPLETED→CLOSED\|SUPPLIER_CONFIRMED`。（F-63 补三条反向边；其 typed row 不得靠 prose 猜测：`SUPPLIER_CONFIRMED→PENDING_SUPPLIER_CONFIRM` 固定 `action_id=PURCHASE_ORDER_APPROVED_CHANGE_RECONFIRM`、`guard_ids=[PURCHASE_ORDER_CHANGE_APPROVED_V1]`、`invariant_ids=[PURCHASE_ORDER_OCCURRED_FACTS_PRESERVED_V1]`、`reverse_fact_triggers=[PURCHASE_ORDER_APPROVED_CHANGE_RECORDED_V1]`；`PARTIALLY_RECEIVED→PENDING_SUPPLIER_CONFIRM` 固定 `action_id=PURCHASE_ORDER_PARTIAL_RECEIPT_CHANGE_RECONFIRM`、`guard_ids=[PURCHASE_ORDER_CHANGE_APPROVED_V1,PURCHASE_ORDER_RECEIVED_PORTION_IMMUTABLE_V1]`、`invariant_ids=[PURCHASE_ORDER_RECEIPT_FACTS_PRESERVED_V1]`、`reverse_fact_triggers=[PURCHASE_ORDER_APPROVED_CHANGE_AFTER_PARTIAL_RECEIPT_V1]`；`COMPLETED→SUPPLIER_CONFIRMED` 固定 `action_id=PURCHASE_ORDER_CREDIT_REVERSAL_REOPEN`、`guard_ids=[PURCHASE_ORDER_INVOICED_QUANTITY_REDUCED_V1]`、`invariant_ids=[PURCHASE_ORDER_OCCURRED_FACTS_PRESERVED_V1,PURCHASE_ORDER_QUANTITIES_NONNEGATIVE_AND_BOUNDED_V1]`、`reverse_fact_triggers=[PURCHASE_INVOICE_CREDIT_APPLIED_V1]`。三者分别承接 PRD:1582 的改单回退、使用方 F-63 补答且 07:664 冻结的已收货部分数量/单价不可变，以及 07:1104 的红冲重开；重新确认后经既有 `SUPPLIER_CONFIRMED→PARTIALLY_RECEIVED` 回到部分收货态继续收货。）VOIDED 守卫为尚无收货、采购发票或已批准付款效果；CLOSED 与所有反向边都必须保留已发生事实。 | `CLOSED\|VOIDED` |
...
#### 14.6.1 状态动作、守卫、不变量与反向事实触发器登记

本表是 §14.6 具名状态语义 token 的唯一含义注册表，不是事件目录的别名。`ACTION` 表示执行状态迁移的领域动作；`GUARD` 是迁移前必须为真的 typed predicate；`INVARIANT` 是迁移前后都必须保持的事实约束；`REVERSE_FACT_TRIGGER` 是由唯一 owner 已提交、可触发反向边求值的不可变事实。销售订单用 `CHANGE_APPROVAL` 表示变更审批状态，批准后执行版本变更动作并保留版本快照事实，另行登记 `sales.sales_order.changed.v1` 通知事件，这是同类先例；采购订单同样不得把“变更已批准”“状态动作已执行”和“通知事件已投递”压成一个无类型 token。事件或事实的具体 wire 由 owning capability 的 `FactDefinitionV1`/`EventDefinitionV1` 绑定生成，本表只拥有状态机引用所需的稳定语义 ID 与精确定义。

<!-- F57-SEMANTIC-TABLE:business_state_semantic_symbol_registry_v1:BEGIN -->
| SemanticId | kind | StateDomain | exact definition |
|---|---|---|---|
| `PURCHASE_INVOICE_CREDIT_APPLIED_V1` | `REVERSE_FACT_TRIGGER` | `PURCHASE_ORDER_V1` | invoice owner 已在一个事务内提交进项红字、原票逐行分配与采购订单行回写；至少一行 `invoiced_quantity` 相对该事务前减少，且只有 owner commit 后该事实才成立。 |
| `PURCHASE_ORDER_APPROVED_CHANGE_AFTER_PARTIAL_RECEIPT_V1` | `REVERSE_FACT_TRIGGER` | `PURCHASE_ORDER_V1` | procure owner 已提交针对 `PARTIALLY_RECEIVED` 订单的唯一批准变更事实；事实绑定 order、change、before/after version，并证明已收货部分未被改写。 |
| `PURCHASE_ORDER_APPROVED_CHANGE_RECONFIRM` | `ACTION` | `PURCHASE_ORDER_V1` | 消费当前订单的唯一已批准变更，将 `SUPPLIER_CONFIRMED` 原子迁到 `PENDING_SUPPLIER_CONFIRM` 以重新取得供应商确认；不得改写任何已发生收货、发票、付款或审计事实。 |
| `PURCHASE_ORDER_APPROVED_CHANGE_RECORDED_V1` | `REVERSE_FACT_TRIGGER` | `PURCHASE_ORDER_V1` | procure owner 已提交针对 `SUPPLIER_CONFIRMED` 订单的唯一批准变更事实；事实绑定 order、change、before/after version，且先于重确认动作可见。 |
| `PURCHASE_ORDER_CHANGE_APPROVED_V1` | `GUARD` | `PURCHASE_ORDER_V1` | 当前订单与目标版本存在恰一个已 `APPROVED`、尚未应用且审批主体与 order/change/version 全部匹配的变更决定；草稿、驳回、旧版本或重复应用均为 false。 |
| `PURCHASE_ORDER_CREDIT_REVERSAL_REOPEN` | `ACTION` | `PURCHASE_ORDER_V1` | 仅消费同订单的 `PURCHASE_INVOICE_CREDIT_APPLIED_V1`，在减少后的开票数量不再满足完成条件时把 `COMPLETED` 原子迁到 `SUPPLIER_CONFIRMED`；不撤销原发票或红字事实。 |
| `PURCHASE_ORDER_INVOICED_QUANTITY_REDUCED_V1` | `GUARD` | `PURCHASE_ORDER_V1` | 已提交进项红字使至少一条关联订单行的 `invoiced_quantity` 严格小于红字前值，并使该直接费用订单不再满足完成谓词；未提交、错订单、零差额或负数结果均为 false。 |
| `PURCHASE_ORDER_OCCURRED_FACTS_PRESERVED_V1` | `INVARIANT` | `PURCHASE_ORDER_V1` | 状态反向迁移只能追加变更、红字和审计事实；既有收货、退货、原发票、付款、供应商确认及其引用和发生时点均不得删除、覆盖或重写。 |
| `PURCHASE_ORDER_PARTIAL_RECEIPT_CHANGE_RECONFIRM` | `ACTION` | `PURCHASE_ORDER_V1` | 消费部分收货后的唯一批准变更，将 `PARTIALLY_RECEIVED` 原子迁到 `PENDING_SUPPLIER_CONFIRM`；保留累计收退货事实并只允许未收货部分进入重新确认。 |
| `PURCHASE_ORDER_QUANTITIES_NONNEGATIVE_AND_BOUNDED_V1` | `INVARIANT` | `PURCHASE_ORDER_V1` | 每行在迁移前后都满足 `received_quantity>=0`、`0<=returned_quantity<=received_quantity` 与 `0<=invoiced_quantity<=quantity`；红字或重开不得制造负数、超订单量或累计漂移。 |
| `PURCHASE_ORDER_RECEIPT_FACTS_PRESERVED_V1` | `INVARIANT` | `PURCHASE_ORDER_V1` | 订单变更不得改写已提交收货、采购退货及其批次/序列事实；`received_quantity` 与 `returned_quantity` 必须继续由这些 owner facts 重算并与迁移前相等。 |
| `PURCHASE_ORDER_RECEIVED_PORTION_IMMUTABLE_V1` | `GUARD` | `PURCHASE_ORDER_V1` | 对每条已部分收货订单行，变更后的订单数量不得低于已收货净数量，且已收货部分对应的数量与净单价逐值等于生效前版本；仅未收货部分可变。 |
<!-- F57-SEMANTIC-TABLE:business_state_semantic_symbol_registry_v1:END -->

该表严格按 `SemanticId` 的 UTF-8 bytes 排序且 12 行 exact。四列 codec 固定为 `MARKDOWN_CODE_UPPER_TOKEN`、`MARKDOWN_CODE_STATE_SEMANTIC_SYMBOL_KIND`、`MARKDOWN_CODE_UPPER_TOKEN`、`UTF8_EXACT`。`STATE_SEMANTIC_SYMBOL_REGISTRY_V1` validator 必须把每行归一化成唯一 `SemanticTokenV1` key 和 typed `symbol_kind,state_domain_id,definition`；`state_domain_id` 必须 exact-resolve 到 `STATE_INVARIANT_REGISTRY`，定义不得为空。`PURCHASE_ORDER_V1` 的三个反向 transition 必须分别引用恰一个 `ACTION` 与 `REVERSE_FACT_TRIGGER`、一个或多个 `GUARD` 和 `INVARIANT`，且全部 12 行恰为这三条边的引用闭包；错 kind、重复/孤儿行、未注册 token、共享 catch-all trigger 或由 prose 临时造 alias 均阻止激活。历史 `docs/f57-api-state-domains.seed.tsv` 没有这些 typed 列，继续保持逐字节不变。
