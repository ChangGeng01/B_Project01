...
首次迁移落地及以后每次结构变更都必须同时满足数据库元数据、~~阶段 5 §3.4、阶段 6 的价格权限表定义~~（**F-68 更正**：这两处对照物属已被 F-57 取代的十四阶段；现行对照物是 **G0 生成的 data dictionary 与价格权限的 F-57 承接契约**）和本分册逐项一致；固定单目标引用的未落实真实外键数、法人列遗漏数与无候选键目标数必须均为零。
