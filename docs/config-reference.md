...
trust/topology/storage current roots 只能从 fresh `HEALTHY` 串行轮换：一个 active-config CAS 固定旧 head/旧 roots 与新 roots，进入 `TRANSITIONING`；只允许创建一份以旧 head checked+1 为序号、绑定新 roots 的 bridge checkpoint。随后状态必须是 `BOOTSTRAPPING + CURRENT_ROOTS_ROTATION`，禁止再创建 checkpoint，只能完成该 bridge 的 A/B 离线复制与验证，闭合后回到 `HEALTHY`。第二次轮换在恢复 `HEALTHY` 前禁止。所有 retained refs 都须 typed-load 为唯一连续 `BackupCheckpointV1` 链，current head exact-bind current trust/topology/storage tuple；`INITIALIZING|BOOTSTRAPPING|TRANSITIONING|NON_SUPPRESSIBLE_RISK` 一律禁止 PITR、发布、恢复认证与 production activation，只有带显式 current head 的 fresh `HEALTHY` 可进入这些操作。
